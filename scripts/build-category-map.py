...
def main():
    ...
    parts = [
        ...
        '## Three-line Summary',
        '',
        'Navigation layer grouping the method-based wiki categories under a 6-stage clinical workflow (diagnosis → hard-tissue surgery → perio/pharmacology → conservative/endodontics → prosthodontics/implants → orthodontics/specialty); folders are unchanged.',
        'Page counts are auto-tallied from each folder at every deploy build.',
        'Use this map to locate the right category before searching — method-based, not disease-based.',
        '',
        '## 세줄요약',
        '',
        'method-based 카테고리를 임상 동선 6단계로 그룹핑한 탐색 지도 — 폴더는 그대로 두고 그 위에 얹은 레이어.',
        '페이지 수는 매 배포 빌드에서 폴더로부터 자동 집계. (진단→경조직수술→치주·약물→보존·근관→보철·임플란트→교정·특수)',
        '카테고리 검색 전 이 지도로 방법론/술식 기준 폴더를 먼저 확인할 것.',
        '',
        '---',
        '',
    ]
