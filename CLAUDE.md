...
| `AUDITS.md` | 감사 추가/변경, `logs/` 해석. 15개 감사 표·compounding 지표 |
...
```bash
python3 scripts/daily-audit.py     # 15 audits → logs/
```

3 classic + 1 rationale은 error(block), 나머지 11은 **signal — 절대 block하지 않는다.** 감사는 거울이다: ingest 압력은 gate가 아니라 가시성으로 self-correct한다 (gate는 임상 워크플로에서 번아웃·회피를 유발). 개별 감사 설명·compounding 지표는 `AUDITS.md`.
