---
title: "Failure Risk of Composite Resin and Amalgam Restorations: A Systematic Review and Meta-Analysis"
authors: Woroud Al-Sulimmani, Asmaa Al-Rasheedi, Hebah Al-Daraan, Muna Al-Mutairi, Yash Brahmbhatt, Hesham Al-Hazmi, Hend Al-Qaderi
year: 2025
doi: 10.1016/j.identj.2025.100871
category: [dental-materials]
pdf_path: /Users/oracleneo/llm-wiki/papers/al-sulimman-2025-composite-amalgam-failure-risk-sr-ma.pdf
pdf_filename: al-sulimman-2025-composite-amalgam-failure-risk-sr-ma.pdf
source_collection: external
---

## Three-line Summary

SR+MA (13 clinical/observational studies; permanent posterior teeth, ≥12-month follow-up) comparing failure risk of composite resin vs amalgam restorations.

Pooled relative risk was 0.96 (95% CI 0.68–1.34), indicating no statistically significant difference in failure risk between composite and amalgam; failure proportions ranged widely (amalgam 0–50%, composite 0–62.7%) with no significant publication bias (Egger's p>0.05).

Existing evidence is insufficient to declare one material superior; future systematic reviews need to control for caries risk, tooth position, surface number, and operator skill before recommending routine material substitution.

## 세줄요약

SR+MA (임상·관찰 연구 13편; 영구 구치부, 추적 ≥12개월) — 복합레진 vs 아말감 수복물 실패 위험 비교.

풀링 상대위험도(Relative Risk, RR) 0.96 (95% CI 0.68–1.34) — 두 재료 간 실패 위험 유의차 없음; 실패율 범위 넓음(아말감 0–50%, 레진 0–62.7%), 출판 편향 유의하지 않음(Egger p>0.05).

현존 근거는 어느 재료도 우월하다고 선언하기에 불충분; 우식 위험도·치아 위치·치면 수·술자 숙련도를 통제한 추가 체계적 문헌고찰이 필요.

## Why Ingested

[역소급 작성 2026-07-05 — 원 인제스트(2026-05-18) 당시 동기 기록 없음. [[overviews/direct-restorative-materials-history-overview]]에서 확인되는 현재 역할을 백필.] [[overviews/direct-restorative-materials-history-overview]]의 "후방부-내구성 라인(아말감)" 4편 중 하나 — worthington-2021(Cochrane, 5년 유사)·tobias-2024(실사용 65만+명, HR 1.29)·bhagwat-2025(SR, 아말감 16년 vs 레진 11년) 사이에서 RR 0.96(NS)이라는 "유의차 없음" 결과를 제공, 재료 우월성 논쟁이 아직 미해결임을 보강.

## 1. Document Information
- **Journal**: International Dental Journal (Elsevier/FDI), 2025
- **DOI**: 10.1016/j.identj.2025.100871
- **Available online**: 26 June 2025
- **Study type**: Systematic review + meta-analysis (PRISMA)

## 2. Key Contributions
- 13 studies included; primary outcome = restoration failure (replacement, fracture, secondary caries, postoperative sensitivity, toothache)
- **Meta-analysis result**: RR 0.96 (95% CI 0.68–1.34) — no statistically significant difference between composite and amalgam
- Failure proportions: amalgam 0–50%, composite 0–62.7% (wide between-study variation)
- No significant publication bias (Egger's test p > 0.05)
- Conclusion: existing evidence insufficient to declare one material superior; future reviews need to control for confounders (caries risk, operator skill, tooth position)

## 3. Methodology
- **Databases**: PubMed, Cochrane, Google Scholar
- **Inclusion**: Clinical trials + observational studies; English-language; ≥12 months follow-up; studies from 1990 onward; permanent posterior teeth
- **Quality assessment**: Cochrane RoB (RCTs); Newcastle-Ottawa Scale (observational)
- **Meta-analysis**: Random-effects RR; publication bias via Egger's test

## 4. Key Results
| Outcome | RR | 95% CI | Significance |
|---|---|---|---|
| Failure risk (composite vs amalgam) | 0.96 | 0.68–1.34 | NS |

Failure proportion range: amalgam 0–50%, composite 0–62.7%

## 5. Limitations
- Heterogeneous failure definitions across 13 studies
- Did not account for caries risk category, tooth position (single vs multi-surface), operator experience
- No subgroup analysis by surface number
- Short follow-up in some included studies

## 6. Related Work
- Bhagwat et al. 2025 (amalgam vs composite SR, Cureus)
- Tobias et al. 2024 (big-data real-world amalgam vs composite, Bioengineering)
- Aswal et al. 2023 (CAD/CAM vs conventional SR+MA, Cureus)
