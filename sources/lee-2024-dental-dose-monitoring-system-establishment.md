---
title: "Establishment of dose monitoring system for dentistry"
authors: Chena Lee
year: 2024
doi: null
category: [radiology]
pdf_path: /Users/oracleneo/llm-wiki/papers/lee-2024-dental-dose-monitoring-system-establishment.pdf
pdf_filename: lee-2024-dental-dose-monitoring-system-establishment.pdf
source_collection: external
---

## Why Ingested

신규 [[radiology]] subcategory의 첫 paper. 한국 치과 방사선량 모니터링 시스템 국가 단위 구축 — 우리 클리닉 CBCT 사용 빈도 증가 + 환자 방사선 노출 우려 대응 SOP 근거. [[radiology/kang-2024-national-dental-radiological-dose-management]] DRL paper와 한 쌍.

## Three-line Summary
대한치과의사협회지 2024: 한국 치과 X-ray 장비 (intraoral·panoramic·CBCT)의 국가 단위 dose monitoring system 구축 — 통합 data collection 계획·pilot operation·전문가 의견 기반 향후 확장.


## 1. Document Information
- 대한치과의사협회지 (Journal of Korean Dental Association) 62(2), 2024
- Chena Lee, Department of Oral and Maxillofacial Radiology, College of Dentistry, Yonsei University
- 한국질병관리청 (KDCA) 연구 지원

## 2. Key Contributions
- Intraoral·panoramic·CBCT 3종 dental X-ray system 구조 조사
- 국가 단위 통합 dose monitoring system architecture 제안
- Pilot operation 결과 + future extension 권고

## 3. Methodology and Architecture
- System design + survey
- 통합 data collection plan 수립
- Pilot operation 실시
- 전문가 의견(gathering expert opinions) 기반 시스템 보강

## 4. Key Results and Benchmarks
- Dental X-ray equipment 3종 (intraoral·panoramic·CBCT) 구조 분석
- 국가 단위 dose monitoring system 시범 구축
- 향후 확장 방향 + system supplements 제시

## 5. Limitations and Future Work
- Pilot 단계 — 전국 deployment 미확정
- Operator·imaging mode 별 가변성 표준화 필요
- 통합 reporting platform 부재

## 6. Related Work
- [[radiology/kang-2024-national-dental-radiological-dose-management]] — DRL 관점, 같은 시점·같은 issue

## 7. Glossary
- DRL: Diagnostic Reference Levels
- ICRP: International Commission on Radiological Protection
- CBCT: Cone-Beam Computed Tomography
