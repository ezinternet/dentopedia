---
title: "Socket-shield Technique vs Conventional Immediate Implant Placement for Esthetic Rehabilitation: A Systematic Review and Meta-analysis"
authors: Gharpure A.S., et al.
year: 2022
doi: N/A
category: ['immediate-implant']
pdf_path: /Users/oracleneo/llm-wiki/papers/gharpure-2022-socket-shield-conventional-implant-sr-ma.pdf
pdf_filename: gharpure-2022-socket-shield-conventional-implant-sr-ma.pdf
source_collection: external
---

## One-line Summary
소켓실드(socket shield) 기법 vs 기존 즉시임플란트 비교 SR+MA.

## 1. Document Information
- **Title**: Socket-shield Technique vs Conventional Immediate Implant Placement for Esthetic Rehabilitation: A Systematic Review and Meta-analysis
- **Authors**: Gharpure A.S., et al.
- **Year**: 2022
- **Journal**: Journal of Oral and Maxillofacial Surgery
- **DOI**: N/A
- **Study Type**: sr+ma
- **Category**: immediate-implant

## 2. Key Contributions
- 소켓실드군: 협측 골 흡수 유의하게 적음 (MD -0.5mm)
- 협측 연조직 퇴축: 소켓실드 우위
- 소켓실드 합병증(shield 이탈, 노출) 보고

## 3. Methodology and Architecture
PubMed/Embase 체계적 검색; RCT+전향적 연구 포함.

## 4. Key Results and Benchmarks
소켓실드 협측 골 보존 효과 통계적 유의. 그러나 합병증(shield fracture/exposure) 5–17% 보고.

## 5. Limitations and Future Work
포함 연구 품질 다양; 장기 자료 부족.

## 6. Related Work
- [[immediate-implant/socket-shield/atef-2021-socket-shield-xenograft-immediate-implant-rct]] — related
- [[immediate-implant/socket-shield/ogawa-2022-socket-shield-technique-systematic-review]] — related

## 7. Glossary
socket shield: 소켓실드; shield exposure: 실드 노출(합병증); buccal bone: 협측 골
