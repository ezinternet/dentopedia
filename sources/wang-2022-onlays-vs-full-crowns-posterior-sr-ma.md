---
title: "Onlays/partial crowns versus full crowns in restoring posterior teeth: a systematic review and meta-analysis"
authors: Bingjie Wang, Jiayan Fan, et al.
year: 2022
doi: 10.1186/s13005-022-00337-y
category: ['inlay']
pdf_path: /Users/oracleneo/llm-wiki/papers/wang-2022-onlays-vs-full-crowns-posterior-sr-ma.pdf
pdf_filename: wang-2022-onlays-vs-full-crowns-posterior-sr-ma.pdf
source_collection: external
---

## Three-line Summary
구치부 수복: 온레이/부분관 vs 완전관 비교 SR+MA — 생존율, 합병증, 치질 보존 이점.

## 1. Document Information
- **Title**: Onlays/partial crowns versus full crowns in restoring posterior teeth: a systematic review and meta-analysis
- **Authors**: Bingjie Wang, Jiayan Fan, et al.
- **Year**: 2022
- **Journal**: Head & Face Medicine
- **DOI**: 10.1186/s13005-022-00337-y
- **Study Type**: sr+ma
- **Category**: inlay

## 2. Key Contributions
- 5년 생존율: 온레이/부분관 93.5% vs 완전관 94.2% (차이 없음)
- 온레이/부분관: 치질 삭제량 현저히 적음(최소 침습)
- 합병증 유형 차이: 온레이에서 파절 더 많음

## 3. Methodology and Architecture
PubMed/Embase/Cochrane SR+MA; 무작위·비무작위 임상 연구.

## 4. Key Results and Benchmarks
생존율 유사. 온레이: 치질 보존 + 생존율 동등. 파절 합병증 낮지 않음 → 재료 선택·교합 설계 중요.

## 5. Limitations and Future Work
이질성; 장기 자료 부족.

## 6. Related Work
- [[inlay/prott-2025-partial-coverage-restorations-posterior-scoping]] — related
- [[inlay/bresser-2023-intracoronal-restorations-longevity-sr-ma]] — related

## 7. Glossary
onlay: 온레이; partial crown: 부분관; full crown: 완전관; MIR: Minimally Invasive Restoration 최소침습수복
