---
title: "Comparison of 2002 AECG and 2016 ACR/EULAR classification criteria and added value of salivary gland ultrasonography in suspected primary Sjögren syndrome"
authors: Maëlle Le Goff, Divi Cornec, Sandrine Jousse-Joulin, et al.
year: 2017
doi: 10.1186/s13075-017-1475-x
category: [drug/systemic-disease]
pdf_path: /Users/oracleneo/llm-wiki/papers/legoff-2017-sjogren-classification-criteria-comparison.pdf
pdf_filename: legoff-2017-sjogren-classification-criteria-comparison.pdf
source_collection: external
---

## One-line Summary
pSS 의심 환자에서 2002 AECG vs 2016 ACR/EULAR 기준 비교 + 침샘 초음파(SGUS)의 부가가치 평가.

## 1. Document Information
- **Title**: Comparison of 2002 AECG and 2016 ACR/EULAR classification criteria and added value of salivary gland ultrasonography in suspected primary Sjögren syndrome
- **Authors**: Maëlle Le Goff, Divi Cornec, Sandrine Jousse-Joulin, et al.
- **Year**: 2017
- **Journal**: Arthritis Research & Therapy
- **DOI**: 10.1186/s13075-017-1475-x
- **Study Type**: cross-sectional
- **Category**: drug

## 2. Key Contributions
- 두 기준 간 일치도(κ=0.73) 및 차이 분석
- SGUS 추가 시 진단 정확도 향상 — 특히 항체 음성 환자에서 유용
- SGUS의 신기준 통합 가능성 제안

## 3. Methodology and Architecture
단일기관 전향적 코호트; 170명 pSS 의심 환자.

## 4. Key Results and Benchmarks
신기준이 AECG보다 민감도 약간 높음; SGUS 이상 소견은 항체 음성 환자 분류에 도움.

## 5. Limitations and Future Work
단일기관; SGUS 판독자 간 신뢰도 이슈.

## 6. Related Work
- [[drug/shiboski-2016-acr-eular-sjogren-classification-criteria]] — related

## 7. Glossary
SGUS: Salivary Gland UltraSound 침샘초음파; AECG: American-European Consensus Group 기준
