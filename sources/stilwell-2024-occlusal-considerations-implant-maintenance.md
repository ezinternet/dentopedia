---
title: "Occlusal considerations in maintaining health of implants and their restorations"
authors: Charlotte Stilwell
year: 2024
doi: 10.1038/s41415-024-7407-7
category: [implants]
pdf_path: /Users/oracleneo/llm-wiki/papers/stilwell-2024-occlusal-considerations-implant-maintenance.pdf
pdf_filename: stilwell-2024-occlusal-considerations-implant-maintenance.pdf
source_collection: external
---

## Three-line Summary
BDJ 2024;236(10):773: narrative overview of occlusal checks for implant maintenance — hardware complications (fracture 0.5%), biological complications (MBL, peri-implantitis), 4-step annual occlusal assessment protocol.

## 1. Key Results
- Implant fracture rate: ~0.5% (rare); occlusal overload risk is real but uncommon
- Osseointegration biophysics differ from PDL: no shock absorption, reduced proprioception
- Hardware complication risks: screw loosening, component fracture, restoration fracture
- Biological risks: bone loss, peri-implantitis from occlusal overload
- 4-step occlusal check: patient input + restoration integrity + occlusal scheme + protection + spatial changes
- Parafunction: major risk factor — requires additional protection (e.g., splint)
