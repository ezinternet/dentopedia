---
title: "Association of the Inferior Alveolar Nerve Position and Nerve Injury: A Systematic Review and Meta-Analysis"
authors: Yangjie Li, Ziji Ling, Hang Zhang, Hanyu Xie, Ping Zhang, Hongbing Jiang, Yu Fu
year: 2022
date: 2022-09-16
doi: 10.3390/healthcare10091782
source: li-2022-ian-position-nerve-injury-sr-ma.md
category: [oral-surgery]
confidence: sr+ma
pdf_path: /Users/oracleneo/llm-wiki/papers/li-2022-ian-position-nerve-injury-sr-ma.pdf
pdf_filename: li-2022-ian-position-nerve-injury-sr-ma.pdf
source_collection: external
tags: [ian, inferior-alveolar-nerve, iac, canal-position, lingual, buccal, third-molar, nerve-injury, paresthesia, meta-analysis, cbct, opg, risk-factor]
superseded_by: thereza-bussolaro-2026-cbct-ian-predictive-factors-sr-ma
superseded_scope: partial
---

> [!note] Partially superseded → [[oral-surgery/thereza-bussolaro-2026-cbct-ian-predictive-factors-sr-ma]]
> 18-study SR+MA (GRADE moderate-high) reports lingual position OR 3.82 (95% CI 1.95–7.39), updating this paper's OR 4.96 (I²=65%, directional only). Thereza-Bussolaro also quantifies 7 additional CBCT features. This page retains value as the foundational CBCT-position reference and as the anchor OR cited in earlier overviews. (set 2026-06-26)

## One-line Summary

Systematic review and meta-analysis (Healthcare 2022; 11 studies) of inferior alveolar canal (IAC) position relative to lower third molars: a lingual IAC position is significantly associated with higher inferior alveolar nerve (IAN) injury risk than a buccal position, supporting CBCT-based pre-surgical nerve-position assessment.

## 한줄요약
하치조신경관(IAC) 위치(설측 vs 협측)와 하치조신경(IAN) 손상 연관성 SR+MA (Healthcare 2022): 11편 포함; 설측 위치 → 신경 손상 위험 유의하게 높음; 사전 영상평가로 고위험군 식별 가능.

## Summary
Systematic review and meta-analysis comparing IAN injury rates between buccal and lingual positions of the inferior alveolar canal (IAC) relative to lower third molars (LM3). Searched PubMed, Cochrane CENTRAL, Web of Science, Journals@Ovid (2009–2021). 1,063 articles initially searched; full texts of 53 read; 11 studies included. Meta-analysis via Review Manager (Cochrane).

Lingual IAC position (canal on tongue side of roots) is significantly associated with higher IAN injury rate compared to buccal position. Provides quantitative justification for CBCT-based pre-surgical nerve position assessment.

## Key Contributions
- Lingual IAC: highest IAN injury risk — the nerve has less bone buffer between root apex and nerve when canal is lingual
- Buccal IAC position: comparatively lower risk, though not risk-free
- Supports CBCT use when OPG suggests IAC-root proximity — lingual position determination requires cross-sectional imaging
- Meta-analysis provides the strongest current evidence for this specific anatomical risk factor

## Methodology
- **Databases**: PubMed, Cochrane CENTRAL, Web of Science, Journals@Ovid
- **Period**: 2009–2021
- **Included**: 11 studies comparing IAN injury rate by IAC position (buccal vs lingual)
- **Outcome**: IAN injury rate (paresthesia, anesthesia, neurosensory disturbance)
- **Meta-analysis**: Review Manager (Cochrane Collaboration)

## Results

| IAC Position | IAN Injury Risk |
|---|---|
| Lingual (tongue-side) | Significantly higher — highest risk |
| Buccal (cheek-side) | Lower |
| Meta-analysis result | Lingual position: significantly increased IAN injury (statistically significant) |

## Related Papers
- [[oral-surgery/sarikov-2014-ian-injury-third-molar-review]] — IAN paresthesia 0.35–8.4%; additional risk factors (age, impaction type, surgeon experience)
- [[oral-surgery/damico-2025-third-molar-surgical-indications-guidelines]] — coronectomy as option for high IAC-risk cases
