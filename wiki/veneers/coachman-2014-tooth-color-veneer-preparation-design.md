...
- **Design**: technique-description case report (single illustrative patient).
- **Planning**: DSD photographic/video analysis → Visagism-guided anatomy → additive diagnostic wax-up → silicone indexes → bis-acrylic APT (Luxatemp, DMG) intraorally → patient/team approval → impression of approved mock-up.
- **Preparation through the APT**: single-ringed depth burs (0.3, 0.5, 0.7 mm) cut grooves on the mock-up surface; sequence = incisal grooves (1–1.5 mm for incisal stratification) → buccal grooves (depth set by required color change, here 0.2–0.5 mm) → union of grooves → APT removed → cervical/interproximal finishing lines under microscope with retraction cords.
- **Restoration**: ultrathin lithium disilicate (IPS e.max, Ivoclar), ~0.3 mm buccal thickness. Feldspathic ceramic discussed as an alternative (lower flexural strength, greater layering versatility).

## Results

Per-region calculation for the case (A2 → A1), from the paper's Table 1:
...
- **Middle and incisal thirds: zero reduction** (EV ≥ LT) → enamel preserved entirely in those regions.
- Buccal reduction limited to 0.2–0.5 mm for the small color change; final veneers ~0.3 mm.
- **Limitations**: case report only (no survival/comparative data); additive mock-ups are unsuitable for malformed/malpositioned/crowded teeth needing prosthetic reorientation (orthodontics or an initial prep may be required); significantly dark teeth and diastema closure require intrasulcular margin extension; incisal preparation design remains debated in the literature.
