...
| `radiology/mri` | 방사선학·MRI | MRI as a dental diagnostic imaging modality — dentomaxillofacial MRI applications (dental/oral/maxillofacial trauma, TMJ evaluation with dental-dedicated MRI, tooth-enamel ZTE imaging), MRI artifacts from dental materials & implants (titanium/ceramic, geometry effects), and MRI physics/technique primers written for clinical readers (principles, sequences, geometric fidelity) that underpin interpretation. (X-ray/CBCT imaging → `radiology`; TMJ *treatment* papers → `tmj`) |
