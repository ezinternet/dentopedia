...
이 파일은 `wiki/{category}/` 라우팅의 단일 출처(single source of truth)다. CLAUDE.md는 라우팅 원칙만 담고, 전체 카테고리 목록·서브카테고리 분기 규칙은 여기서 관리한다. **CLAUDE.md의 repo 트리는 카테고리를 나열하지 않는다** — 두 곳에 두면 반드시 drift한다 (2026-07-15 정렬 시 실제로 CLAUDE.md 트리와 이 표가 어긋나 있었다).
