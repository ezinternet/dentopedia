---
title: "Noncarious cervical lesions: Morphology and progression, prevalence, etiology, pathophysiology, and clinical guidelines for restoration"
authors: "Goodacre CJ et al."
year: 2022
date: 2022-01-01
doi: "10.1111/jopr.13585"
source: goodacre-2022-nccl-morphology-etiology-restoration-guidelines.md
category: nccl
confidence: narrative-review
pdf_path: /Users/oracleneo/llm-wiki/papers/goodacre-2022-nccl-morphology-etiology-restoration-guidelines.pdf
pdf_filename: goodacre-2022-nccl-morphology-etiology-restoration-guidelines.pdf
source_collection: external
tags: [nccl, abfraction, sclerotic-dentin, restoration-guidelines]
---

## Three-line Summary

Narrative review (PubMed) synthesizing NCCL morphology (saucer vs V-shape), progression, prevalence (9–93% across populations, ≥50 µm/yr height gain in low D/H lesions), etiology (multifactorial: abrasion, acid, GERD, salivary hypofunction; abfraction controversial), and pathophysiology.

Prevalence increases with age (severe lesions 29.9% at 26–30 yrs vs 42.6% at 46–50 yrs); progression risk factors include low salivary buffering and dietary acid; the D/H ratio distinguishes saucer (mainly height change) from V-shaped lesions (both height and depth increase).

Clinical restorative guidelines include: delay restoration until clinically necessary; texture sclerotic dentin with fine diamond; bevel enamel margins; 30-second extended etching on sclerotic dentin; light scrubbing adhesive application (20 s); SDF as non-restorative option when caries coexists.

## 세줄요약

PubMed narrative review: NCCL 형태(saucer형 vs V자형), 진행(저 D/H 병변 연 ≥50 µm 높이 증가), 유병률(9–93%; 연령증가와 비례), 병인(다인성: 마모·산·GERD·타액기능저하; abfraction 논쟁적), 병태생리 종합.

유병률은 연령에 따라 증가(중증 병변: 26–30세 29.9% vs 46–50세 42.6%); D/H 비율이 두 형태의 진행 양상을 구분; 저 타액 완충능·식이 산이 진행 위험인자.

수복 가이드라인: 임상적 필요 전까지 수복 연기; 경화상아질을 세립 다이아몬드로 텍스처링; enamel bevel; 경화상아질 30초 연장 산부식; 접착제 가벼운 scrubbing 도포(20초); 우식 공존 시 SDF 비수복 옵션.

## Summary
This review synthesizes the NCCL literature across morphology, progression, prevalence, etiology, and pathophysiology, then derives clinical restoration guidelines. NCCLs occur either as saucer-like depressions progressing mainly in height or V-shaped notches progressing in both height and depth, with progression accelerating as the depth/height ratio rises. Prevalence ranges from under 10% to over 90% depending on population and criteria, increasing with age; premolar facial surfaces are most affected. Etiology is multifactorial (toothbrush abrasion, dietary acid, GERD, salivary hypofunction), with the abfraction/occlusal-stress contribution still controversial. Management should be conservative-first; when restoration is indicated, the authors recommend surface texturing of sclerotic dentin, enamel beveling, extended 30-second dentin etching, and adhesive application with a light 20-second scrubbing motion.

## Key Contributions
- Two-type morphologic framework (saucer vs V-shaped) with distinct progression behavior; D/H ratio as progression predictor.
- Quantitative progression benchmarks: >=50 µm/yr height progression in low-D/H lesions; volume loss 0.36-1.50 mm3/yr.
- Balanced reading of the abfraction controversy (supportive and opposing literature both cited).
- Step-by-step restorative protocol addressing sclerotic dentin's etch resistance; SDF as non-restorative option when caries coexists.

## Methodology
PubMed narrative search covering NCCL morphology, progression, prevalence, etiology, pathophysiology, and restoration. No systematic review protocol, risk-of-bias assessment, or meta-analysis; evidence integrated qualitatively with clinical guidelines proposed by the authors.

## Results
Prevalence reports span 9.1-93% (e.g., 13.1-17.1% in lower-range studies vs 61.7-63% in higher-range studies); per-tooth involvement 5.7-17.2%. Severe lesions: 29.9% of ages 26-30 vs 42.6% of ages 46-50. Progression risk: low salivary buffering, age, dietary acid. Restorative guidance: delay until clinically necessary; composite resin most common material; texture sclerotic dentin with fine diamond; 30-s etching of sclerotic dentin; light scrubbing adhesive application 20 s; flowable + sculptable composite layering acceptable.

## Related Papers
- [[nccl/grippo-2012-abfraction-abrasion-biocorrosion-nccl-20year]] — extends (adopts multifactorial framing Grippo formalized)
- [[nccl/roberts-2022-demineralization-attrition-erosion-abfraction-nccl]] — companion pathophysiology review (shared author Roberts)
- [[nccl/peumans-2020-nccl-treatment-when-why-how]] — reinforces (conservative-first treatment philosophy)
- [[resin-bonding/doshi-2023-universal-adhesive-nccl-strategy-sr-ma]] — applies-to (adhesive strategy evidence for NCCL restoration)
