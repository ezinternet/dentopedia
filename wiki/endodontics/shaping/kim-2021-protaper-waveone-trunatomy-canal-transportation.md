---
title: "Comparison of the canal transportation of ProTaper GOLD, WaveOne GOLD, and TruNatomy in simulated double-curved canals"
authors: HyeWon Kim, Su-Jin Jeon, Min-Seock Seo
year: 2021
date: 2021-10-19
doi: 10.1186/s12903-021-01854-z
source: kim-2021-protaper-waveone-trunatomy-canal-transportation.md
category: [endodontics/shaping]
evidence_level: in-vitro
pdf_path: /Users/oracleneo/llm-wiki/papers/kim-2021-protaper-waveone-trunatomy-canal-transportation.pdf
pdf_filename: kim-2021-protaper-waveone-trunatomy-canal-transportation.pdf
source_collection: external
tags: [niti-file, trunatomy, protaper-gold, waveone-gold, transportation, s-shaped-canal]
---

## Three-line Summary
In vitro pre/post-staining superimposition study (n=60 S-shaped resin blocks, 3 groups × 20) comparing canal transportation at 1-mm increments from apex to 9 mm among ProTaper GOLD, WaveOne GOLD, and TruNatomy, using paired t-test and one-way ANOVA/Kruskal-Wallis.

TruNatomy showed significantly less canal transportation at 3 mm and 5 mm levels (p<0.05), the shortest preparation time, and least resin removal; ProTaper GOLD produced the largest apical transportation at 2–3 mm and removed the most resin; WaveOne GOLD was intermediate.

The minimally tapered TruNatomy system is the most conservative option for S-shaped or similarly complex curved canals, at the cost of a thinner canal wall — a trade-off requiring attention to residual dentine thickness in narrow or thin roots.

## 세줄요약
ProTaper GOLD, WaveOne GOLD, TruNatomy 3종 NiTi 성형 기구 비교 in vitro 연구 — S형 레진 블록(n=60, 3군 × 20개) 전후 착색 중첩법으로 치근단에서 9 mm까지 1 mm 간격 근관 이동량 측정(paired t-test, one-way ANOVA/Kruskal-Wallis).

TruNatomy는 3 mm·5 mm 수준에서 유의하게 적은 근관 이동(p<0.05), 가장 짧은 성형 시간, 가장 적은 레진 제거; ProTaper GOLD는 치근단 2–3 mm 이동량 최대·레진 제거량 최다; WaveOne GOLD는 중간.

최소 테이퍼의 TruNatomy가 S형 만곡 근관에서 가장 보존적 — 단, 얇은 근관벽은 좁거나 세장한 치근에서 잔존 상아질 두께(Residual Dentin Thickness) 주의 필요.

## Summary
Three nickel-titanium shaping systems were compared in 60 S-shaped resin canals. The minimally-tapered TruNatomy showed the lowest canal transportation in the apical region and the shortest preparation time, while ProTaper GOLD removed the most resin and produced the largest apical transportation. Findings support TruNatomy as a more conservative option for S-shaped or otherwise complex canals.

## Key Contributions
- Same-block, identical-protocol comparison of three NiTi systems
- Quantitative apical-curve transportation data favoring TruNatomy
- Shaping time and resin-removal data for chair-side workflow planning

## Methodology
- Design: in vitro experimental
- Sample: 60 S-shaped resin blocks, 3 groups × 20
- Procedure: pre/post-staining superimposition, transportation measured at 1-mm increments from apex up to 9 mm
- Statistics: paired t-test within group, one-way ANOVA and Kruskal-Wallis between groups, α=0.05

## Results
- **TruNatomy**: significantly less canal transportation at 3- and 5-mm levels (p<0.05)
- **ProTaper GOLD**: largest transportation in apical curve (2-3 mm levels, p<0.05)
- Resin removal: TruNatomy < WaveOne GOLD < ProTaper GOLD across all sections (p<0.05)
- Shaping time: TruNatomy < WaveOne GOLD < ProTaper GOLD (p<0.05)

## Limitations
- Resin blocks lack dentin hardness/microhardness
- Operator skill not reported
- Single canal geometry (S-shape)
- No clinical endpoints (success, pain, healing)

## Related Papers
- [[endodontics/shaping/almohareb-2023-cyclic-fatigue-niti-autoclave]] — same files, fatigue side
- [[endodontics/shaping/khalil-2019-edgeevolve-protaper-gold-cyclic-fatigue]] — ProTaper Gold fatigue under double curvature
- [[endodontics/shaping/aminsobhani-2022-glide-path-pathfinder-s-canal]] — S-shape transportation, glide path stage
- [[endodontics/anatomy/song-2008-cross-sectional-morphology-and-minimum-canal]] — minimum dentin thickness, safety margin
