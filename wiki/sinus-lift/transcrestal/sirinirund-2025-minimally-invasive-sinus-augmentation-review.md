---
title: "Minimally Invasive Sinus Augmentation: A Systematic Review"
authors: Benyapha Sirinirund, Amanda B. Rodriguez Betancourt, Riccardo Scaini, Yu-Chang Wu, Hsun-Liang Chan
year: 2025
date: 2025-01-01
doi: 10.1111/cid.13403
journal: Clinical Implant Dentistry and Related Research 2025;27:e13403
source: sirinirund-2025-minimally-invasive-sinus-augmentation-review.md
confidence: sr
category: [sinus-lift/transcrestal]
pdf_path: /Users/oracleneo/llm-wiki/papers/sirinirund-2025-minimally-invasive-sinus-augmentation-review.pdf
pdf_filename: sirinirund-2025-minimally-invasive-sinus-augmentation-review.pdf
source_collection: external
tags: [minimally-invasive, vsa, lsa, sinus-lift, endoscope, digital-planning, balloon, hydraulic]
---

## 한줄요약
SR (36연구, 2,732 MSFA): 잔존골 ≥5mm = VSA, <5mm = LSA; 스토퍼·풍선·수압·내시경·현미경 — 모든 최소침습기법에서 충분한 골증량

## Summary
SR (36연구, 2732 MSFA). 최소 침습 VSA·LSA 기법 분류·기술. **잔존 골 ≥5mm: VSA; <5mm: LSA**. 모든 최소 침습 기법에서 충분한 골증량 및 임플란트 성공 달성. 천공 발생률 대부분 미보고.

## 잔존 골높이별 술식 선택

| 잔존 골높이 | 술식 | 최소 침습 기법 |
|---|---|---|
| ≥5mm | VSA | 스토퍼 회전기, 수압, 내시경, 현미경 |
| <5mm | LSA | 소창, 디지털 가이드 |

## 최소 침습 VSA 기법
- **스토퍼 회전기**: 깊이 제한 → 맹목적 조작 개선
- **풍선/수압**: 막 박리 정밀화
- **내시경/현미경**: 막 실시간 확인, 천공 조기 발견
- **디지털 계획**: 가이드 기반 정밀 식립

## Clinical Relevance
- 최소 침습화 → 이환율↓, 합병증↓ 가능성 (직접 비교 연구 부족)
- 내시경/현미경 병용: 천공 즉시 확인 가능
- 잔존 골 경계(5mm) 기준으로 VSA/LSA 결정

## Related Papers
- [[sinus-lift/lateral/starch-jensen-2017-sinus-floor-augmentation-treatment-modalities]] — 술식 선택 기준
- [[sinus-lift/lateral/scaini-2025-lateral-sinus-floor-elevation-ridge-augmentation]] — LSA 상세 프로토콜
