---
title: "Longitudinal Study of Bone Height Change between Two Approaches for Sinus Floor Elevation"
authors: J.-H. Park, Y.-G. Kim, J.-Y. Suh, D.-H. Lee, J.-W. Kim, J.-M. Lee
year: 2023
date: 2023-06-12
doi: 10.3390/medicina59061132
source: park-2023-bone-height-change-sinus-elevation.md
category: sinus-lift/lateral
confidence: retrospective
pdf_path: /Users/oracleneo/llm-wiki/papers/park-2023-bone-height-change-sinus-elevation.pdf
pdf_filename: park-2023-bone-height-change-sinus-elevation.pdf
source_collection: external
tags: [crestal-approach, lateral-approach, sinus-elevation, 3-year-follow-up, bone-resorption, korea]
---

## 한줄요약
후향적 종단 코호트 (Retrospective longitudinal, n=103 Korean patients, 3-year follow-up): 측방 접근 (Lateral Window) 12.00 mm vs 치조정 접근 (Crestal Approach, Summers) 8.07 mm 의 graft height gain — 그러나 3년간 흡수 (Resorption) 는 거의 동일 (0.95 vs 0.98 mm), 대부분 1년차에 발생. 잔존 골 높이 (Residual Bone Height, RBH) 에 따른 술식 선택이 장기 안정성에는 차이를 주지 않는다.

## Summary
경북대 치과병원의 후향적 cohort: 103명 상악 구치부 임플란트 + sinus floor elevation 환자를 crestal vs lateral approach 로 나누어 술후 즉시, 1년, 2년, 3년 시점 orthopantomograph 로 graft 높이를 추적했다. **lateral** 이 절대 graft 높이는 약 4 mm 더 크지만, 흡수율은 동일하게 ~1 mm/3 year, 그 중 대부분이 첫 해 발생. 임플란트 술자의 선택 기준은 RBH (≥4 mm crestal, <4 mm lateral) — 장기 안정성 우려로 한쪽을 배제할 필요 없음.

## Key Contributions
- Crestal vs lateral 의 **3년 graft height** 직접 비교.
- 두 술식 모두 첫 해 (Year-1) 에 흡수가 집중되고 이후 plateau.
- 임상 선택 기준은 RBH 와 환자 침습성 선호 — 장기 결과는 동등.
- 한국 코호트 reference data.

## Methodology
- 103명 환자, 2017–2020, 경북대.
- 술식: crestal (Summers osteotome) or lateral (Tatum window) — RBH 기반 결정.
- Outcome: 술후 즉시 / 1년 / 2년 / 3년 panoramic 상 graft 높이.
- 통계: 군 간 비교 + 군 내 time-series.

## Results
| 지표 | Crestal | Lateral |
|---|---|---|
| Total bone gain (post-op) | 8.07 mm | 12.00 mm |
| 3-year total resorption | 0.98 mm | 0.95 mm |
| Year-1 resorption (largest portion) | 대부분 | 대부분 |
| Year 2–3 변화 | 최소 | 최소 |

**임상 함의**:
- RBH ≥4 mm → crestal (덜 침습적, 흡수율 동일).
- RBH <4 mm → lateral 필수 (12 mm gain 가능).
- 어느 술식이든 **1년차 흡수** 가 graft 최종 결과를 결정 → graft 재료/술기 최적화 필요.

## Related Papers
- [[sinus-lift/lateral/wen-2023-lateral-sinus-floor-defects-retrospective]] — lateral sinus floor elevation 장기 follow-up
- [[sinus-lift/transcrestal/farina-2023-minimal-invasiveness-transcrestal-sinus-elevation]] — transcrestal 비침습성
- [[sinus-lift/transcrestal/wu-2023-dynamic-navigation-transcrestal-sinus-floor]] — dynamic navigation crestal
- [[sinus-lift/lateral/yue-2022-sinus-floor-elevation-before-extraction]] — sinus lift timing
