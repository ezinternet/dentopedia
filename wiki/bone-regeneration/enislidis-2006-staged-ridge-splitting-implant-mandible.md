---
title: "Preliminary report on a staged ridge splitting technique for implant placement in the mandible: a technical note"
authors: "Enislidis G, Wittwer G, Ewers R"
year: 2006
date: 2006-01-01
doi: ""
source: enislidis-2006-staged-ridge-splitting-implant-mandible.md
category: [bone-regeneration]
confidence: case-report
source_collection: pubmed-text
full_text: false
pmid: "16796289"
source_url: https://pubmed.ncbi.nlm.nih.gov/16796289/
text_path: /Users/oracleneo/llm-wiki/papers/enislidis-2006-staged-ridge-splitting-implant-mandible.txt
text_filename: enislidis-2006-staged-ridge-splitting-implant-mandible.txt
tags: [ridge-split, greenstick-fracture, horizontal-augmentation, mandible, staged, corticotomy, abstract-only]
relations:
  - type: extends
    target: ayoub-2018-ridge-splitting-horizontal-augmentation-case
  - type: reinforces
    target: simion-1992-jawbone-enlargement-split-crest-gtr
---

## Three-line Summary

Prospective technical note (n=5 consecutive patients, 6 long-span mandibular sites, 17 implants) describing a two-stage ridge-split where a first-stage buccal corticotomy predetermines the greenstick-fracture line before the ridge is split 40 days later.

All buccal segments fractured exactly along the planned corticotomy, and all 17 implants were stable, bone-surrounded, and loaded with fixed partial dentures at 6 months.

The technique converts an uncontrollable mandibular fracture risk into a planned, periosteally pedicled buccal graft — but the evidence level is limited (small case series, abstract-only, no control arm).

## 세줄요약

전향적 기술노트(연속 5명, 하악 6부위, 17 임플란트): 1단계 협측 피질골절개(corticotomy)로 녹색골절 위치를 미리 결정한 뒤 40일 후 치조제를 분할하는 2단계 술식.

전 협측 분절이 계획된 절개선에서 골절, 17개 임플란트 전부 6개월에 골 둘러싸인 상태로 안정·고정성 보철 부하 성공.

예측 불가한 하악 골절 위험을 계획된 골막 유경 이식편으로 전환하는 개념적 기여가 있으나, 소수 증례·초록만 수집(abstract-only)으로 근거 수준 제한.

## Summary

Single-stage alveolar ridge-splitting widens a narrow ridge by deliberately producing a greenstick fracture of the buccal cortical plate, but in the mandible the cortical thickness makes the fracture line uncontrollable, so the risk of an uncontrolled complete fracture (malfracture) of the buccal segment is high. Enislidis et al. address this with a staged protocol: a first-stage rectangular buccal corticotomy defines the future fracture line; after 40 days of healing the ridge is split, leaving the buccal periosteum attached to the lateralized segment. Across 5 consecutive patients (6 long-span mandibular sites, 17 implants), all buccal segments fractured exactly along the predetermined corticotomy, and all implants were stable, bone-surrounded, and successfully loaded with fixed partial dentures at 6 months. The key conceptual advance is that the greenstick-fracture location is predetermined and the buccal segment remains a periosteally pedicled graft — its perfusion shifting from internal spongy-bone supply to external periosteal supply between the two stages.

## Key Contributions

- Identifies why single-stage greenstick fracture is **uncontrollable in the mandible** (cortical thickness) and high-risk for malfracture.
- Predetermines the fracture line with a **first-stage buccal corticotomy**, converting an uncontrolled split into a planned one.
- Preserves buccal-segment vascularity by keeping it **periosteally pedicled** (internal→external perfusion shift across the 40-day interval).
- Reports 100% planned-fracture success and 100% 6-month implant stability/loading in the small series.

## Methodology

Prospective consecutive case series (technical note). 5 patients, 6 long-span edentulous mandibular areas, ridges <5 mm wide. Stage 1: corticotomy outlining a rectangular buccal segment. 40-day healing. Stage 2: ridge split leaving buccal periosteum attached to the lateralized segment; 17 implants placed; implant–bone gap grafted with venous blood + porous algae-derived hydroxyapatite. Outcomes: planned-vs-actual fracture site, 6-month implant stability and bone coverage, prosthetic loading. No control arm; descriptive analysis only.

## Results

- All 6 buccal segments fractured **as planned** at the basal corticotomy (no reported malfracture).
- 6 months: all 17 implants stable and surrounded by bone.
- Fixed partial denture loading successful in all cases.
- Evidence level: small prospective case series (`case-report`, n<10); no inferential statistics; abstract-only ingest.

## Related Papers

- [[bone-regeneration/ayoub-2018-ridge-splitting-horizontal-augmentation-case]] — extends; single-stage ridge-split case that this staged protocol is designed to de-risk.
- [[bone-regeneration/simion-1992-jawbone-enlargement-split-crest-gtr]] — reinforces; the classic split-crest origin of the deliberate-greenstick concept.
- [[implants/versah-protocols/guner-2025-osseodensification-ridge-split-implant-stability]] — refines; modern osseodensification-assisted ridge split addressing the same width problem.
