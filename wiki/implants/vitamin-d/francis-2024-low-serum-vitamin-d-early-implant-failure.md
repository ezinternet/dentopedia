---
title: "The Relationship of Low-Serum Vitamin D and Early Dental Implant Failure."
authors: Francis JR, Barber HD, Beals D, Siu T
year: 2024
date: 2024-06-01
doi: 10.1563/aaid-joi-D-23-00168
source: francis-2024-low-serum-vitamin-d-early-implant-failure.md
category: [implants/vitamin-d]
evidence_level: prospective
source_collection: pubmed-text
full_text: false
pmid: "38530826"
source_url: https://pubmed.ncbi.nlm.nih.gov/38530826/
text_path: /Users/oracleneo/llm-wiki/papers/francis-2024-low-serum-vitamin-d-early-implant-failure.txt
text_filename: francis-2024-low-serum-vitamin-d-early-implant-failure.txt
tags: [vitamin-d, early-implant-failure, negative-result, serum-25OHD, cohort]
relations:
  - type: contradicts
    target: mohsen-2024-vitamin-d-deficiency-osseointegration-prospective
---

## Three-line Summary

Prospective cohort (174 implants / 109 patients) measuring serum vitamin D on the day of implant placement and following patients to restoration (~3–6 months) to assess correlation with early dental implant failure.

No statistically significant correlation was found between low serum vitamin D and early failure; counterintuitively, the 8 failed cases had a higher mean serum vitamin D (42.54 ng/mL) than the 101 successful patients (31.92 ng/mL).

This null finding tempers a one-directional reading of the vitamin D–osseointegration literature; small failure-event count (n=8) limits statistical power and the directional reversal may reflect chance.

## 세줄요약

전향적 코호트(임플란트 174개/환자 109명): 임플란트 식립 당일 혈청 25-하이드록시비타민 D(serum 25-hydroxyvitamin D, 25(OH)D) 측정 후 보철 수복(약 3–6개월)까지 추적해 조기 임플란트 실패(Early Dental Implant Failure, EDIF)와의 상관관계를 분석.

낮은 혈청 비타민 D와 조기 실패 사이에 유의한 상관관계 없음(음성 결과); 오히려 실패 8건의 평균 비타민 D(42.54 ng/mL)가 성공군(31.92 ng/mL)보다 높아 역방향성 관찰.

실패 사례 수가 8건으로 검정력이 제한적이며 역방향성은 우연일 수 있으나, 비타민 D–골유착 문헌의 일방향적 해석에 균형을 제공하는 반례 연구이다.

## Summary

This prospective cohort assessed whether serum vitamin D levels measured on the day of implant placement relate to early dental implant failure. Across 174 implants in 109 patients followed to restoration (~3–6 months), 8 patients experienced an early failure (defined as ≥50% bone loss or implant mobility). Contrary to the prevailing hypothesis, the failed cases had a *higher* mean serum vitamin D (42.54 ng/mL) than the successful patients (31.92 ng/mL), and the difference was not statistically significant. The authors concluded there was no correlation between low serum vitamin D and early implant failure.

This is a negative-result paper and functions as a counterweight to the more positive vitamin D → osseointegration literature. The small number of failure events (n = 8) limits statistical power, and the directional finding may reflect chance rather than a protective absence of effect. (abstract-only — full text not retrieved from PMC.)

## Key Contributions

- A null / negative finding in a subdomain otherwise weighted toward a positive vitamin D association.
- Counterintuitive observation: early-failure cases had higher mean serum vitamin D (42.54 vs 31.92 ng/mL) than successes.
- Day-of-placement serum measurement tied to a defined failure endpoint, followed to restoration.

## Methodology

- Design: prospective cohort.
- Sample: 174 implants in 109 patients.
- Exposure: serum vitamin D measured on the day of implant placement.
- Follow-up: to restoration, ~3–6 months.
- Outcome: early implant failure = ≥50% bone loss or implant mobility.
- Analysis: comparison of mean serum vitamin D between failed and successful patients; correlation with low serum vitamin D assessed (difference not statistically significant).
- abstract-only — full text not retrieved; covariate adjustment and statistical detail unavailable.

## Results

- 8 patients had an early implant failure.
- Mean serum vitamin D — failed cases: 42.54 ng/mL.
- Mean serum vitamin D — successful patients: 31.92 ng/mL.
- Difference: not statistically significant.
- Conclusion: no correlation between low serum vitamin D and early implant failure.

## Related Papers

- [[implants/vitamin-d/mohsen-2024-vitamin-d-deficiency-osseointegration-prospective]] — **contradicts**: Mohsen's prospective study reports a positive vitamin D → osseointegration association; this cohort finds the opposite directionality (higher vitamin D in failures, no significant correlation).
- [[implants/vitamin-d/miron-2025-vitamin-d-deficiency-early-implant-failure]] — sibling subdomain page on vitamin D deficiency and early failure; Francis tempers a one-directional reading of the deficiency-failure link.
- [[implants/moy-2005-dental-implant-failure-rates-risk]] — foundational implant failure rates / risk-factor anchor; situates vitamin D among multiple competing failure risk factors.
