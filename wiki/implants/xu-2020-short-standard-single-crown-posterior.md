---
title: "Short versus standard implants for single-crown restorations in the posterior region: A systematic review and meta-analysis"
authors: "Xu X, Hu B, Xu Y, Liu Q, Ding H, Xu L"
year: 2020
date: 2020-11-01
doi: "n/a (J Prosthet Dent 2020;124(5):530-538; PII S0022-3913(19)30670-5)"
source: xu-2020-short-standard-single-crown-posterior.md
category: [implants]
confidence: sr+ma
pdf_path: /Users/oracleneo/llm-wiki/papers/xu-2020-short-standard-single-crown-posterior.pdf
pdf_filename: xu-2020-short-standard-single-crown-posterior.pdf
source_collection: external
tags: [short-implant, single-crown, posterior, marginal-bone-loss, survival]
relations:
  - type: refines
    target: xu-2020-short-vs-longer-implants-posterior-5year-sr-ma
---

## Three-line Summary

SR+MA (PRISMA/PROSPERO, 5 RCTs, short ≤6 mm vs standard >6 mm single crowns in the posterior region, searched to August 2019): short implants matched standard implants for short-term survival (RR 0.99, 95% CI 0.97–1.02, p=0.72) but showed significantly lower long-term survival (RR 0.94, 95% CI 0.90–0.99, p=0.01).

Marginal bone loss did not differ between short and standard implants despite the higher crown-to-implant ratio of short implants (MD 0.00, 95% CI −0.10 to 0.11, p=0.94).

Short single crowns are acceptable posteriorly but carry a time-dependent survival penalty — the non-splinted scenario removes the architectural support that multi-unit bridges provide to short implants.

## 세줄요약

SR+MA(PRISMA/PROSPERO, 5 RCT, short ≤6 mm vs standard >6 mm, 구치부 단일 크라운, 검색 2019년 8월): 단기 생존율은 차이 없었으나(RR 0.99, p=0.72) 장기 생존율은 short가 유의하게 낮았다(RR 0.94, 95% CI 0.90–0.99, p=0.01).

높은 크라운-임플란트 비율(Crown-to-Implant ratio)에도 불구하고 변연골소실(MBL)은 두 군 간 차이 없었다(MD 0.00, p=0.94).

구치부 단일 크라운으로서 short implant는 허용되나 시간이 지남에 따라 생존 차이가 나타나며, 스플린트 없는 단독 설계는 다수 연결 보철이 제공하는 구조적 지지를 결여한다.

## Summary

This SR+MA restricted to RCTs isolated the single-crown (nonsplinted) posterior scenario, where short implants cannot borrow stability from adjacent splinted units. Five trials were pooled. Short implants (≤6 mm) matched standard implants for survival in the short term but showed significantly poorer survival on long-term follow-up, while marginal bone loss did not differ despite the higher crown-to-implant ratio of short implants. The clinical message is conditional: short single crowns are acceptable but carry a time-dependent survival penalty that splinting may mitigate — a refinement of the broader "short ≈ standard" narrative.

## Key Contributions

- Isolates the nonsplinted single-crown posterior scenario from multi-unit/splinted designs.
- Separates short- vs long-term survival, surfacing a time-dependent gap.
- Confirms higher C/I ratio does not worsen MBL.

## Methodology

PRISMA SR+MA (PROSPERO CRD42018112978). PubMed, EMBASE, Cochrane (before August 2019); RCTs only; Cochrane RoB. 1954 references screened → 5 trials in quantitative synthesis. Short ≤6 mm vs standard >6 mm supporting single crowns in posterior alveolar bone.

## Results

- Short-term survival: no difference (RR 0.99; 95% CI 0.97–1.02; P=.72).
- Long-term survival: short poorer (RR 0.94; 95% CI 0.90–0.99; P=.01).
- MBL: no difference (MD 0.00; 95% CI −0.10 to 0.11; P=.94).

## Related Papers
- [[implants/xu-2020-short-vs-longer-implants-posterior-5year-sr-ma]] — refines (same first author, ≥5y observation, longer implants; this page adds single-crown/RCT restriction)
- [[overviews/implant-length-selection-why-not-always-short]] — supports caution argument
- [[overviews/short-implant-vs-sinus-augmentation-decision]] — decision overview
