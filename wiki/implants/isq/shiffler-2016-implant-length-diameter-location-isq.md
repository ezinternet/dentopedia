---
title: "Effect of Length, Diameter, Intraoral Location on Implant Stability"
authors: Shiffler K, Lee D, Rowan M, Aghaloo T, Pi-Anfruns J, Moy PK
year: 2016
date: 2016-12-01
doi: 10.1016/j.oooo.2016.06.016
source: shiffler-2016-implant-length-diameter-location-isq.md
category: [implants/isq]
confidence: retrospective
pdf_path: /Users/oracleneo/llm-wiki/papers/shiffler-2016-implant-length-diameter-location-isq.pdf
pdf_filename: shiffler-2016-implant-length-diameter-location-isq.pdf
source_collection: external
tags: [ISQ, implant-location, implant-length, implant-diameter, maxilla, mandible, primary-stability, retrospective]
---

## 한줄요약
후향적 연구(n=200 NobelReplace Tapered, UCLA): 구강 내 위치(하악>상악)가 ISQ의 가장 유의한 결정 인자; 임플란트 길이는 임상적으로 유의미한 경향(p=0.08/0.09); 직경·성별·나이는 ISQ에 영향 없음; 생존율 98% (OOOO 2016).

## Summary
Retrospective observational study (n=200 NobelReplace Tapered Groovy implants, 143 patients, UCLA) evaluating the effect of implant length, diameter, and intraoral location on ISQ at placement (T1) and follow-up (T2). Intraoral location was the most significant factor: mandibular implants consistently showed higher ISQ than maxillary at both timepoints. Implant length trended toward clinical relevance (p=0.08 at T1, p=0.09 at T2) — not statistically significant but clinically noteworthy. Diameter was not significantly associated with ISQ. Gender and age did not affect ISQ or survival. Overall survival: 98%.

## Key Contributions
- Confirms mandible > maxilla ISQ advantage (statistically significant at both T1 and T2)
- Length may have clinically relevant effect on stability — supports using longer implants where bone permits
- Diameter: does not drive ISQ — challenging assumptions about wider = more stable
- Gender and age: negligible effect — supports implant placement across patient demographics

## Methodology
Retrospective. 200 NobelReplace Tapered Groovy implants, 143 patients. Age 32–93 (mean 63.7). UCLA OMS center, May 2007–October 2011. Dimensions: 4.3×10, 4.3×13, 5×10 mm. 44% had extractions before placement; median extraction-to-placement 4.3±1.76 months. 49% sinus graft in maxillary posterior. ISQ at T1 and T2. Analysis: ANOVA + linear regression.

## Results
| Variable | Effect on ISQ | Significance |
|---|---|---|
| Intraoral location | Mandible > maxilla | Significant (T1 and T2) |
| Implant length | Trend: longer → higher | p=0.08 (T1), p=0.09 (T2) |
| Implant diameter | No effect | NS |
| Gender | No effect | NS |
| Age | No effect | NS |
| Survival rate | 98% | — |

## Related Papers
- [[implants/isq/huang-2020-isq-clinical-significance-literature-review]] — 17-factor ISQ review including location and bone quality
- [[implants/isq/tisci-2026-isq-it-mbl-survival-sr-ma]] — ISQ-IT SR+MA; location heterogeneity contributes to high I²
