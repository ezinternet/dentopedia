---
title: "Short vs long implants with sinus floor elevation in posterior maxilla: a network meta-analysis"
authors: Zhang Y, Tang X, Zhang Y, Cao C, et al.
year: 2024
date: 2024-12-23
doi: 10.1186/s12903-024-05377-1
source: zhang-2024-short-vs-long-implants-sinus.md
category: implants
confidence: sr+ma
pdf_path: /Users/oracleneo/llm-wiki/papers/zhang-2024-short-vs-long-implants-sinus.pdf
pdf_filename: zhang-2024-short-vs-long-implants-sinus.pdf
source_collection: external
tags: [short-implant, sinus-floor-elevation, posterior-maxilla, network-meta-analysis, marginal-bone-loss]
---

## One-line Summary
Network meta-analysis (17 studies, 1,076 patients, 1,751 implants) of posterior-maxilla strategies — short implants (4–8 mm) without sinus elevation vs long implants (≥8 mm) with immediate or delayed sinus floor elevation (SFE): implant survival statistically indistinguishable (failure long vs short OR 1.26, NS) but short implants showed significantly less marginal bone loss (MBL), with complication trends also favoring short.

## 한줄요약
네트워크 메타분석 (Network Meta-Analysis, NMA), 17건/환자 1,076명/임플란트 1,751개: 후방 상악동 거상술 (Sinus Floor Elevation, SFE)에서 단축형 임플란트 (Short Implant, 4–8 mm)는 장축형 (≥8 mm) 대비 생존율은 차이 없지만 변연골소실 (Marginal Bone Loss, MBL)이 통계적으로 유의하게 적음.

## Summary
This 2024 BMC Oral Health network meta-analysis compares three posterior-maxilla rehabilitation strategies in one model: short implants (4–8 mm) without sinus elevation, long implants (≥8 mm) with delayed sinus floor elevation (SFE), and long implants with immediate SFE. Seventeen studies (1,076 patients; 1,751 implants) published January 2014 through November 2024 were synthesized following PRISMA-NMA. The headline result is clinically pragmatic: implant survival is statistically indistinguishable between approaches, but short implants are associated with significantly less marginal bone loss, while complication rates trend (non-significantly) in favor of short implants.

## Key Contributions
- A single network model joining the short-vs-long debate with the immediate-vs-delayed debate.
- Provides a current evidence map (through November 2024) prospectively registered with PROSPERO (CRD42023495027).
- Quantifies the MBL advantage of short implants — the strongest signal in the dataset.

## Methodology
PRISMA-NMA-compliant search of Cochrane Library, PubMed, Embase, Scopus, and Web of Science. RCTs and prospective comparative studies were included. Pairwise and network meta-analyses were performed for: (1) implant failure (primary), (2) marginal bone loss, (3) biological complications, (4) mechanical complications. Risk of bias was assessed; heterogeneity was reported by I² and consistency tested through node-splitting.

## Results
- Implant failure long vs short: OR 1.26 (95% CI 0.53–3.00), NS — trend favors long but underpowered.
- Biological complications short vs long: OR 0.47 (95% CI 0.19–1.18), NS.
- Mechanical complications short vs long: OR 0.94 (95% CI 0.45–1.94), NS.
- Marginal bone loss: short implants showed significantly less MBL than long implants whether the long implants were placed immediately or after delayed SFE (MD ≈ −0.17 mm vs immediate long; (precise CI not in first 15 pages; see PDF) vs delayed long).
- No significant differences in network ranking between immediate and delayed long-implant SFE strategies.

## Clinical Bottom Line
For the posterior maxilla with reduced residual ridge height, short implants (4–8 mm) are a defensible alternative to long implants with sinus elevation. Survival is comparable; MBL favors short implants; complication trends also favor short implants but did not reach significance.

## Related Papers
- ~~(미ingest) esposito-2014-short-vs-long-implants~~ — earlier Cochrane comparison
- ~~(미ingest) pjetursson-2008-sinus-elevation-survival~~ — sinus elevation survival baseline
- [[sinus-lift/transcrestal/farina-2023-minimal-invasiveness-transcrestal-sinus-elevation]] — alternative to lateral SFE
