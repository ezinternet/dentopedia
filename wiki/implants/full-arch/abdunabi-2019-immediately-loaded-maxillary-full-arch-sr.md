---
title: "Impact of immediately loaded implant-supported maxillary full-arch dental prostheses: a systematic review"
authors: "Abdunabi A, Morris M, Abi Nader S, de Souza RF"
year: 2019
date: 2019-03-14
doi: "10.1590/1678-7757-2018-0600"
source: abdunabi-2019-immediately-loaded-maxillary-full-arch-sr.md
category: [implants]
evidence_level: sr
pdf_path: /Users/oracleneo/llm-wiki/papers/abdunabi-2019-immediately-loaded-maxillary-full-arch-sr.pdf
pdf_filename: abdunabi-2019-immediately-loaded-maxillary-full-arch-sr.pdf
source_collection: external
tags: [full-arch, immediate-loading, edentulous-maxilla, fixed-prosthesis, patient-satisfaction]
---

## Three-line Summary

Systematic review of 4 controlled clinical trials (meta-analysis precluded by heterogeneity; 8 databases to Jan 2019) comparing immediate vs early/conventional loading of implant-supported maxillary full-arch fixed prostheses in edentulous patients.

Immediate loading showed only a transient patient-satisfaction advantage (lost by 12 months); implant success and prosthetic complications did not differ significantly across 3 of 4 trials.

Evidence is weak and insufficient for firm protocol recommendations — robust RCTs in the edentulous maxilla are still needed before immediate full-arch loading can be standardized.

## 세줄요약

체계적 고찰 (4개 CCT; 이질성으로 메타분석 불가; 8개 데이터베이스 2019.1까지) — 무치악 상악 전악 임플란트 지지 고정성 보철에서 즉시 vs 조기/통상 부하 비교.

즉시 부하의 환자 만족도 이점은 일시적(12개월에 소실); 임플란트 성공률·보철 합병증은 3편에서 유의차 없음.

근거 미약 — 상악 무치악 즉시 전악 부하 표준화를 위한 견고한 RCT 추가 필요.

## Summary

This systematic review evaluated immediate loading of fully implant-supported maxillary full-arch fixed prostheses against early and conventional loading. Only 4 controlled clinical trials qualified, and heterogeneity precluded meta-analysis. The qualitative synthesis found weak, inconsistent evidence: a short-lived satisfaction advantage for immediate loading and no significant differences in implant success or prosthetic complications. The authors stress that immediate loading shows promise in the edentulous maxilla but that robust RCTs are required before firm protocol recommendations.

## Key Contributions

- Dedicated evidence summary for the higher-risk maxillary full-arch immediate-loading indication.
- Documents the scarcity and heterogeneity of controlled trials.

## Methodology

- SR of CCTs; 8 databases searched to Jan 2019; duplicate screening/extraction; Cochrane RoB tool; qualitative synthesis (no pooling).

## Results

- 4 CCTs from 1,052 screened.
- Satisfaction: transient advantage for immediate loading (lost by 12 months).
- Implant success / prosthetic complications: no significant differences in 3 trials.

## Related Papers

- [[implants/liu-2021-immediate-delayed-loading-overdenture-mbl-sr-ma]] — companion: loading protocols in the edentulous mandible (overdentures)
- [[implants/benic-2014-loading-protocols-single-implant-crowns-sr-ma]] — loading protocols for single-implant crowns
