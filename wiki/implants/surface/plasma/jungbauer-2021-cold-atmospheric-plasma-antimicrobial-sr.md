---
title: "The Antimicrobial Effect of Cold Atmospheric Plasma against Dental Pathogens—A Systematic Review of In-Vitro Studies"
authors: Jungbauer G, Moser D, Müller S, Pfister W, Sculean A, Eick S
year: 2021
date: 2021-02-19
doi: 10.3390/antibiotics10020211
source: jungbauer-2021-cold-atmospheric-plasma-antimicrobial-sr.md
category: [implants/surface/plasma]
evidence_level: sr
pdf_path: /Users/oracleneo/llm-wiki/papers/jungbauer-2021-cold-atmospheric-plasma-antimicrobial-sr.pdf
pdf_filename: jungbauer-2021-cold-atmospheric-plasma-antimicrobial-sr.pdf
source_collection: external
tags: [plasma, antimicrobial, biofilm, oral-pathogens, time-dependent, in-vitro]
---

## Three-line Summary

Systematic review of in-vitro studies (PubMed, Embase, Scopus; 55 studies included, methodological quality 31–92%) examining CAP's antimicrobial effect against dental pathogens with standardized parameter extraction.

CAP consistently reduced dental pathogen viability across studies; reduction factors clustered by pathogen type, working gas, and treatment time, with a time-dependent increase in antimicrobial effect being the most consistent finding.

High heterogeneity across device types, gases, and exposures precludes any specific protocol recommendation; multi-species biofilm models representative of clinical conditions are lacking.

## 세줄요약

줄1: in vitro 체계적 문헌고찰(PubMed·Embase·Scopus; 55편, 방법론적 품질 31–92%): 구강 병원균 대상 CAP 항균 효과, 표준화 파라미터 추출.

줄2: 전 연구에서 CAP가 구강 병원균 생존율 감소; 감소폭은 병원균·작동가스·처리시간에 따라 군집, 시간의존적 효과 증가가 가장 일관된 소견.

줄3: 장치·가스·노출 이질성이 높아 특정 프로토콜 권고 불가; 임상 환경을 반영하는 다균종 바이오필름 모델 부재.

## Summary

The broad antimicrobial backbone of the cluster. CAP reliably kills oral pathogens in vitro, and the effect grows with treatment time — but the magnitude depends heavily on pathogen, gas, and exposure, so no standard dose can yet be recommended. Most evidence is short of mature multi-species biofilm, which is what matters clinically.

## Key Contributions

- 55-study synthesis + a standardized in-vitro assessment tool.
- Pinpoints the three determinants of effect (pathogen, gas, time).

## Methodology

- PubMed, Embase, Scopus. Device parameters/setups/cultivation extracted; quality-scored.

## Results

- 55 studies (quality 31–92%).
- Reduction factors varied strongly; clustered by pathogen/gas/time.
- Consistent time-dependent increase of antimicrobial effect.

**Boundary (living-document):** in-vitro only; high heterogeneity → no protocol/device recommendation; multi-species biofilm models needed before RCTs.

## Related Papers

- [[implants/surface/plasma/alqutaibi-2023-cap-bacterial-reduction-implants-sr]] — implant-surface decontamination SR.
- [[implants/surface/plasma/lee-2022-plasma-contaminated-zirconia-gingivalis-osteoblast]] — primary: NTP vs P. gingivalis on zirconia.
- [[implants/surface/plasma/zeng-2020-nanostructured-titanium-non-thermal-plasma]] — decontamination + osseointegration.
