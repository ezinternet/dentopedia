---
title: "IOS complete-arch scanning accuracy: SR + network meta-analysis"
authors: Viktória Vitai, Anna Németh, Eleonóra Sólyom, et al.
year: 2023
date: 2023-07-27
doi: 10.1016/j.jdent.2023.104636
source: vitai-2023-intraoral-scanner-complete-arch-sr-network-ma.md
category: [digital-workflow]
confidence: sr+ma
pdf_path: /Users/oracleneo/llm-wiki/papers/vitai-2023-intraoral-scanner-complete-arch-sr-network-ma.pdf
pdf_filename: vitai-2023-intraoral-scanner-complete-arch-sr-network-ma.pdf
source_collection: external
tags: []
---

## One-line Summary
SR + network meta-analysis of IOS complete-arch scanning accuracy — device-specific trueness and precision rankings provided; error increases with arch extension and when scanning beyond natural teeth.

## 한줄요약
IOS 전악 스캔 정확도 SR + 네트워크 MA — 기기별 trueness·precision 순위 제시, 스캔 전략·악궁 연장 시 오차 증가.

## Summary
SR and network meta-analysis evaluating and ranking complete-arch scanning accuracy (trueness, precision) across multiple IOS devices.

## Key Contributions
- Network MA ranks IOS devices by trueness and precision for complete-arch scans
- Error propagation increases with arch length and extended edentulous spans
- Scan strategy (scan path) significantly affects accuracy

## Methodology
PubMed/Embase systematic search; in vitro studies; network meta-analysis.

## Results
Significant inter-device accuracy differences for complete-arch scans. All devices show error accumulation with arch extension. No single device universally superior.

## Related Papers
- [[digital-workflow/buhl-2025-intraoral-scanner-full-arch-accuracy-invitro]] — related
- [[digital-workflow/alkadi-2023-intraoral-scanner-accuracy-factors]] — related
