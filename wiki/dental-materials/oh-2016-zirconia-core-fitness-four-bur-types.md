---
title: "Evaluation of Marginal and Internal Fitness of Zirconia Cores Fabricated on Abutments Prepared with Four Different Tip-designed Burs"
authors: Sang-Hoon Oh, Hong-Bing Shi, Hyeong-Rok Noh, Jeong-Hui Ji, Min-Ho Lee, Tae-Sung Bae
year: 2016
date: 2016-01-01
doi: 10.14815/kjdm.2016.43.3.281
journal: 대한치과재료학회지 2016;43(3):281-288
source: oh-2016-zirconia-core-fitness-four-bur-types.md
confidence: in-vitro
category: [dental-materials]
pdf_path: /Users/oracleneo/llm-wiki/papers/oh-2016-zirconia-core-fitness-four-bur-types.pdf
pdf_filename: oh-2016-zirconia-core-fitness-four-bur-types.pdf
source_collection: external
tags: [zirconia, cad-cam, marginal-fitness, internal-fitness, bur, preparation, crown, in-vitro]
---

## 한줄요약
In vitro: torpedo·deep chamfer bur → 우수한 변연/내부 적합도; shoulder bur → 90% 완전 안착 실패; 임상 부적합

## Summary
버 tip design 4종(torpedo, deep chamfer, sloped shoulder, shoulder)으로 형성된 지대치에서 CAD/CAM 지르코니아 코어 적합도 비교. **Torpedo·Deep chamfer → 변연·내면 적합성 유의하게 우수**. Shoulder 버 → full seating 실패 90%, 임상 부적합.

## 버별 적합도 비교

| 버 종류 | Full seating 실패 | 변연 적합성 | 임상 권장 |
|---|---|---|---|
| **Torpedo** | 0/10 | ★★★ | ✅ 권장 |
| **Deep chamfer** | 0/10 | ★★★ | ✅ 권장 |
| Sloped shoulder | 6/10 | ★★ | ⚠️ 주의 |
| **Shoulder** | **9/10** | ★ | ❌ 부적합 |

- 임상 변연 허용치: 100-150μm → Torpedo·Deep chamfer 충족
- ZPC 합착 후: 모든 군 간극 증가; shoulder 군 허용치 초과

## Clinical Relevance
- CAD/CAM 지르코니아 크라운 지대치 형성 시 **Torpedo 또는 Deep chamfer 버** 사용
- Shoulder 버: 내면 간섭 → full seating 불가 → 기공사 내면 수정 필수 → 오차 누적
- 축벽 경사 12°, 변연 폭 0.7mm 설계가 적합도 최적화의 전제 조건

## Related Papers
- [[digital-workflow/singh-2025-intraoral-scanners-accuracy-umbrella-review]] — 디지털 인상/스캔 정확도 (IOS)
- [[dental-materials/huettig-2021-flowability-tear-strength-dental-impression]] — 기존 탄성 인상재 물성
