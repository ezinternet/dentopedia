---
title: "Survival Rates of Amalgam and Composite Resin Restorations from Big Data Real-Life Databases in the Era of Restricted Dental Mercury Use"
authors: Guy Tobias, Tali Chackartchi, Jonathan Mann, Doron Haim, Mordechai Findler
year: 2024
date: 2024-06-10
doi: 10.3390/bioengineering11060579
source: tobias-2024-amalgam-composite-survival-big-data.md
category: [dental-materials]
confidence: retrospective
pdf_path: /Users/oracleneo/llm-wiki/papers/tobias-2024-amalgam-composite-survival-big-data.pdf
pdf_filename: tobias-2024-amalgam-composite-survival-big-data.pdf
source_collection: external
tags: [amalgam, composite-resin, survival-rate, big-data, real-world, maccabi, multi-surface, single-surface, failure-rate, cox-regression]
relations:
  - type: contradicts
    target: al-sulimman-2025-composite-amalgam-failure-risk-sr-ma
---

## One-line Summary
Big-data retrospective cohort (Israel Maccabi, 58 clinics, 650,000+ patients, 2014–2021): amalgam annual failure rate 3.5% vs composite 4.5%; HR 1.29; significant difference only in multi-surface composite restorations.

## 한줄요약
Big-data 후향적 코호트 (이스라엘 Maccabi, 58개소, 650,000명 이상, 2014–2021): 아말감(Amalgam) 연간 실패율 3.5% vs 복합레진(Composite Resin) 4.5%; HR 1.29; 다면(Multi-surface) 레진에서만 유의한 차이.

## Summary
Largest real-world dataset comparing amalgam and composite resin restoration survival. Retrospective cohort using electronic health records from 58 dental clinics (440 dental units) in the Maccabi Healthcare Fund (Israel), 2014–2021. Primary outcome: restoration failure defined as replacement, root canal treatment, or extraction within follow-up. Kaplan-Meier survival analysis and Cox proportional hazards regression with adjustment for age, sex, number of surfaces, tooth type, and operator.

Annual failure rate: amalgam 3.5% vs composite 4.5% (HR 1.29, p<0.001). Single-surface restorations showed no significant difference between materials. Multi-surface composite restorations had significantly higher failure rates than amalgam equivalents.

## Key Contributions
- Largest real-world dataset on amalgam vs composite survival (>650,000 patients; ~260,905 restored patients over 2014–2021)
- Composite HR 1.29 overall (p<0.001), driven by multi-surface restorations
- Single-surface: no significant material difference — key clinical take-away
- Provides evidence in the context of Minamata Convention mercury restrictions

## Methodology
- **Design**: Retrospective cohort; electronic health records
- **n**: >650,000 patients; ~260,905 with restorations placed 2014–2021
- **Setting**: 58 clinics, 440 dental units, Maccabi Healthcare Fund, Israel
- **Failure definition**: Restoration replacement, RCT, or extraction
- **Analysis**: Kaplan-Meier + Cox regression; confounders: age, sex, surface number, tooth type, operator

## Results

| Material | Annual Failure Rate | HR vs Amalgam | p |
|---|---|---|---|
| Amalgam | 3.5% | reference | — |
| Composite | 4.5% | 1.29 | <0.001 |
| Single-surface | NS | — | — |
| Multi-surface composite | Significantly higher | — | <0.001 |

## Related Papers
- [[dental-materials/al-sulimman-2025-composite-amalgam-failure-risk-sr-ma]] — SR+MA; no significant RR difference (RR 0.96); contradicts real-world HR gap
- [[dental-materials/bhagwat-2025-amalgam-composite-posterior-longevity-sr]] — SR; amalgam 16yr vs composite 11yr median survival
- [[dental-materials/zirconia/aswal-2023-cadcam-zirconia-lithium-disilicate-crowns-sr-ma]] — CAD/CAM vs conventional SR+MA
