---
title: "Study on the polymerization shrinkage and mechanical-physical properties of dental resin composite modified by polyurethane dimechacrylate and glass flake fillers"
authors: "Feng D, He X, Dong S, Yang Z, Zhang L, Zhu S"
year: 2024
date: 2024-10-24
doi: "10.1016/j.jdent.2024.105426"
source: feng-2024-polyurethane-dimethacrylate-low-shrinkage-composite.md
category: [resin]
confidence: in-vitro
source_collection: pubmed-text
full_text: false
pmid: "39454988"
text_path: /Users/oracleneo/llm-wiki/papers/feng-2024-polyurethane-dimethacrylate-low-shrinkage-composite.txt
text_filename: feng-2024-polyurethane-dimethacrylate-low-shrinkage-composite.txt
tags: [polymerization-shrinkage, low-shrinkage-composite, polyurethane-dimethacrylate, matrix-modification, material-development, aging]
relations:
  - type: extends
    target: meereis-2018-polymerization-shrinkage-stress-resin-sr-ma
---

## Three-line Summary
In-vitro material-development study (J Dent 2024) synthesizing experimental Bis-GMA/TEGDMA composites modified with 20 wt% polyurethane dimethacrylate oligomer (PU-MA) and 65–75 wt% glass-flake/borosilicate fillers, measuring polymerization shrinkage kinetics and mechanical properties before and after aging.

The best formulation (PU-PG-75%) achieved low volumetric shrinkage (1.35%) and retained flexural strength and Vickers hardness better than commercial controls after aging (60-day water immersion + 5000 thermocycles); higher filler loading increased water sorption/solubility and reduced depth of cure.

PU-MA matrix modification is a viable resin-matrix strategy for low-shrinkage composites, but the filler-loading trade-off (↑ filler → ↓ shrinkage, ↑ water sorption, ↓ depth of cure) must be optimized.

## 세줄요약
Bis-GMA/TEGDMA 기반 복합레진에 20 wt% 폴리우레탄 디메타크릴레이트(PU-MA) 올리고머 + 65–75 wt% 글래스 플레이크 충전재를 추가한 실험적 재료를 개발·평가한 in-vitro 연구(J Dent 2024).

최적 조성(PU-PG-75%)은 부피수축 1.35%로 낮고 60일 침수+5000회 열순환 노화 후 상용제품 대비 굽힘강도·경도가 상대적으로 안정; 충전재 함량 증가 → 수분흡수·용해도 증가·광투과 깊이 감소.

PU-MA 기질 변형은 실용적인 저수축 전략이나, 충전재 최적화 없이는 내수성·광중합 깊이 트레이드오프가 발생함.

## Summary
This Journal of Dentistry (2024) study from Jilin University synthesized experimental composites by adding a polyurethane dimethacrylate oligomer (PU-MA, 20 wt% of the resin) to a Bis-GMA/TEGDMA base, combined with 65-75 wt% glass-flake/Si-Al-borosilicate fillers. The aim was to reduce polymerization shrinkage while preserving mechanical performance through resin-matrix modification — the strategy that meta-analytic evidence identifies as the most effective for shrinkage-stress control. The best formulation (PU-PG-75%) reached a low volumetric shrinkage of 1.35% and, crucially, retained its flexural and hardness properties better than commercial controls after aging (60-day water immersion plus 5000 thermocycles). The trade-off was that higher filler loading increased water sorption/solubility and reduced depth of cure.

## Key Contributions
- Demonstrates PU-MA (20 wt%) as a matrix-modification route to low volumetric shrinkage (1.35%)
- Maps the filler trade-off: ↑ filler → ↓ shrinkage but ↑ water sorption/solubility and ↓ depth of cure
- Shows PU-PG-75% retains mechanical properties after aging better than commercial composites
- Real-time shrinkage kinetics via laser triangulation

## Methodology
Hand-fabricated composites: Bis-GMA/TEGDMA + PU-MA + 65-75 wt% mixed glass-flake/borosilicate fillers. Polymerization shrinkage kinetics by laser-triangulation; flexural strength/modulus by three-point bending and micro Vickers hardness, measured before and after aging (60-day water immersion + 5000 thermocycles). One-way ANOVA/LSD; independent T-test (pre/post aging); two-way ANOVA (composition × aging); α=0.05.

## Results
- PU-PG-75%: volume shrinkage 1.35% (lowest)
- ↑ filler content → significantly ↑ water sorption & solubility and ↓ depth of cure (P<0.05)
- After aging: commercial controls' flexural strength, elastic modulus, Vickers hardness dropped markedly; PU-PG-75wt% relatively stable
- 20 wt% PU-MA effectively decreases shrinkage; aging significantly reduces flexural properties of experimental materials

## Related Papers
- [[resin/meereis-2018-polymerization-shrinkage-stress-resin-sr-ma]] — extends (concrete instance of the resin-matrix-modification strategy it identifies as most effective)
- [[resin/he-2026-fluorinated-thiol-ene-low-shrinkage-composite]] — parallel low-shrinkage material-development approach (different chemistry)
- [[resin/melo-2023-bioactive-dental-resins-restorative-dentistry]] — experimental-composite formulation/vulnerability context
