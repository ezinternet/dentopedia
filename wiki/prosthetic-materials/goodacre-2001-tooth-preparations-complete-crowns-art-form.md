---
title: "Tooth preparations for complete crowns: An art form based on scientific principles"
authors: Charles J. Goodacre, Wayne V. Campagni, Steven A. Aquilino
year: 2001
date: 2001-04-01
doi: 10.1067/mpr.2001.114685
source: goodacre-2001-tooth-preparations-complete-crowns-art-form.md
category: [prosthetic-materials]
confidence: narrative-review
pdf_path: /Users/oracleneo/llm-wiki/papers/goodacre-2001-tooth-preparations-complete-crowns-art-form.pdf
pdf_filename: goodacre-2001-tooth-preparations-complete-crowns-art-form.pdf
source_collection: external
tags: [tooth-preparation, complete-crown, TOC, taper, resistance-form, retention-form, finish-line, occlusal-reduction, jpd]
relations:
  - type: extends
    target: tiu-2015-clinical-tooth-preparations-measuring-sr
  - type: reinforces
    target: roudsari-2011-auxiliary-features-resistance-short-molars-invitro
  - type: reinforces
    target: yang-2015-auxiliary-resistance-marginal-fitness-short-molar
---

## Three-line Summary

Narrative review synthesizing ~250 years of complete-crown preparation literature (J Prosthet Dent 2001;85:363-376), condensing 50 years of scientific data into nine principles.

Key numeric guidelines: 10-20° total occlusal convergence (TOC), ≥4 mm occlusocervical height for molars (≥3 mm for other teeth), and occlusocervical-to-faciolingual ratio ≥0.4; no finish-line design was proven superior for marginal fit.

These benchmarks displaced the historically taught but clinically unachievable low single-digit taper and remain the standard teaching reference for complete-crown preparation.

## 세줄요약

narrative-review (J Prosthet Dent 2001;85:363-376): ~250년 완전피개 치관 준비 문헌을 종합해 9가지 과학적 원칙을 제시 — 특히 최근 50년 MEDLINE+수동 검색 기반.

핵심 수치: TOC 10-20°, 대구치 교합-치경 높이 ≥4 mm(기타치 ≥3 mm), 교합-치경:협-설 비율 ≥0.4; 어떤 finish line도 변연 적합도 우위 증명 안 됨.

역사적으로 가르쳤던 6° taper는 임상적으로 달성 불가 — 이 리뷰의 10-20° TOC가 현대 교육 기준이 됨; 단기 수복치경 부족 시 보조 groove/box 추가 권고.

## Summary

Goodacre, Campagni, and Aquilino review ~250 years of complete-coverage tooth preparation, emphasizing the prior 50 years of scientific data, and condense the literature into nine principles spanning mechanical, biologic, and esthetic success. The central numeric guidelines — 10-20° TOC, minimal axial height (4 mm molars / 3 mm others), and an occlusocervical-to-faciolingual ratio ≥0.4 — became the standard contemporary teaching benchmarks, displacing the historically taught (but clinically unachievable) low single-digit taper.

## Key Contributions

- **Nine scientific principles** for complete-crown preparation, organized by mechanical/biologic/esthetic success.
- **10-20° TOC** established as the clinically realistic convergence target.
- **Resistance form as a system**: prep height, base width, and taper together — captured by the occlusocervical-to-faciolingual ratio ≥0.4 — not taper alone.
- **Auxiliary resistance features** (axial grooves/boxes, preferentially proximal) prescribed when dimensions are deficient (short / over-tapered teeth).
- **Finish line**: selection by crown type/esthetics/operator; enhanced marginal fit from a specific finish-line type was **not validated**; supragingival location preferred.

## Methodology

- Narrative/critical literature review (not systematic; no meta-analysis).
- MEDLINE search plus extensive manual search; English-language articles; emphasis on scientific data of the last 50 years within a 250-year scope.

## Results

| Principle | Guideline |
|---|---|
| Total occlusal convergence (TOC) | 10-20° |
| Minimal occlusocervical (axial) height | 4 mm molars / 3 mm other teeth |
| Occlusocervical-to-faciolingual ratio | ≥0.4 |
| Line angles | Preserve facioproximal/linguoproximal; round all |
| Auxiliary resistance | Grooves/boxes (proximal preferred) when geometry deficient |
| Finish line type | Choose by crown/esthetics/ease; no proven marginal-fit advantage |
| Finish line location | Supragingival more acceptable |
| Surface | Reasonable smoothness |

## Clinical Implications [근거강함]

- The "6° taper" remains a teaching ideal; **10-20° TOC** is the achievable, defensible clinical band — consistent with Tiu 2015's documented historic drift.
- When a molar is short (<4 mm axial) or the O-C:F-L ratio falls below 0.4, do not rely on reducing taper (impractical intraorally); instead add **proximal grooves/boxes** to restore resistance.
- Finish-line choice should be driven by the restorative material, esthetics, and operator skill rather than an expectation of superior marginal fit from one geometry.

## Related Papers

- [[overviews/crown-preparation-geometry-convergence-resistance-retention]] — central synthesis hub; this review is the canonical guideline source it builds on
- [[prosthetic-materials/tiu-2015-clinical-tooth-preparations-measuring-sr]] — SR quantifying the historic TOC drift (6° → 10-22°) this review narrates
- [[prosthetic-materials/roudsari-2011-auxiliary-features-resistance-short-molars-invitro]] — in-vitro confirmation of auxiliary grooves/boxes for short molars (principle #5)
- [[prosthetic-materials/yang-2015-auxiliary-resistance-marginal-fitness-short-molar]] — auxiliary resistance features and marginal fit in short molars
- [[prosthetic-materials/contrepois-2013-marginal-adaptation-ceramic-crowns-sr]] — ceramic-era SR addressing the marginal-fit question left open here
