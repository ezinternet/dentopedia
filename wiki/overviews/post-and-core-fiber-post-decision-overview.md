...
status: done
source_wiki:
  - wiki/post-and-core/fathi-2022-prosthetic-restoration-endodontically-treated-teeth-umbrella.md
  ...
  - wiki/post-and-core/mously-2024-anterior-endocrowns-alternative-core-crown.md
  - wiki/post-and-core/lenz-2023-endocrown-core-crown-biomechanical-sr.md
  - wiki/post-and-core/yuen-2025-nonmetallic-customized-post-core-outcomes.md
  - wiki/resin/susita-2026-comparative-analysis-stress-distribution-teeth.md
...
tags: [post-and-core, fiber-post, ferrule, endocrown, endodontically-treated-teeth, glass-fiber, failure-mode, parapost, umbrella-review, amstar2]
---

> [!summary] 한국어 핵심요약
>
> - **핵심 메시지**: 근관치료치(endodontically treated tooth, ETT) 수복은 단일 재료 선택이 아니라 **순차적 의사결정**이다 — ①포스트 필요여부 → ②ferrule 보존 → ③포스트 재료, 그리고 결정의 무게중심은 재료보다 **잔존 치질·실패 양상(failure mode)**에 있다.
> - **① 포스트가 필요한가**: 임상 17편·ETT 7,330개 SR+MA에서 포스트 식립이 무(無)포스트 대비 생존을 유의하게 향상(P<0.001) — 단 이익은 **관상부 치질 소실 정도에 조건부** (Alenezi 2024).
> - **포스트는 보조, ferrule이 주역 — 랜드마크**: 5년↑ 전향 임상 8편 SR에서 **7/8편이 포스트의 독립적 효과 없음**을 보고, ferrule·치벽(cavity wall) 보존이 생존의 지배적 인자 — "Ferrule comes first, post is second" (Naumann 2017).
> - **포스트 대안 — 엔도크라운(endocrown)**: 전치부에서 엔도크라운이 포스트-코어-크라운(post-core-crown, PCC)과 파절저항 동등 이상이며 **실패가 복구 가능(repairable)**, ferrule이 지배적 인자, 리튬디실리케이트(lithium disilicate)가 최적 재료 (Mously 2025, 12편 in-vitro/FEA SR). 생역학 SR 31편(구치·소구치·전치)에서도 엔도크라운이 (포스트-)코어-크라운 대비 피로·정적하중 생존 **동등 이상**, 수복재·시멘트 응력 더 낮음, 파절양상 더 유리 (Lenz 2023).
> - **② ferrule이 최강 가변 예후인자**: 원주형(circumferential) ferrule이 파이버포스트 ETT 성공률을 높임(직접분석 RR=1.28, 95% CI 1.06–1.54; 간접 풀링은 비유의) — **재료 선택보다 ferrule 보존이 우선** (Al-Dabbagh 2024, 8편/치아 407개; Naumann 2017과 일관).
> - **③ 포스트 재료 — 글라스파이버 ≈ 금속(통계적으로), 단 심한 손상 치아에선 파이버 우위**: SR+MA 7 RCT에서 글라스파이버 92.8% vs 금속 78.1% 생존, **차이 비유의**(Tsintsadze 2022); 15년 RCT(182개)도 파이버 경향 우위(68.5% vs 53.6%, NS) (Van Landuyt 2025). 반면 잔존 치벽 **≤2벽의 심한 손상 ETT**만 본 RCT-전용 SR+MA 4편에서는 파이버 생존이 **유의하게** 우위(RR 0.57, 95% CI 0.33–0.97, P=.04; GRADE high), 치근파절률은 차이 없음 (Wang 2019).
> - **활력 유무 자체가 예후인자**: 전부피개 보철 SR 26편에서 지대치 **활력(vital)이 생존을 유의하게 향상**(FDP 84.9% vs 비활력 81.3%, P=.049); 비활력치 중 **파이버포스트 + 올세라믹 단관이 5년 95.0%로 최고**, **포스트 없는 비활력 단관이 85.7%로 최저** (Hawthan 2023).
> - **진짜 차이는 실패 양상**: 금속/강성 포스트는 **치근파절(root fracture)로 파국적·수복 불가** 실패, 파이버포스트·엔도크라운은 **복구 가능한** 실패 — 생존율 숫자보다 이 차이가 임상 의사결정의 핵심 (Van Landuyt 2025; Mously 2025; Lenz 2023). 비금속 **맞춤형(customized)** 포스트도 같은 결: CAD/CAM 지르코니아는 주조 포스트와 동등, PEEK·PEKK는 탈락(debonding) 빈발, 개별성형 섬유강화복합재(EverStick·Ribbond)는 수복 가능한 파절 (Yuen 2025).
> - **생역학 앵커**: 파이버포스트 FEA 응력분포 분석이 탄성계수(elastic modulus)를 상아질에 정합시키는 monoblock 개념의 생역학적 근거를 제공 (susita-2026).
> - **메타-근거 계층 (Fathi 2022 umbrella review)**: SR/MA 14편·원저 118편·표본 10,971개를 AMSTAR2로 평가한 우산리뷰 결과, 이 overview의 결론들은 메타 수준에서 교차 검증된다 — 단일 크라운 5년 생존율 ~98.3% vs 엔도크라운(Endocrown) ~91.4%(Al-Dabbagh 구성요소); 포스트 유무(94% vs 92%) 차이 비유의; 저위험편향 리뷰에서만 파이버포스트 성공률이 금속 우위, 치근파절률은 동등; **포스트 탈락(debonding)이 단일크라운·고정성보철의 1위 실패 양상**. 단 이질성이 커 중등도 신뢰도.
...
Synthesis of 23 post-and-core papers (including Fathi 2022 umbrella review of 14 SR/MA, 118 studies, >10,971 samples — AMSTAR2-graded meta-level validation): what actually weakens an ETT is coronal cavity preparation, not endodontics itself — MOD cavities cost ~63% cuspal stiffness vs ~5% for root canal treatment alone (Reeh 1989) — which is why post placement, ferrule preservation, and coverage extent (crown vs onlay vs direct resin) all outrank post material as decisions.
...
근관치료치(Endodontically Treated Tooth, ETT) 수복 23편(Fathi 2022 umbrella review 포함 — SR/MA 14편·원저 118편·10,971+ 표본 AMSTAR2 메타검증) 종합 — 치아를 실제로 약화시키는 것은 근관치료 자체(강성 손실 ~5%)가 아니라 수복을 위한 MOD cavity 삭제(~63%)이며(Reeh 1989), 이 때문에 포스트 여부·ferrule 보존·수복 범위(크라운 vs 온레이 vs 직접레진) 결정이 모두 포스트 재료 선택보다 우선한다.
...
3. **Then, and only then, material — and material barely matters statistically, except in severely damaged teeth.** Across the full ETT spectrum, glass-fiber and metal posts show no statistically significant survival difference (Tsintsadze 2022 SR+MA; Van Landuyt 2025 15-yr RCT). But when the analysis is restricted to **severely damaged teeth with ≤2 remaining coronal walls**, an RCT-only SR+MA (4 RCTs, GRADE high for survival) finds fiber posts survive significantly *better* than metal (RR 0.57, 95% CI 0.33–0.97, P=.04), with no difference in root-fracture rate — countering the old notion that stiffer metal posts protect the root (Wang 2019). What otherwise separates the materials is **failure mode**: rigid metal/cast posts concentrate stress and fail by catastrophic, unrestorable root fracture, whereas fiber posts (and endocrowns) fail in repairable ways (Lenz 2023). The umbrella review (Fathi 2022) adds that across the SR/MA pool, **post debonding is the leading failure mode** for single crowns and fixed prostheses — meaning the bond between post and root canal wall, not root fracture, accounts for the majority of clinical failures, and this too favors fiber (elastic modulus-matched) systems whose debonding is recoverable. This failure-mode logic extends to *customized nonmetallic* systems: a 12-study/829-tooth SR found CAD/CAM zirconia comparable to cast metal, PEEK/PEKK prone to debonding, and individually formed fiber-reinforced composites (EverStick, Ribbond) favorably (repairably) failing (Yuen 2025). The FEA literature (susita-2026) explains why — matching post elastic modulus to dentin distributes stress rather than concentrating it at the post-dentin interface (the "monoblock" rationale). Bench data on the specific **ParaPost** (titanium) system corroborate this: photoelastic analysis shows the rigid ParaPost concentrates stress apically (vertical) and cervically (oblique) while fiber and cast posts distribute it evenly (Rezaei 2010), and paired fracture-resistance studies find pooled FRC-post specimens significantly out-resist ParaPost specimens (397.6 vs 265.6 N, P<0.001, Izadi 2020; replicated 380.1 vs 262.8 N, Goutam 2022) — with the core build-up material (Photo Core / LuxaCore / Core Max II) making no difference, reinforcing that the post, not the core resin, governs mechanical behavior.
...
- [[post-and-core/fathi-2022-prosthetic-restoration-endodontically-treated-teeth-umbrella]] — **[source]** AMSTAR2 umbrella review (14 SR/MA, 118 studies, >10,971 samples): single crown 5yr ~98.3% vs endocrown ~91.4% survival; post debonding is the #1 failure mode; fiber > metal only in low-bias SR/MA on *success rate*, root-fracture rates equal; moderate confidence overall (9/14 SR/MA low risk of bias)
...
- [[post-and-core/lenz-2023-endocrown-core-crown-biomechanical-sr]] — endocrown vs (post-)core-crown biomechanics SR (31 studies, all tooth types)
- [[post-and-core/al-dabbagh-2024-circumferential-ferrule-fiber-posts-survival]] — ferrule effect (strongest modifiable predictor)
- [[post-and-core/hawthan-2023-survival-fixed-prosthetics-vital-nonvital]] — vital vs nonvital abutment survival; post type × crown material interaction
- [[post-and-core/tsintsadze-2022-survival-rates-glass-fiber-metal-posts]] — glass-fiber vs metal survival SR+MA
- [[post-and-core/wang-2019-fiber-posts-vs-metal-posts-severely-damaged]] — fiber > metal survival in severely damaged ≤2-wall ETT (RR 0.57, GRADE high)
- [[post-and-core/van-landuyt-2025-rct-glass-fiber-versus-metal-posts]] — 15-yr RCT; failure-mode divergence
- [[post-and-core/yuen-2025-nonmetallic-customized-post-core-outcomes]] — customized nonmetallic posts (zirconia/PEEK/PEKK/FRC) outcomes
- [[resin/susita-2026-comparative-analysis-stress-distribution-teeth]] — fiber-post FEA stress distribution / modulus matching
- [[post-and-core/rezaei-2010-comparison-stresses-fiber-parapost-casting]] — photoelastic: rigid ParaPost concentrates stress, fiber/cast distribute it (failure-mode mechanism)
