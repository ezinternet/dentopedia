---
title: "Overview: Severe Jaw Atrophy — the Graftless Rehabilitation Ladder (숏·경사·관골·골막하 임플란트)"
...
| [[sinus-lift/lateral/rocha-2023-sinusitis-rate-sinus-lift-zygomatic-ma]] | SR+MA | 27 studies (9 ZI / 18 SFE) | 3 — cost layer | Sinusitis 3.76% ZI vs 1.11% sinus lift overall; **sinus-slot ZI 21.62% vs extrasinus ZI 0.00%**; lateral SFE 1.35% vs transcrestal 0.00% — risk is the path, not the procedure | sr+ma |
...
```mermaid
flowchart TD
    A[Severe jaw atrophy<br/>augmentation refused, contraindicated,<br/>or anatomically futile] --> B{Residual bone height<br/>allows an intraosseous implant?}
    B -->|Posterior, RBH ~5-6 mm| C[RUNG 1: short implant<br/>95.36% at 8 y, MBL plateaus after y3<br/>maxilla = higher risk, hygiene = protective]
    B -->|Anterior/palatal bone available,<br/>full-arch| D[RUNG 2: tilted + axial<br/>implant 93.91% / prosthesis 99.31%<br/>prefer AXIAL where anatomy permits: less MBL]
    B -->|Alveolus unusable<br/>resected or pneumatized| E{Zygomatic body intact?}
    E -->|Yes| F[RUNG 3: zygomatic implant<br/>pooled survival 97%<br/>MUST use extrasinus path]
    E -->|No / ZI failed / not a candidate| G[RUNG 4: customized subperiosteal<br/>97.8% at 3 y but 54.1% at 6 y<br/>SALVAGE ONLY, selected patients]
    F --> H["NEVER sinus-slot:<br/>sinusitis 21.62% vs 0.00% extrasinus"]
    G --> I["Watch soft tissue, not bone:<br/>exposure/dehiscence = main late failure"]
    C --> M[Every rung: maintenance program<br/>complication-free prosthesis survival only 61.9%]
    D --> M
    F --> M
    G --> M
```
...
1. **Do not climb for convenience.** Each rung buys anatomic freedom at the price of a failure mode the rung below does not have. The indication to move up is that the rung below is *anatomically unavailable*, not that it is slower or less impressive. [claude해석]

2. **Rung 1 — short implants are the default for posterior vertical atrophy, and the graft has no survival advantage over them** (Chaware 2021, RR 1.01, I²=0%). Two modifiers from the long-term cohort: maxillary placement significantly raises failure risk (p=0.02), and hygiene-visit frequency significantly lowers it (p<0.001) — the second is the only *modifiable* factor in the model, which makes recall scheduling part of the surgical plan, not aftercare. MBL stops progressing after year three. [근거강함]

3. **Rung 2 — when tilting, prefer axial where anatomy permits.** Full-arch tilted+axial rehabilitation is predictable (implant 93.91%, prosthesis 99.31%), but marginal bone loss is significantly greater around tilted than axial implants (P<.0001, Del Fabbro 2022). Tilt is a tool for reaching bone, not a free design choice; every implant that can be axial should be. [근거강함]

4. **Rung 3 — the zygomatic decision is a decision about the surgical path, not about the implant.** Pooled ZI survival of 97% is not what should worry you; the 21.62% sinusitis rate of the sinus-slot technique versus 0.00% extrasinus is (Rocha 2023). Commit to an extrasinus approach or do not commit to the ZI. In oncologic maxillae, placement at the primary resection appears modestly better than a secondary procedure (2% vs 9% failure) and radiotherapy does not preclude rehabilitation (OR 3.42, p=0.055) — but both comparisons are non-significant on small samples, so read them as "not contraindicated," not as "no added risk." [근거강함 for technique; 근거중/미검증 for timing and RT]
