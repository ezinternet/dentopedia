...
So the intuition "I pushed to 55 Ncm, so stability must be excellent" is wrong twice over: the torque does not predict the ISQ, and neither instrument sees the crestal remodeling that will actually decide the 3-year result. This independently converges with the wider ISQ↔IT decoupling documented in [[overviews/isq-loading-threshold]] and [[overviews/implant-macrogeometry-length-diameter-primary-stability-overview]].
...
**Rule 5 — When immediate loading needs the torque, take the torque.**
Lemos's immediate-loading subgroup is the one place low IT looks worse (4.2% vs 1.1% failure). A crestal remodeling cost is preferable to a non-integrating implant under load. Note this is a *subgroup of one study* (n=50/group) — weak, but it is the only held evidence pointing at this decision.
...
- [[implants/isq/cappare-2015-initial-bic-insertion-torque-depth-integral-in-vivo]] — mechanistic: torque–depth integral ↔ initial BIC at placement
