...
> - 충돌 해석: RCT는 단기(6개월)·무작위, 코호트는 12개월·비무작위(선택비뚤림)·부하 후 측정. flapless의 골보존 이점은 **존재하더라도 단기·소규모**이며 장기에 수렴할 가능성. [미검증]
> - **술후 통증·연조직은 flapless 일관 우위**: Tarpara에서 VAS day3 3.1 vs 5.7, day7 0.5 vs 2.4(둘 다 p=0.001)로 통증 약 절반, 6개월 탐침깊이도 낮음(단 12개월엔 소실). 골막혈류 보존·외상 감소가 기전. [확인]
> - **즉시식립 맥락(별도 setting)**: Pitman 2023 SR+MA(RCT only)는 flapless가 협측 경/연조직 보존에 유리한 경향이나 임상·심미 차이는 작고 술자 의존적; Mansouri 2025는 flapless 즉시식립에서 갭 골이식이 경조직 보존에 유리한 경향. [확인]
> - **발치 단독 차원에서는 flapless가 능사 아님**: Araujo 2009(개 5마리 split-mouth)는 flapless 발치도 피판 발치와 동등한 치조제 흡수 — "flapless = 치조제 보존"이라는 통념 반박. [확인]
> - **임상 결정 ladder**: ① 치유된 치조제·단순 단일치·증대 불필요·직접 시야 불필요 → flapless가 통증/연조직 이점으로 합리적 ② 골증대·열개·시야 확보·각화점막 재배치 필요 → 피판 ③ flapless 선택 시 CBCT 기반 가이드로 천공·각도오류 보정 필수(blind drilling 위험). [미검증]
