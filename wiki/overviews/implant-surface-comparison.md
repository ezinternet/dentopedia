...
**Q1.** 상악 구치부 D4 골 환자에서 임플란트 표면 처리를 선택해야 합니다. 이 오버뷰의 골질별 알고리듬에 따르면 어떤 표면이 1차 선택이고 왜입니까? *(근거: Canullo 2024 친수성 stability dip + OD)*

> **모범답안**: D4 골에서 이 오버뷰는 **CA/SLActive + OD(골밀도화) 또는 UV-PF** 조합을 1차로 제시한다. 이유는 두 가지다: ① 친수성 표면(CA/SLActive)은 ISQ 절대값을 올리는 것이 아니라 **D3/D4에서 나타나는 안정성 딥(Stability Dip)을 제거**해 조기 부하 시점을 앞당긴다(Canullo 2024 전향연구: T45 친수성 74.97 vs SLA 73.44, p<0.05). ② OD(Densah burs)는 표면처리가 아니라 osteotomy 기법으로 측방 골 압축을 통해 D3/D4 질의 낮은 골밀도를 보완한다 — 표면처리와 **보완 관계**다. UV-PF는 위축골·6개월 이상 보관 노화 임플란트에 추가 옵션으로 제시된다.

**Q2.** "HA/TCP 코팅 임플란트는 골전도(Osteoconduction)를 향상시킨다"는 문헌을 환자에게 설명했습니다. 이 오버뷰의 최근 정정 내용으로 이 설명을 수정해야 합니까? *(근거: Damerau 2021 SR+MA — 2026-08 근거 갱신)*

> **모범답안**: **수정해야 한다.** 이 오버뷰는 2026-08 근거 갱신으로 HA/TCP 코팅에 대한 우호적 서술을 **뒤집었다**: Damerau 2021(대형동물 15편 SR+MA)에서 이미 거친(rough) 비코팅 티타늄 대비 TCP/HA 코팅은 BIC 유의 우위 없음이 확인됐고, HA는 오히려 14일차에 BIC가 유의하게 낮았다(−6.94%p, p=0.001). "작은 표본 단일 연구가 큰 표본 메타분석에 뒤집힌 사례"로 명시돼 있다. 단, Mg 코팅(Alenezi 2026, BIC 유의 향상)과 Ag 코팅은 전임상에서 여전히 가능성이 있으나 인체 RCT 없음.

**Q3.** UV 광기능화(UV Photofunctionalization, UV-PF)가 생물학적 노화(Biological Aging)를 역전시키는 기전을 이 오버뷰에 근거해 설명하세요. *(근거: Wagstaffe 2019 + Houshmand 2023 — 탄화수소/카르복실기 결합)*

> **모범답안**: 이 오버뷰의 2026-08 갱신에 따르면, **산화막(TiO₂)과 오염막은 다른 층**이다. 티타늄은 가공 직후 영구적으로 산화막이 생기고 이것은 노화해도 사라지지 않는다. "생물학적 노화"란 산화막 위에 **별도로 쌓이는 부착탄소오염(Adventitious Carbon) 층** — 구체적으로 폼산·아세트산 같은 소형 유기산이 카르복실기-Ti 결합으로 화학흡착된 것(Wagstaffe 2019, XPS/STM). UV-PF가 벗겨내는 것은 산화막이 아니라 이 카르복실기-Ti 결합이다. 단, UV 파장이 중요하다: **UVC는 실제로 탄소를 제거하나, UVB는 접촉각만 개선하고 탄소는 그대로** 남는다(Houshmand 2023, EDS 확인). 따라서 "UV 처리"라고 통칭할 것이 아니라 UVC 포함 여부를 확인해야 한다.
