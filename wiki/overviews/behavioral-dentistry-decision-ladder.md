...
### 축 1 — Motivational Interviewing (MI) — 효과 stratification
...
2. **성인 치주염 + 행동 변화 (plaque control·흡연·식이) 필요** → MI 시도 가치 있으나 효과는 BoP 중심 (-6.41pp). Plaque·GI 직접 효과 약함. SRP·SPT와 병용해야.
3. **의료 일반 적응 (당뇨 조절·금연·식이)** → Lundahl 2013 SR+MA가 효과 입증. 치과 진료 중 만성질환 동기부여 보조로 가능.
...
- **Case-specific skills** — 통증 환자, 불안 환자, 만성 질환 환자별 분기.
- **Time-specific skills** — 초진 vs 재진 vs 응급 vs SPT 별 차이.
- **Emerging skills** — Shared decision-making (SDM), informed consent 강화, digital communication (telemedicine).

**임상 ladder (커뮤니케이션 교육)**:
1. **신입 치과의사·위생사** → Generic + case-specific 기본 skill 습득.
2. **임상 5년 이상** → Emerging skill (SDM·informed consent) 강화.
3. **chairside SOP** — 초진 정보 수집 + 진단 설명 + 치료 옵션 + 비용·기간 명시 + 환자 질문 + decision support.
4. **불안 환자** — Acknowledge + 명확한 설명 + 통제감 부여 + 단계적 노출.
5. **만성 질환 환자** (당뇨·항응고제·면역억제) — 위험·이득 평가 + 협조 강조 + 의학적 기록 정확성.

### 축 3 — Attendance Promotion (통증-only 환자 → 정기 검진)
...
**Currie 2021 미래 설계 4축** — 향후 개입은 이 4 dimension 다뤄야:
1. **Dentist 커뮤니케이션** — 통증-only 환자에게 정기 검진의 가치 명확히 전달.
2. **Dentist-환자 관계** — 신뢰·지속성. 단발성 진료보다 longitudinal care.
3. **인식 (perception)** — 정기 검진을 "비용"이 아닌 "예방·투자"로 frame 변화.
4. **비용** — 보험·접근성·시간 비용 모두. 무료 검진 캠페인 효과 일부 확인.

**임상 ladder (한국 임상 적용)**:
1. **통증 호소 환자** → 통증 해소 + "다음 단계" 명시 (이번 처치로 끝이 아닌 maintenance).
2. **3-6mo recall 시스템** — 자동 SMS·전화 + 환자 친화적 trigger.
3. **신뢰 구축 (1-2회 진료 후)** — 관계 형성된 환자가 정기 attendance 가능성↑.
4. **건강보험 활용** — 한국 스케일링 1회/년 보험 적용 등 정책적 trigger.
5. **노인·소아·고위험군 우선** — General population 일률 캠페인보다 stratified approach.

## Phase 2 확장 후보 (Stub)

- [ ] `wiki/overviews/mi-clinical-protocol-dentistry.md` — MI Rollnick 4-process + 치과 특이 case scenarios.
- [ ] `wiki/overviews/dental-anxiety-management.md` — Dental anxiety scoring + intervention (sedation·CBT·behavioral).
- [ ] `wiki/overviews/shared-decision-making-dentistry.md` — SDM 임상 도입 + 환자 선호도 도구.
- [ ] `wiki/overviews/korean-dental-attendance-policy.md` — 한국 보험·정책·문화적 attendance 결정인자.
