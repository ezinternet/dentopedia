---
title: "Overview: Short Implant vs Sinus Augmentation in the Atrophic Posterior Maxilla — Decision Synthesis"
...
> - **결정 1번째 gate(RBH)**: ~5–6 mm 이상 → 6–8 mm 숏이 저이환 default; ~4–5 mm 미만 → 상악동거상(survival 벤치마크 >97%, Derbishi 2026)이 신뢰 경로. 단, Lombardo 2022(RBH 4.45mm, 5mm 초단축+동시 내부거상)와 Cho 2024(RBH 2.1mm, 치조정 접근 전용 키트+staged 프로토콜, n=11)는 이 경계를 아래로 밀어내는 소규모 후향적 근거를 제시 — 대조군·생존율 데이터 부재로 practice-changing 근거는 아님.
...
```mermaid
flowchart TD
    A[Atrophic posterior maxilla<br/>reduced RBH] --> B{Residual bone height?}
    B -->|RBH ≥ 5-6 mm,<br/>placeable short 6-8 mm| C{Crown-to-implant ratio<br/>& occlusal load acceptable?}
    B -->|RBH < 4-5 mm| F[Sinus augmentation<br/>survival >97% benchmark]
    C -->|Yes| D[Short implant 4-8 mm,<br/>no sinus lift<br/>= less MBL, less morbidity]
    C -->|No / heavy bruxism,<br/>long edentulous span| E{Patient priorities?}
    E -->|Minimize morbidity / cost / time| D
    E -->|Maximize long-term track record| F
    F --> G{Crestal vs lateral?}
    G -->|RBH adequate for transcrestal| H[Transcrestal SFE<br/>less post-op pain - Menini]
    G -->|RBH very low / large lift| I[Lateral window SFE]
```
...
1c. **TSFE feasible even at RBH <3 mm with specialized kits (staged protocol).** Cho 2024 (retrospective, n=11, Seoul — Shinhung/Osstem crestal kit + Bio-Oss, staged; mean RCBH 2.1 mm) achieved bone height gain from 2.1 → 8.8 mm (Δ6.7 mm, p<0.001), zero membrane perforations, and adequate post-augmentation dimensions for subsequent implant placement (mean 202-day healing interval). This extends the transcrestal approach below the conventional 4–5 mm minimum, suggesting that with an appropriate specialized kit and staged protocol the "RBH <4–5 mm → lateral window" rule can be further challenged. Caveat: n=11 single center, no control, no implant survival data — treat as proof-of-concept, not a replacement for the RBH gate. [미검증 — n=11 retrospective, no survival endpoint]
2. **Crown-to-implant ratio / occlusal load is the second gate.** Favorable C/I and load → short implant. Heavy bruxism, unfavorable C/I, or long spans bias toward augmentation + longer implants, though survival data still do not penalize short implants. Notably, **Torres-Alemany 2020** found C/I ratio had no significant effect on implant loss (p=0.9) or MBL (p=0.36), so an elevated C/I ratio alone is a weak reason to reject a short implant — the gate is really occlusal load/parafunction, not the geometric ratio per se. One scenario does warrant caution: **Xu 2020** showed that nonsplinted short single crowns (≤6 mm) in the posterior region had poorer long-term survival (RR 0.94, p=.01), so splinting or augmentation should be weighed for isolated short single crowns. When splinting is chosen for short implants, **Anitua 2019** (retrospective, n=16/48 implants, 6.5 mm BTI, 14 months) adds a refinement: short-short (SS) splinting produced significantly less *distal* marginal bone loss (0.37 mm) than short-long (SL) splinting (0.94 mm, p<0.05), with 100% implant and prosthesis survival in both groups — biomechanically matched pairing of equally-short implants is preferred over mixing short + longer when splinting is the chosen strategy. [미검증 — retrospective, 14 months, single-center, author COI]
