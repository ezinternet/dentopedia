...
> [!summary] 한국어 핵심요약
> - 핵심 명제: 임플란트 안정성 지수 (Implant Stability Quotient, ISQ) 부하 결정의 견고한 임계값은 **<70 위험·≥70 안전** (Andersson 2019, n=745, 5년 누적생존율 (CSR) 97.3%). ISQ 언급 27편 횡단 합성.
> - ISQ는 단독 결정 지표가 아니다: 골질·삽입토크 (Insertion Torque, IT)·표면처리·시점 5요소와 함께 해석. ISQ–IT 상관은 r=0.44, I²>90%로 중등도이며 비교 불가 (Tisci 2026 SR+MA) — 두 지표 모두 측정 권장.
> - 원형 임계값 (Sennerby 2008, Oh 2008): ISQ <60 지연부하, 60–70 조기부하+모니터링, >70 즉시/조기부하. 핵심은 단일 70 컷오프가 아니라 **60–70 회색지대의 존재**.
> - 즉시부하 결정: ISQ ≥70 + IT ≥35 Ncm 양조건 충족 시 안전 (Wittneben 2023 SR). IT >40 Ncm 단독으로도 6–8주 조기부하 가능 (Andreatta 2020).
...
> - 거시디자인(macro-design) 보강 (2025–2026 RCT 3편): 테이퍼·깊은 thread는 삽입토크(IT)는 올려도 ISQ를 올린다는 보장 없음 — BLX(테이퍼) IT 우세에도 6주 ISQ는 BL(원통형)이 더 높음(Monje 2025), thread depth↑는 종단 ISQ·MBL 동등에 정확도만 악화(Kwon 2026); 단 즉시부하 3년 추적은 테이퍼가 복합 실패 더 적음(Song 2026). 고-IT를 ISQ 대용으로 쓰지 말 것.
> - 표면처리 신근거 (2022·2025): UV-PF 임상 SR+MA(Lang 2022)가 골유착 속도지수(OSI) MD=+2.13으로 "도달 속도 가속"을 정량 확증(절대 ISQ·MBL pooled은 비유의); SLA vs SLActive 직접 비교 RCT(Vílchez 2025)는 ISQ 궤적·12개월 MBL 동등 → 친수성 ISQ 효과는 D3/D4·가속부하 한정.
> - 신선 발치와: ISQ가 인공적으로 낮음 (~57 vs 치유골 ~72, Bavetta 2019) → IT 값 (ITV) ≥35 Ncm을 1차 기준으로.
> - ISQ 예측인자 (Huang 2017, 557 임플란트 다변량 회귀): T1 ISQ의 일반 예측인자는 **골이식 필요 여부**(계수 −4~−5), T2 ISQ의 일반 예측인자는 **임플란트 직경**(계수 +3.4~+4.2). 길이·성별·연령·골질·치유 단계는 T2에 유의하지 않음. 직경 ≥5mm가 2차 안정성을 확보하는 설계 근거.
> - 치조정 피질골 두께가 ISQ의 지배적 결정인자 (Chatvaratthana 2017 전향적, n=19): r=0.885로 가장 강한 단일 인자; **3 mm 깊이** 협설측 피질골(r=0.535)과 피질/해면골 비율(r=0.488)만 유의 — 6·9 mm 깊이 파라미터는 무관. 해면골 두께는 어떤 깊이에서도 ISQ와 무상관.
...
이 overview는 LLM-Wiki 안에서 ISQ를 언급한 27편의 페이지를 카테고리 횡단으로 모아 합성한다. 근거 등급은 SR+MA 4편 · SR 6편 · RCT 3편 · Prospective 9편을 1차 근거로, narrative-review 9편과 in-vitro 6편을 보조 인용으로 사용했다.

핵심 질문 4개에 답하는 구조다:
1. **부하 시점을 결정하는 ISQ 임계값은 무엇인가** — 절대 임계값 vs. 상대 임계값
2. **골질·잔존골·표면처리에 따라 임계값은 어떻게 변하는가** — 조정 인자
3. **ISQ를 단독 결정 지표로 신뢰할 수 있는가** — 한계와 보완 지표
4. **임상 시나리오별로 어떻게 적용하는가** — 즉시식립·상악동거상·D4 골·심미부
...
| ISQ | 권고 | 근거 시점 |
|---|---|---|
| < 55–60 | 지연 부하 (Delayed loading) | 2008 narrative |
...
이 두 페이지가 모든 후속 임상연구의 기준선이 되었다. 핵심은 **70이라는 단일 컷오프가 아니라 60–70 회색지대의 존재**다.

---

## 2. Confidence 등급별 결론 매트릭스

> 같은 임상 질문에 대해 근거 등급별로 결론이 어떻게 다른지 정리한 매트릭스. **상단 등급이 강한 결론, 하단 등급이 보조 정황** 으로 읽으면 된다.

### 2.1 ISQ 절대 임계값과 임플란트 생존

| Confidence | 출처 | n | 결론 | 임계값 |
|---|---|---|---|---|
...
| **retrospective(n=745)** | [[implants/isq/andersson-2019-rfa-factors-5year-neoss-survival]] | 745 implants, 5yr | **ISQ < 70 at placement = 유의한 실패 예측 (p<0.05)** | **<70 위험, <75 주의, ≥75 저위험** |
| **prospective** | [[implants/isq/bavetta-2024-isq-osstell-osseo-device-comparison]] | 80 implants, 1yr | D1–D2 vs D3–D4 모든 시점 유의차 (p<0.05) | 골질이 임계값 결정 |
...
| **rct(n=54, thread depth)** | [[implants/isq/kwon-2026-thread-depth-guided-immediate-implant-isq-rct]] | 깊은 thread depth → IT↑(≥40Ncm 비율↑)이나 **종단 ISQ·12개월 MBL 동등**, 오히려 각도편차↑(p=0.029, 하악 대구치 최악) | 즉시식립 가이드 |
| **rct(n=28, 61임플란트, 3yr)** | [[implants/isq/song-2026-tapered-straight-implants-immediate-loading-3year]] | 테이퍼 self-tapping이 즉시부하 복합 실패 현저히 적음(4 vs 13), 환자단위 생존 95.8 vs 78.3%(NS); MBL <1mm 동등 | 구치부 즉시부하 |
...
Lekholm-Zarb 분류 기반. [[implants/rosa-2024-do-dental-implants-bone-types]] 에서 발췌, [[implants/isq/bavetta-2024-isq-osstell-osseo-device-comparison]] prospective 데이터로 보강.

| 골질 (Lekholm-Zarb) | 부위 | ISQ 평균 범위 | 부하 결정 권고 |
|---|---|---|---|
| **Type I** (homogeneous compact) | 하악 전치부 | 72–80 | 표준 임계값 적용 (≥70 안전) |
| **Type II** (dense spongy + thick cortical) | 전치부 | 72–75 | 표준 임계값 적용 |
...
- 골밀도화 (Osseodensification, OD)가 type IV에서 가장 큰 ISQ 상승 (Fontes-Pereira 2023 SR)

### 2.4 표면처리·기법별 ISQ 트라젝토리

| Confidence | 표면/기법 | 결론 | 출처 |
|---|---|---|---|
...
| **rct(n=68, split-mouth)** | 변형 SLActive vs 기존 SLA (BLT Roxolid) | **ISQ 궤적·12개월 MBL 동등** (0.04 vs 0.07mm, p=0.658), 양군 생존 100% — 친수성 표면화학 차이가 ISQ trajectory를 바꾸지 않음 | [[implants/isq/vilchez-2025-sla-slactive-split-mouth-rct]] |
| **prospective(n=36, D3/D4)** | 생체활성 나노표면 vs 전통 SLA | **생체활성표면이 stability dip 제거**; 4 vs 7 케이스 지연부하 | [[implants/isq/canullo-2024-bioactive-surface-poor-bone-isq-stability]] |
...
- "표면처리로 임계값 자체가 내려간다"가 아니라 "임계값 도달 속도가 빨라진다"가 정확한 표현 — **Lang 2022 SR+MA가 OSI(=ΔISQ/치유기간) MD=+2.13으로 이 "속도" 명제를 정량 확증**. 반대로 **Vílchez 2025 RCT는 SLActive vs SLA 표면화학 차이만으로는 ISQ 궤적이 동등**함을 보여, 친수성의 ISQ 효과가 좋은 골(D1–D2)에서는 미미하고 **D3/D4·가속부하 시나리오에 한정**됨을 시사
...
### 3.1 표준 임플란트 식립 (D1–D3 골)

```
식립 → IT 측정 → ISQ 측정
├─ ISQ ≥75 + IT ≥35 Ncm: 즉시부하 가능
├─ ISQ 70–74 + IT ≥35 Ncm: 조기부하 (6–8주)
├─ ISQ 65–69: 표준 부하 (3–4개월) + 6주 ISQ 재측정
├─ ISQ 60–64: 지연부하 + ISQ 모니터링; 표면처리·디자인 점검
└─ ISQ <60: 4–6개월 지연 또는 재평가
```

근거: Andersson 2019 (retrospective, n=745) · Wittneben 2023 (SR) · Oh 2008 (in-vivo) · Sennerby 2008 (narrative)

### 3.2 D4 골 (상악 구치부, 후방위축)

```
CBCT 치조정 피질골 두께 평가  ← Chatvaratthana 2017 prospective
└─ 피질골 <1mm: 추가 보강 고려

식립 → IT + ISQ
├─ ISQ ≥65 + IT ≥35 Ncm + 친수성/생체활성 표면: 6–8주 부하 가능
├─ ISQ 60–64 + 전통 SLA: 3–4개월 지연부하
├─ ISQ <60: OD·UV-PF·표면 강화 임플란트 재선택 검토
└─ 옵션: 골밀도화 (OD) 적용 시 baseline ISQ ~5–10 상승 기대
```

근거: Rosa 2024 (SR+MA) · Canullo 2024 (prospective) · Fontes-Pereira 2023 (SR) · Hirota 2020 (prospective)

### 3.3 즉시식립 (Immediate Placement, Type-1)

```
발치 후 즉시식립 → IT 측정이 1차 기준
├─ IT ≥35 Ncm: 즉시부하 후보 (ISQ 보조)
├─ IT 25–35 Ncm + ISQ ≥65: 임시보철 비기능부하 가능
└─ IT <25 Ncm: 지연부하 (12주 이상)

주의: 발치와 ISQ는 인공적으로 낮음 (~57 vs 치유골 ~72) — Bavetta 2019
...
```
RBH (잔존골 높이) 평가
├─ RBH ≥6mm: TSFE + 동시식립 가능
│   ├─ OD 사용: ISQ ~66 at placement (vs 오스테오톰 ~55) — El-Ghobashy RCT
│   └─ 부하 시기: 3개월 ISQ 재측정 후 결정
├─ RBH 4–6mm: TSFE + OD 우선 (Shalash 2023 prospective: 평균 거상 4.42mm, 1년 100%)
└─ RBH <4mm: 측창식 거상 + 2단계 protocol

2단계 측창식 ISQ 궤적 (Ramirez-Fernandez 2017):
T0(식립) 63.8 → T2(3mo) 73.5 → T3(6mo) 74.65 [BBM 이식]
→ 6개월에 native bone과 동등 (Jelusic 2014)
```

근거: Starch-Jensen 2025 (SR+MA) · El-Ghobashy (RCT) · Ramirez-Fernandez 2017 · Jelusic 2014 · Shalash 2023 (prospective)

상세는 [[overviews/sinus-lift-isq-loading-timing]] 참조.
...
| 한계 | 보완 지표 |
|---|---|
| ISQ–IT 상관 r=0.44, I²>90% | IT 병행 측정 (≥35 Ncm) |
| ISQ는 치조정 **3mm 이내 피질골**만 반영 (6–9mm 깊이 무관, 해면골 무관) | CBCT 치조정 피질 두께 사전 측정 — r=0.885 (Chatvaratthana 2017 prospective) |
| 단일 ISQ 값은 trajectory 무시 | 식립·2주·6주 시계열 측정 |
| 골질이 ISQ 값을 좌우 | Lekholm-Zarb 분류로 임계값 조정 |
...
| 항목 | 결론 | 근거 |
|---|---|---|
| Osstell Mentor 신뢰성 | ICC=0.97 (탁월), 단회 측정 충분 | [[implants/isq/herrero-climent-2013-osstell-isq-reliability-icc]] (cross-sectional) |
| Osstell vs Osseo 100 | r>0.95, 호환 가능 | [[implants/isq/bavetta-2024-isq-osstell-osseo-device-comparison]] (prospective) |
| SmartPeg type I vs II | 동등 | Herrero-Climent 2013, Kastel 2019 in-vitro |
| 측정 방향 (협설 vs 근원심) | 임상적 유의차 없음 | Herrero-Climent 2013 |
| 조임 토크 (수기 vs 기계) | ISQ에 영향 없음 | Kastel 2019 in-vitro |
| **Osstell Beacon+(ISQ) vs Anycheck(IST)** | 동등한 경향 (골밀도·길이 반응); Anycheck가 측정자 독립적 (표준 토크 고정) | [[implants/isq/lee-2024-primary-implant-stability-isq-devices-invitro]] (in-vitro, n=60, polyurethane) |

**실무**: SmartPeg 한 번 체결, 협설 측정 한 번이면 충분. 4방향 평균 의무 아님.
...
3. **ISQ 트라젝토리 패턴 분류** — Dip 유형 (없음·얕음·깊음·진행성 하락) 분류와 실패 위험 매트릭스가 아직 정립 안됨.
4. **즉시식립에서 ISQ 무용성** — Bavetta 2019에서 발치와 ISQ 57.6도 100% 성공. 발치와 전용 임계값 또는 ISQ 외 지표 (Periotest 등) 필요.
...
3. **6–8주 조기부하**: IT >40 Ncm 단독으로도 가능 (Andreatta 2020 prospective)
4. **D4 골**: ISQ 58–65이 정상 분포 — 70 미달이라고 자동 지연 결정 아님; 표면처리·OD로 보정 (Rosa 2024 SR+MA + Canullo 2024 prospective)
5. **신선 발치와**: ISQ 신뢰 불가, ITV ≥35 Ncm 우선 (Bavetta 2019 retrospective)
6. **상악동 동시식립**: OD 사용 시 식립시 ISQ ~10 상승 (Starch-Jensen 2025 SR+MA)

**ISQ를 절대 단독으로 결정에 사용하지 말 것.** IT, 골질, 표면처리, 시점을 함께 평가.
...
- [[implants/isq/vilchez-2025-sla-slactive-split-mouth-rct]] — reinforces: SLA vs SLActive 직접 비교 split-mouth RCT (n=68/136임플란트). ISQ 궤적·12개월 MBL(0.04 vs 0.07mm) 동등, 생존 100% — 친수성 표면화학 차이가 ISQ trajectory를 바꾸지 않음을 §2.4에서 확인 (rct, 2025).
- [[implants/isq/monje-2025-implant-design-stability-early-healing-rct]] — refines: BLX(테이퍼) vs BL(원통) RCT (85임플란트, 6주 dense RFA). 테이퍼 IT 우세(42.4 vs 30.6 Ncm)지만 6주 ISQ는 원통형이 더 높음(75.2 vs 72.2, p=0.02) — IT≠ISQ 임상 재확인 (rct, 2025).
- [[implants/isq/kwon-2026-thread-depth-guided-immediate-implant-isq-rct]] — refines: thread depth RCT(54개). 깊은 thread → IT↑이나 종단 ISQ·12개월 MBL 동등, 각도편차 오히려↑(p=0.029) — 거시디자인으로 IT만 올려도 ISQ·정확도 이득은 없음 (rct, 2026).
- [[implants/isq/song-2026-tapered-straight-implants-immediate-loading-3year]] — extends: 테이퍼 vs 직선 즉시부하 3년 RCT(61임플란트). 테이퍼 복합 실패 현저히 적음(4 vs 13), MBL <1mm 동등 — 즉시부하 시나리오에서 테이퍼 매크로디자인 유리 (rct, 2026).
- [[implants/isq/al-juboori-2024-cortical-thickness-implant-stability-sr]] — reinforces: 피질골 두께 SR — 두꺼운 피질골이 1차·2차 안정성 모두 향상(IT·ISQ↑); di-Stefano 2021·Nkenke 2003 재확인 (§2.1) (sr, 2024).
- [[implants/surface/lang-2022-photofunctionalization-implant-stability-marginal-bone-loss-sr-ma]] — reinforces: UV-PF 임상 SR+MA — OSI MD=+2.13(p=0.007)으로 "표면처리는 임계값 도달 속도를 높인다" 명제를 정량 확증, 절대 ISQ·MBL pooled은 비유의 (§2.4) (sr+ma, 2022).

**갱신 메모 (2026-06-26)**: 이번 갱신은 본 overview의 두 핵심 thesis를 **2025–2026 RCT/SR+MA로 직접 보강**한다 — (1) "ISQ≠IT, 둘 다 측정하라"(§2.2): 거시디자인 RCT 3편(Monje·Kwon·Song)이 테이퍼·깊은 thread가 IT는 올려도 ISQ는 보장 못 함을 임상에서 재확인. (2) "표면처리는 임계값이 아니라 도달 속도를 바꾼다"(§2.4): Lang 2022 SR+MA가 OSI로 속도 효과를 정량화하고, Vílchez 2025 RCT가 좋은 골에서 SLA≈SLActive ISQ 동등을 보임. 6대 원칙·임계값은 불변, 근거 등급만 상승.

### 신규 추가 (2026-06-28)

- [[implants/isq/huang-2017-multivariate-regression-isq-prediction]] — extends: 557 임플란트 다변량 회귀분석 (SICace + Osstem, 2 surgeon, 2 clinic). T1 일반 예측인자 = 골이식(β=−4~−5), T2 일반 예측인자 = 직경(β=+3.4~+4.2); 길이·성별·연령·골질은 T2에서 비유의 — §2.1 표 및 §4 "骨질이 ISQ를 좌우" 보강 (retrospective, 2017).
- [[implants/isq/chatvaratthana-2017-cortical-bone-crestal-buccolingual-isq]] — reinforces: 전향적(n=19, Conelog, 구치부). 치조정 피질골 두께 r=0.885; 3mm 깊이 피질/해면골 비율 r=0.488; 6·9mm 깊이·해면골 두께 비유의 — §2.1 피질골 열 및 §4 "ISQ는 치조정 3mm 이내 피질골만 반영" 명제 직접 확증 (prospective, 2017).
- [[implants/isq/lee-2024-primary-implant-stability-isq-devices-invitro]] — extends: 체외연구(n=60, polyurethane 15/30 PCF, 3 length). Osstell Beacon+(ISQ/RFA) vs Anycheck(IST/DCA): 두 기기 골밀도·길이 반응 동등(R²↑), Anycheck가 측정자 독립적(표준 토크 고정 vs SmartPeg 수기조임 변동); 저밀도골에서 길이 증가 효과 확인(>10mm), 중밀도골 10mm 이상 부가 효과 없음 — §5 기기 신뢰성 표 보강 (in-vitro, 2024).
...
이 overview는 LLM-Wiki 보유 27편을 기반으로 합성되었다. 다음 영역의 추가 PDF 확보가 권장된다:
- ISQ 절대 임계값에 대한 prospective multi-system RCT
- D4 골 + 표면처리 조합별 최적 부하 시점 RCT
- 즉시식립 발치와 전용 안정성 지표 (ISQ 대안)
- 장기(>5년) ISQ–MBL 상관 prospective

## Clinical Quiz
<!-- quiz_spec -->

**Q1.** 상악 구치부에 식립한 임플란트의 ISQ를 측정했더니 65입니다. 이 오버뷰에 근거해 부하를 지연해야 합니까? *(근거: Type IV 골 보정 원칙)*

> **모범답안**: **자동 지연 불필요.** 이 오버뷰는 "Type IV 골(상악 구치부)에서 ISQ 58–65가 정상 분포"임을 명시한다(Rosa 2024 Type IV 실패위험 1.92배이나 이것은 이미 정상 임계값 범위 내 수치). 70 미달이라고 자동 지연 결정하는 것은 Type IV 골의 생물학적 특성을 무시한 오판이다. 올바른 접근: ① IT(삽입토크)를 함께 확인해 ≥35 Ncm인지 체크 ② 60–70 회색지대에서는 개별 모니터링(2–4주 후속 RFA)으로 안정성 딥 이후 궤적 확인 ③ Type IV에서는 친수성/생체활성 표면·OD가 안정성 딥을 제거해 더 안전한 부하 결정을 도와준다.

**Q2.** 즉시식립 후 즉시부하(72시간 이내 임시보철 장착)를 계획합니다. 이 오버뷰가 제시하는 즉시부하 결정 기준 2가지를 설명하세요. *(근거: Wittneben 2023 SR + Andreatta 2020)*

> **모범답안**: 이 오버뷰는 즉시부하에 **ISQ ≥70 + IT ≥35 Ncm 양조건 충족**을 요구한다(Wittneben 2023 SR). 하나만 충족해서는 부족하다 — ISQ–IT 상관이 r=0.44에 불과해(Tisci 2026 SR+MA) 두 지표는 호환 대리변수가 아니다. 6–8주 조기부하의 경우 IT >40 Ncm 단독으로 근거가 있다(Andreatta 2020). 신선 발치와에서는 ISQ가 인위적으로 낮게 측정되므로(치유골 ~72 대비 ~57, Bavetta 2019) IT를 1차 기준으로 삼는다.

**Q3.** 신선 발치와에 즉시식립 직후 ISQ를 측정했더니 56입니다. 즉시부하를 포기해야 합니까? 이 오버뷰의 해석 원칙을 설명하세요. *(근거: 신선 발치와 ISQ 특성 + ITV 우선 원칙)*

> **모범답안**: 신선 발치와에서 ISQ는 **인위적으로 낮게 측정**된다(치유골 평균 ~72 대비 ~57, Bavetta 2019). 이 수치 자체로 부하 가능 여부를 판단하면 안 된다. 이 오버뷰는 "신선 발치와에서는 IT(삽입토크, ITV) 값을 1차 기준으로 사용"할 것을 명시한다. IT ≥35 Ncm이 확인된다면 즉시부하를 고려할 수 있다. 또한 안정성 딥 이후 ISQ 궤적 — 단일 시점 절대값보다 2–4주 후속 RFA 측정 — 이 더 신뢰할 수 있는 정보를 제공한다.
