...
> - 내인성 조절자인 5α-환원 신경스테로이드 (neurosteroid, 예: allopregnanolone)는 주로 T-type Ca²⁺ 통로 (CaV3.2) 차단으로 강력한 말초 진통을 내고, GABA-A 강화는 단독으로는 진통을 못 내지만 T-type 차단 진통을 증폭한다 (pathirathna-2005).
> - 이 신경스테로이드 축은 폐경 후 여성 호발 BMS의 "성호르몬·신경보호 스테로이드 저하" 가설과 분자 접점을 이룬다 (pathirathna-2005 ↔ jaaskelainen-2018).
...
Synaptic inhibition by GABA-A and glycine receptors is carried by Cl⁻ flux, so its *direction and strength* depend on the transmembrane Cl⁻ gradient, i.e. the anion equilibrium potential (E_Cl). That gradient is set by two cation-chloride cotransporters: **NKCC1** (accumulates intracellular Cl⁻) and **KCC2** (extrudes it). [[orofacial-pain/price-2009-chloride-regulation-pain-pathway]] reframes Gate Control Theory in these molecular terms: downregulation of KCC2 (and/or upregulation of NKCC1) collapses the Cl⁻ gradient, GABAergic inhibition fails, and hyperalgesia/allodynia emerge — the same disinhibition logic seen in epilepsy. This is a narrative review centered on the spinal dorsal horn (the first central synapse).

## 2. Same channel, opposite sign — the peripheral inversion

[[orofacial-pain/wilke-2020-chloride-underrated-ion-nociceptors]] extends the chloride story to primary afferents (DRG/nociceptors), which maintain **unusually high intracellular Cl⁻**. The consequence is a paradox central to orofacial nociception: opening the *same* Cl⁻ channels that hyperpolarize (inhibit) dorsal-horn neurons can **depolarize (excite)** peripheral sensory neurons, depending on the actual Cl⁻ reversal potential. The review catalogs the nociceptor Cl⁻-channel repertoire — GABA-A, glycine, Ca²⁺-activated Cl⁻ channels (anoctamin/TMEM16A, bestrophin, tweety), CLC, CFTR, VRAC — and flags **anoctamin 1 (TMEM16A)** as a prominent excitatory Ca²⁺-activated Cl⁻ channel, with inflammatory mediators directly modulating Cl⁻ flux. Takeaway: chloride is a legitimate analgesic target *in the periphery*, not only centrally.

## 3. Endogenous modulation — neurosteroids and T-type Ca²⁺ channels

[[orofacial-pain/pathirathna-2005-t-type-calcium-gaba-neurosteroids]] (rat in vivo thermal nociception + in vitro electrophysiology) dissects how endogenous 5α-reduced neurosteroids (e.g. allopregnanolone) produce potent **peripheral** analgesia. Using mechanism-selective analogues: a T-type-only compound (ECN) is analgesic; a GABA-A-only compound (CDNC24) is **not** analgesic alone; but sub-threshold CDNC24 + ECN gives markedly enhanced, bicuculline-sensitive analgesia. Conclusion: **T-type Ca²⁺ channel (CaV3.2) blockade is the primary peripheral anti-nociceptive mechanism; GABA-A potentiation amplifies but does not independently drive it.** Because these are gonadal/neuroactive steroids, this axis is the molecular bridge to BMS's postmenopausal predominance (Section 4).
...
- The subtypes **overlap** within individuals.
