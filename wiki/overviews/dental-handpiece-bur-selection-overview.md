---
title: "Overview: Dental Handpieces & Burs — Type Selection, Indications & Cutting-Instrument Alternatives"
type: synthesis
category: overviews
date: 2026-06-25
...
source_papers:
  ...
  - wiki/dental-handpiece/zhang-2020-er-cr-ysgg-microleakage-cavity-preparation-sr-ma.md
  ...
  - wiki/dental-handpiece/pegg-2019-air-driven-dental-unit-mechanics.md
...
> [!summary] 한국어 핵심요약
> - **치과 회전절삭 시스템 (rotary cutting system)** 선택은 세 축의 문제 — ①핸드피스 종류(에어터빈/전동 하이스피드/로우스피드), ②bur 종류(다이아몬드=연삭, 텅스텐카바이드=절삭), ③대안 절삭기구(어븀 레이저). 본 오버뷰는 위키가 보유한 4편으로 이 결정 틀을 정리하되, **핵심 적응증 근거(절삭효율·발열) 공백을 명시**한다.
> - **에어로졸 위험 서열**(가장 강한 보유 근거): 에어터빈 (air turbine) > 하이스피드 (전동) ≈ 로우스피드 > 초음파 스케일러 > 어븀 레이저. 에어터빈이 <1 μm 미세 에어로졸을 압도적으로 생성 (Lahdentausta 실제 환자 84명·253건; Matys 마네킹).
> - **감염위험 상황 핵심 권고**: 러버댐을 쓸 수 없으면 **에어터빈을 전동 하이스피드 핸드피스로 대체** — 같은 절삭력에 미세 에어로졸은 유의하게 낮음 (Lahdentausta).
> - **로우스피드 (low-speed) 핸드피스**는 더 굵은 (>5 μm) 입자를 내고, 실리콘러버 연마 시 입자수 자체는 최다 — 즉 "저속 = 안전"이 아니라 입자 *크기 분포*가 다름 (Lahdentausta, Matys).
> - **흡입 (suction) 이 술자 노출의 1차 가변인자**: 고용량흡입기 (High-Volume Evacuator, HVE)가 타액흡인기 대비 마네킹 에어로졸 ~4배, 술자 ~3배 감소; 넓은 맞춤형 HVE·Zirc evacuator는 최대 88% 저감 (Matys).
> - **bur 종류별 역할**: 다이아몬드 bur (연삭형)는 법랑질·세라믹·지르코니아에, 텅스텐카바이드 bur (절삭형)는 상아질·아말감·변연 마감에 유리. **지르코니아에서의 절삭효율은 인비트로 근거 보강**: 다이아몬드 버 3종 전부 카바이드 버보다 소결 지르코니아 절삭효율 유의 우위, 전 버 시간 경과 시 효율 저하·SEM 마모 확인 — 카바이드 버는 "rather ineffective" (Peters 2019 in-vitro). 임상 선호도는 Sharma 2014 설문(다이아몬드 75% > 카바이드 15%).
> - **레이저 vs bur 적응증**: Er,Cr:YSGG 레이저 와동형성은 전통 bur와 **미세누출 동등** (SR+MA, 유의차 없음) — 레이저의 가치는 봉쇄력이 아니라 *무접촉·저진동·저소음·마취 절감* (Zhang).
...
임상 결론: 레이저는 환자 편의·마취 절감 도구; 버 선택은 **기질×핸드피스 상호작용** — 지르코니아는 항상 다이아몬드 우위(Peters 2019), 비지르코니아 기질에서 전동 핸드피스 사용 시 카바이드 우위 가능성(Ercoli 2009); 전동 핸드피스 전체 절삭효율 에어터빈 대비 높고 치수강 온도 낮음; 이 토크 차이의 기계적 기원은 터빈(공기제트 직접 구동, 저토크·가변) vs 회전베인 마이크로모터(고토크·일정) 구조 차이(Pegg 2019). 기존 공백 모두 해소됨.

## Thesis

Choosing a dental cutting setup is not one decision but three linked ones — **which handpiece, which bur, and whether a non-rotary alternative (erbium laser) is warranted**. Eight papers the wiki now holds cover all three originally-flagged axes: *exposure/safety*, *adhesion*, *bur-material matching*, and *handpiece engine mechanics*. Pegg 2019 provides the mechanical structure of air-driven handpieces (turbine vs rotary-vane micromotor), explaining the torque mechanism that Ercoli 2009 proposed but did not elaborate. Ercoli 2009 bench-quantifies electric-vs-turbine cutting efficiency (electric superior; carbide > diamond on Macor with electric). Peters 2019 confirms diamond > carbide on sintered zirconia regardless of handpiece. The critical cross-paper finding: **bur-type superiority is substrate- and handpiece-dependent** — diamond always wins on zirconia; carbide may win on non-zirconia substrates with electric handpiece (higher torque allowing full carbide blade engagement). All bench evidence is abstract-only; natural-tooth substrate data remain unquantified in held papers.

## Evidence Map

| Paper | Design | n | Axis | Key Finding | Confidence |
|---|---|---|---|---|---|
| [[dental-handpiece/lahdentausta-2022-aerosol-concentrations-size-distributions-clinical]] | Prospective clinical | 84 pts / 253 procedures | Handpiece type | Air turbine = highest <1 μm + total aerosol; low-speed raises >5 μm; high-speed NS vs own background but > oral exam; **substitute air turbine → electric high-speed** in epidemic conditions | prospective |
| [[dental-handpiece/matys-2020-dental-aerosol-hazard-risk-dental]] | In-vitro manikin | 3 sites, 7 suction modes | Handpiece type + suction | High-speed > low-speed > Er:YAG laser aerosol; **HVE cuts aerosol ~4× (manikin)/~3× (operator) vs saliva ejector; Zirc evacuator 88%**; silicone-rubber low-speed polishing = highest particle count | in-vitro |
| [[dental-handpiece/zhang-2020-er-cr-ysgg-microleakage-cavity-preparation-sr-ma]] | SR+MA | 13 in-vitro (11 meta) | Laser vs bur | **Er,Cr:YSGG ≈ bur for microleakage** (NS, all margins); acid etching after laser helps self-etch adhesives (RR=2.69) not etch-and-rinse | sr+ma |
...
- *Held evidence (engine mechanics — Pegg 2019, abstract-only):* **turbine** = air-jet-driven rotor, ~300,000–400,000 RPM, low and variable torque (stall-prone under load); **low-speed air micromotor** = rotary vane motor, ~25,000 RPM, higher and more constant torque. This torque difference is the mechanistic basis for Ercoli 2009's electric > turbine cutting efficiency finding.
...
- *Held evidence (clinical outcomes):* across 20 in-vivo RCTs, **laser ≈ bur for restoration survival (RR 1.51, NS) and pulp vitality (RR 1.46, NS)**, with **significantly less anesthesia (RR 0.29) and pain (RR 0.35)** but **significantly longer excavation time (MD +2.23)**, at low/very-low certainty (Esteves-Oliveira 2025). The laser's case is therefore patient comfort and anesthesia avoidance traded against chair-time — not a restoration-quality upgrade.

**4. Exposure control (applies to every rotary choice)**
- Suction device is the dominant controllable exposure factor: **always pair rotary cutting with high-volume evacuation**, not a saliva ejector alone; wide/customized HVEs and the Zirc evacuator give the largest reductions (Matys). Combine with rubber dam, pre-procedural rinse, and adequate ventilation/fallow time (median aerosol half-life ~20 min; air-turbine fallow ~98.5 min — Lahdentausta).
...
- *Filled 2026-08-13 (abstract-only):* electric vs air-turbine cutting efficiency + temperature — Ercoli 2009: electric superior overall; carbide outperforms diamond with electric on Macor.
- *Filled 2026-08-13 (abstract-only):* zirconia-substrate bur cutting efficiency — Peters 2019: diamond > carbide on sintered zirconia.
- *Filled 2026-08-13 (abstract-only):* handpiece engine mechanics — Pegg 2019: turbine (air-jet, high RPM, low torque) vs rotary-vane micromotor (lower RPM, higher torque); full structural details pending full-text.
...
- Bur-selection evidence is a single abstract-only survey; no held bench data on real-world cutting-efficiency decline with reuse.

## Related Papers

- [[dental-handpiece/lahdentausta-2022-aerosol-concentrations-size-distributions-clinical]] — handpiece-type aerosol ranking (clinical); air-turbine → high-speed substitution.
- [[dental-handpiece/matys-2020-dental-aerosol-hazard-risk-dental]] — handpiece + suction aerosol control (bench); low-speed < high-speed < laser.
- [[dental-handpiece/zhang-2020-er-cr-ysgg-microleakage-cavity-preparation-sr-ma]] — laser-vs-bur instrument indication; microleakage equivalence + acid-etch rule.
...
- [[dental-handpiece/pegg-2019-air-driven-dental-unit-mechanics]] — narrative review: mechanical structure of turbine (high RPM, low/variable torque) vs rotary-vane air micromotor (lower RPM, higher/constant torque); compressor, rheostat, coupling, maintenance.
