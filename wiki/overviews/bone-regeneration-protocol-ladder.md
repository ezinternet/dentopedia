---
title: "Bone Regeneration — ARP & GBR Protocol Ladder"
...
본 페이지는 wiki/bone-regeneration/ 44 paper와 wiki/immediate-implant/ 일부를 4축으로 재분류한 spine. EFP/AO 2019 consensus (Avila-Ortiz)의 4 modality 분류를 적용해 임상가가 발치 시점에 즉시 따라갈 수 있도록 변환.

## Methodology (Synthesis Approach)

- spine 기준: SR+MA·Cochrane·EFP/AO consensus 우선. 단일 RCT는 spine 미진입, spoke로 인용.
- 본 페이지는 발치 후 ARP에 집중. GBR (수평·수직 ridge augmentation, peri-implantitis 동반)은 별도 페이지로 위임.
- 한국 임상 환경 조정: 이종골 (Bio-Oss/Geistlich) · 동종골 (DemBone) · 합성골 (β-TCP) 가용성 명시.
...
2. 즉시 식립 불가 + 4-6mo 후 임플란트 계획 → **ARP 수행**.
3. 임플란트 계획 없음 + 보철 (FPD·denture) → ARP 선택적. 심미부위는 권장.
4. 협측골 결손 (≥ 3mm) + 심미부위 → ARP + soft tissue seal (FGG 또는 connective tissue graft).
...
2. 협측골 결손 (1-2벽 결손) → flap 거상 + DBBM + dPTFE 막 (Cytoplast). 4-6w 후 제거.
...
- [ ] `wiki/overviews/gbr-vertical-horizontal-ridge-augmentation.md` — ARP가 아닌 GBR (이미 흡수된 ridge 재건) — block graft·tenting screw·tissue engineering.
- [ ] `wiki/overviews/sinus-lift-decision-ladder.md` — 상악 후방 잔존골 (RBH)별 lateral vs transcrestal vs osseodensification. (이미 일부 작업: [[overviews/sinus-lift-technique-selection]])
- [ ] `wiki/overviews/peri-implantitis-bone-regeneration.md` — peri-implantitis defect GBR (resective vs regenerative).
- [ ] `wiki/overviews/dbbm-vs-synthetic-vs-autogenous-comparison.md` — 재료 head-to-head 깊은 비교.
- [ ] `wiki/overviews/socket-classification.md` — Elian / Caplanis / Juodzbalys socket classification + 각 type별 protocol.

## 한국 임상 환경 조정 사항

- Bio-Oss (Geistlich, 스위스) 한국 정식 수입. 비용 1.0g 약 25-30만원.
- β-TCP — KoreaBone (한국), OsteoOn (한국) 등. 비용 1/3 수준. evidence 약함.
- 자가치아입자 — Smart Dentin Grinder (Korea Tooth Bank) chairside 처리 가능. 환자 동의 + 발치 치아 즉시 처리 prerequisite.
- 동종골 (DemBone) — 미국 evidence rich이나 한국 가용성·종교적 issue로 제한.
- Cytoplast dPTFE — 한국 정식 수입. 비용 1매 약 8-12만원.
...
- [ ] 본 overview는 baseline. 새 graft material·새 membrane 신제품 ingest 시 본 page 갱신.
- [ ] 위생사 어시스트 매뉴얼 — "ARP set-up SOP" (Bio-Oss · collagen 막 · suture · 봉합사 종류).
- [ ] 환자 안내 — "발치 후 골 보존이 왜 필요한가" 평어 설명서.
- [ ] 비용 표 — ARP add-on 옵션별 환자 부담 비교.
