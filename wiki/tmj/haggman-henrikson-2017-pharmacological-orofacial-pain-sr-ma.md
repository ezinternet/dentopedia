---
title: "Pharmacological treatment of oro-facial pain – health technology assessment including a systematic review with network meta-analysis"
authors: B. Häggman-Henrikson, P. Alstergren, T. Davidson, E.D. Högestätt, P. Östlund, S. Tranæus, S. Vitols, T. List
year: 2017
date: 2017-10-01
doi: 10.1111/joor.12539
source: haggman-henrikson-2017-pharmacological-orofacial-pain-sr-ma.md
category: [tmj]
evidence_level: sr+ma
pdf_path: /Users/oracleneo/llm-wiki/papers/haggman-henrikson-2017-pharmacological-orofacial-pain-sr-ma.pdf
pdf_filename: haggman-henrikson-2017-pharmacological-orofacial-pain-sr-ma.pdf
source_collection: external
tags: [tmd, bms, pharmacological-treatment, nma, nsaids, cyclobenzaprine, clonazepam, capsaicin, hyaluronate, corticosteroid, grade, orofacial-pain]
---

## Three-line Summary

SR plus network meta-analysis (SBU health technology assessment; 57 RCTs screened, 41 included, 2,033 patients) evaluating pharmacological treatments for orofacial pain across three subgroups: TMD-joint (15 studies), TMD-muscle (9 studies, NMA), and burning mouth syndrome (BMS, 17 studies, NMA).

NSAIDs and intra-articular injections (corticosteroid, hyaluronate) are effective for TMD-joint pain; cyclobenzaprine ranked highest in NMA for TMD-muscle; clonazepam (topical > systemic) and capsaicin ranked highest for BMS — all low to moderate GRADE evidence.

Cyclobenzaprine for TMD-muscle and clonazepam for BMS are the NMA-based top-ranked agents but carry important caveats (sedation and dependency risk respectively); uniform GRADE low evidence limits confident recommendations across all categories.

## 세줄요약

구강안면통증 약물치료를 평가한 SR+네트워크 메타분석(NMA) (SBU 보건기술평가; 57 RCT 스크리닝, 41편 포함, 2,033명; TMD-관절 15편·TMD-근육 9편 NMA·구강작열감증후군(BMS) 17편 NMA).

TMD-관절: NSAIDs·관절강내 코르티코스테로이드·히알루론산 효과적; TMD-근육 NMA: 시클로벤자프린(Cyclobenzaprine) 최고 순위; BMS NMA: 클로나제팜(topical > systemic)·캅사이신 최고 순위 — 전반적 GRADE 저~중 근거.

시클로벤자프린은 진정 부작용, 클로나제팜은 장기 의존성 위험 — 단기 처방 원칙; 전 범주 GRADE 저 근거로 신뢰도 있는 권고에 한계.

## Summary
This health technology assessment by the Swedish SBU agency systematically reviewed pharmacological treatments for orofacial pain. After screening 1,552 articles, 41 RCTs (2,033 patients) met final criteria. Three subgroups were analyzed: TMD-joint (15 studies), TMD-muscle (9 studies), BMS (17 studies). Network meta-analysis was possible for TMD-muscle (8 studies) and BMS (5 studies). Key findings: NSAIDs and intra-articular injections (corticosteroid, hyaluronate) are effective for TMD-joint pain; cyclobenzaprine for TMD-muscle; clonazepam and capsaicin for BMS. Evidence quality is uniformly low–moderate per GRADE, limiting confident clinical recommendations.

## Key Contributions
- Only NMA-based SR for orofacial pain pharmacotherapy to date (2017)
- TMD-joint: NSAIDs effective systemically; IA corticosteroid and hyaluronate both effective
- TMD-muscle NMA: cyclobenzaprine top-ranked; no other agent significantly superior to placebo
- BMS NMA: clonazepam and capsaicin reduce pain intensity significantly
- GRADE evidence low across all categories — calls for more rigorous RCTs

## Methodology
- SBU health technology assessment framework + GRADE quality rating
- PubMed / Cochrane / EMBASE to 1 March 2017; hand search
- RCTs only; chronic orofacial pain ≥3 months; adults ≥18
- Risk-of-bias: Cochrane tool → only low/medium studies included in NMA
- Primary outcome: pain intensity reduction (VAS/NRS)

## Results

| Subgroup | Treatment | Effect | GRADE |
|---|---|---|---|
| TMD-joint | NSAIDs (systemic) | Effective | Moderate |
| TMD-joint | IA corticosteroid | Effective | Low |
| TMD-joint | IA hyaluronate | Effective | Low |
| TMD-muscle | Cyclobenzaprine | Effective (NMA) | Low |
| BMS | Clonazepam | Effective (NMA) | Low |
| BMS | Capsaicin | Effective (NMA) | Low |

- NMA (TMD-muscle): cyclobenzaprine SUCRA highest; diazepam, other relaxants not significantly superior to placebo
- NMA (BMS): clonazepam (topical > systemic) > capsaicin > placebo; alpha-lipoic acid inconsistent

## Clinical Relevance
| 상황 | 1차 선택 | 대안 |
|---|---|---|
| TMD-관절통 | NSAIDs (이부프로펜 400–600 mg) | IA 히알루론산 또는 코르티코스테로이드 |
| TMD-근육통 | 시클로벤자프린 (단기) | 스플린트 + 물리치료 병행 |
| BMS | 클로나제팜 국소 rinse | 캅사이신 topical / 전신 저용량 |

주의: 시클로벤자프린은 진정 부작용 있음; 클로나제팜 장기 사용 시 의존성 위험 — 단기 처방 원칙.

## Related Papers
- [[tmj/pigozzi-2021-quality-of-life-tmd-sr-ma]] — TMD 환자 삶의 질 SR+MA
- [[tmj/bmj-2023-tmd-chronic-pain-management-guideline]] — TMD 만성 통증 치료 가이드라인
- [[tmj/wang-2024-tmj-ovd-elevation-occlusal-loss-rats]] — OVD 변화와 TMJ 손상 메커니즘
