---
title: "Incidence of Kidney Toxicity of Non-Steroidal Anti-Inflammatory Drugs in Critically Ill Patients"
authors: Gomes C, Pereira MIP, Valério P, Barroso JB, Mendes T, Pereira JG
year: 2025
date: 2025-12-01
doi: 10.1016/j.jcrc.2025.155202
source: gomes-2025-nsaids-kidney-toxicity-icu-retrospective.md
category: [drug]
confidence: retrospective
pdf_path: /Users/oracleneo/llm-wiki/papers/gomes-2025-nsaids-kidney-toxicity-icu-retrospective.pdf
pdf_filename: gomes-2025-nsaids-kidney-toxicity-icu-retrospective.pdf
source_collection: external
tags: [NSAIDs, AKI, kidney-toxicity, ICU, adverse-effects, renal-function, drug-safety]
---

## 한줄요약
ICU 후향적 코호트(n=1,157, 포르투갈, 2015–2023): NSAIDs 사용이 AKI 독립 위험인자(OR 1.98); 중증 AKI(stage 3)는 2년 생존율 악화(aHR 1.42); 신기능 회복해도 장기 생존 유의한 차이 없음 — 신기능 저하 환자 NSAID 처방 주의 근거 (J Crit Care 2025).

## Summary
Retrospective cohort study (n=1,157 ICU patients, Hospital Vila Franca de Xira, Portugal; Jan 2015–Dec 2023) assessing kidney toxicity of NSAIDs in critically ill patients. NSAID use was independently associated with AKI (OR 1.98, p=0.009) and severe AKI/stage 3 (34% vs 26%, p=0.008). Severe AKI was associated with significantly worse 2-year survival (aHR 1.42, 95% CI 1.09–1.86, p=0.011). Non-severe AKI had no significant impact on long-term survival. Renal function recovery did not significantly improve 2-year survival once severe AKI occurred. NSAIDs were far more common in surgical (62%) vs medical ICU patients (23%).

## Key Contributions
- Quantifies NSAID-associated AKI risk: OR 1.98 (independently significant)
- Severe AKI (stage 3) drives long-term harm — not just acute inconvenience
- Renal function recovery does not fully reverse survival disadvantage of severe AKI
- ICU context is extreme, but findings validate the biological mechanism relevant to all NSAID prescribers
- Dental relevance: high-risk patients (elderly, CKD, diuretic/ACEI users) need renal function consideration before NSAID prescribing post-surgery

## Methodology
Retrospective cohort. n=1,157 (ICU stay >24h). Exclusion: pre-existing renal dysfunction. AKI tracked by serum creatinine + urine output. 9-year follow-up (2-year survival outcomes). Logistic regression (AKI incidence); Cox proportional hazards (survival).

## Results
| Outcome | NSAID | Non-NSAID | p |
|---|---|---|---|
| AKI rate | 67% | 51% | 0.009 |
| Severe AKI (stage 3) | 34% | 26% | 0.008 |
| 2-year survival: severe AKI | aHR 1.42 (worse) | Reference | 0.011 |
| 2-year survival: renal recovery | 80% | 78% | 0.636 (NS) |

Surgical ICU patients NSAID use: 62% (non-scheduled), 61% (scheduled); Medical: 23%.

## Related Papers
- [[drug/etikala-2019-nsaids-periodontal-implant-therapy-review]] — NSAIDs and osseointegration; tissue-level drug concerns
- [[drug/kotowska-rodziewicz-2023-nsaids-role-dentistry-review]] — NSAIDs in dentistry; contraindications including renal
- [[drug/feldman-2024-nonopioid-opioid-third-molar-rct]] — ibuprofen+APAP PRN (not continuous) — lower cumulative renal exposure than scheduled dosing
