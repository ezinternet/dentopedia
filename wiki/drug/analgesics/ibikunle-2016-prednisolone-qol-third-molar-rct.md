---
title: "Oral health-related quality of life following third molar surgery with either oral administration or submucosal injection of prednisolone"
authors: "Ibikunle et al."
year: 2016
date: 2016-07-22
doi: "10.1007/s10006-016-0571-4"
source: ibikunle-2016-prednisolone-qol-third-molar-rct.md
category: [drug/analgesics]
evidence_level: rct
pdf_path: /Users/oracleneo/llm-wiki/papers/ibikunle-2016-prednisolone-qol-third-molar-rct.pdf
pdf_filename: ibikunle-2016-prednisolone-qol-third-molar-rct.pdf
source_collection: external
tags: [prednisolone, third-molar, QoL, OHIP-14, submucosal, oral-administration]
synthesized_in: overviews/drug-analgesics-postop-pain-overview
relations:
  - type: contradicts
    target: drug/analgesics/larsen-2021-methylprednisolone-doses-split-mouth-rct
    note: "opposite conclusion on corticosteroid vs OHIP-14 QoL after third-molar surgery — this RCT (n=186) found prednisolone (oral/submucosal) significantly improved QoL vs no-drug control; Larsen 2021 (n=52, double-blind split-mouth) found methylprednisolone null vs placebo at all three doses (20/30/40mg). Different drug/route (prednisolone oral+submucosal vs methylprednisolone IM) and design (parallel unblinded-severity vs double-blind split-mouth) are real confounds, but both test the same corticosteroid-vs-QoL question and reach opposite statistically-supported answers — a head-on conflict, not a different-angle refinement. Already flagged in prose on both pages; this formalizes the typed edge."
---

## Three-line Summary
Three-arm RCT (n=186, 62 per group) from a Nigerian tertiary hospital compared oral prednisolone 40 mg, submucosal injection prednisolone 40 mg, and no prednisolone in patients undergoing third molar surgery, using OHIP-14 as the primary outcome assessed at days 1, 3, and 7.
Both prednisolone groups showed significantly better oral health-related quality of life than controls (p<0.05); submucosal injection produced significantly less QoL deterioration than oral administration (p=0.001).
This is the largest RCT establishing OHIP-14 as primary endpoint for M3 corticosteroid research, and the positive QoL signal for prednisolone directly contrasts with Larsen 2021's null result for methylprednisolone, suggesting drug- or route-level differences warrant further investigation.

## 세줄요약
줄1: 나이지리아 3차 병원의 3군 무작위대조시험 (Randomized Controlled Trial, RCT) (n=186, 군당 62명)에서 경구 프레드니솔론 (Prednisolone) 40 mg, 점막하 주사 (Submucosal Injection) 40 mg, 무투여 대조군을 비교하여 구강건강관련 삶의 질 (Oral Health-Related Quality of Life, OHRQoL, OHIP-14)을 주요 결과 지표로 평가했다.
줄2: 프레드니솔론 투여군(경구·점막하 모두)은 미투여 대조군 대비 삶의 질이 유의하게 좋았으며(p<0.05), 점막하 주사군이 경구 투여군보다 삶의 질 저하가 유의하게 적었다(p=0.001).
줄3: OHIP-14를 주요 평가 지표로 삼은 최대 규모 RCT로, 프레드니솔론의 긍정적 삶의 질 신호는 라르센(Larsen) 2021의 메틸프레드니솔론 (Methylprednisolone) null 결과와 대비되어 약물·경로별 차이에 대한 추가 연구가 필요함을 시사한다.

## One-line Summary
Three-arm RCT (n=186) found prednisolone 40 mg (oral or submucosal) significantly improved OHRQoL vs no prednisolone after third molar surgery, with submucosal injection superior to oral administration (OHIP-14, p=0.001).

## 한줄요약
3군 RCT (n=186)에서 프레드니솔론(Prednisolone) 40 mg(경구 또는 점막하 주사)은 미투여 대비 구강건강관련 삶의 질(OHRQoL, OHIP-14)을 유의하게 개선했고, 점막하 주사가 경구 투여보다 우월했다(p=0.001).

## Summary
Ibikunle et al. randomized 186 patients undergoing third molar surgery into three equal groups: oral prednisolone 40 mg (Group A), submucosal injection 40 mg prednisolone (Group B), or no prednisolone (Group C). QoL was assessed using OHIP-14 preoperatively and at postoperative days 1, 3, and 7. All groups showed significant postoperative QoL deterioration peaking at day 1 with gradual recovery. Both prednisolone groups showed significantly better QoL than controls. Submucosal injection (Group B) produced significantly less QoL deterioration than oral administration (Group A, p=0.001). This is one of the few RCTs using QoL (rather than pain/trismus/swelling) as primary endpoint, providing patient-centered evidence for submucosal prednisolone. Contrasts with Larsen 2021 (methylprednisolone, OHIP-14 null result) — different drug, positive signal.

## Key Contributions
- Largest single RCT (n=186, equal groups) with OHIP-14 as primary endpoint for M3 surgery corticosteroid research
- Demonstrates submucosal route superiority over oral route for QoL outcomes
- Positive QoL signal for prednisolone contrasts with Larsen 2021 null result for methylprednisolone — drug-level difference or route effect?
- Patient-reported outcome (PRO) focus aligns with modern evidence standards

## Methodology
Three-arm RCT. n=186 (62 per group). Groups: A=oral prednisolone 40mg; B=submucosal injection prednisolone 40mg; C=no prednisolone. Assessment: preoperative + days 1, 3, 7. Primary outcome: OHIP-14 total and subscale scores. Classification: affected (OHIP ≤28) vs not affected (>28). Institution: Nigeria (tertiary hospital).

## Results
- All groups: Significant OHIP-14 score increase postoperatively (worse QoL), most marked at day 1
- Groups A+B vs C: Prednisolone significantly associated with better QoL and earlier recovery (p<0.05)
- Group B vs A: Submucosal injection significantly less QoL deterioration (p=0.001)
- Pattern: Day 1 worst → gradual improvement through day 7

## Related Papers
- [[drug/analgesics/larsen-2021-methylprednisolone-doses-split-mouth-rct]] — OHIP-14 null result for methylprednisolone (contradicts; different drug)
- [[drug/analgesics/liborio-2025-methylprednisolone-third-molar-sr-ma]] — QoL as secondary endpoint in SR+MA
- [[drug/analgesics/markiewicz-2008-corticosteroids-reduce-postoperative-morbidity-after-third-molar-surgery]] — foundational SR+MA
