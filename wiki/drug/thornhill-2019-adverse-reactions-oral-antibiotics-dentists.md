...
## Key Contributions
- 치과 ADR 통계의 실세계 anchor — 정밀한 백만 처방당 분율로 환자 설명·informed consent 가능.
- "Amoxicillin 알레르기 → clindamycin" 임상 관행에 정량적 도전: clindamycin 치명적 ADR 위험이 훨씬 큼.
- Macrolide의 QT/심장 부정맥 사망 위험을 처음으로 치과 처방 맥락에서 정량.
- Amoxicillin+clavulanic acid의 hepatobiliary ADR(특히 cholestatic jaundice) 단독 시그널.

## Methodology
- NHS Digital Prescription Cost Analysis: 2010–2017 영국 1차 진료 모든 경구 항생제 처방; 치과의사 별도 추출.
- Yellow Card MHRA Interactive Drug Analysis Profile: 자발적 ADR 보고 데이터.
- 비율 = ADR 보고 / 처방 수(백만 단위).
- MedDRA 코딩(27 system organ class → 14 + "other"로 단순화).
- 한계: 자발적 보고는 절대 발생률 과소평가하나 상대 비교는 유효.

## Results
- **Amoxicillin**: 처방 점유 64.8%; 전체 ADR 21.5/백만; 치명적 0.1/백만 — 최저.
- **Metronidazole**: 전체 70.6, 심각 51.4, 치명적 0.7/백만 — amoxicillin의 ~3배.
- **Clindamycin**: 치명적 2.9/백만(최고); 거의 모두 C. difficile 장염; GI ADR도 최고(155.5/백만).
- **Penicillin V**: 면역계 ADR 최다 — 20%+가 아나필락시스/아나필락토이드; 0.15 사망/백만 처방.
- **Macrolides**: 심장 부정맥 ADR이 주요 사망 원인 (azithromycin 사망의 2/3); QT 연장 → torsades de pointes.
