---
title: "Ridge alterations following implant placement in fresh extraction sockets: an experimental study in the dog"
authors: Maurício G. Araújo, Flavia Sukekava, Jan L. Wennström, Jan Lindhe
year: 2005
date: 2005-06-01
doi: 10.1111/j.1600-051X.2005.00726.x
source: araujo-2005-ridge-alterations-immediate-implant-dog.md
category: [immediate-implant]
confidence: animal
pdf_path: /Users/oracleneo/llm-wiki/papers/araujo-2005-ridge-alterations-immediate-implant-dog.pdf
pdf_filename: araujo-2005-ridge-alterations-immediate-implant-dog.pdf
source_collection: external
tags: [immediate-implant, ridge-resorption, bundle-bone, beagle, SLA, histomorphometry]
---

## 한줄요약
animal(J Clin Periodontol 2005): 비글견 즉시식립(SLA)·발치창 — 협측골 흡수 > 설측; bundle bone 완전소실; 즉시식립은 발치 후 치조제 흡수 예방 불가.

## Summary
Beagle dog (n=5) experimental study placing SLA-surface implants immediately in fresh extraction sockets. Histomorphometric analysis at 3 months shows buccal bone resorption exceeds lingual, bundle bone disappears entirely, and implant does not prevent post-extraction ridge remodeling. Companion animal study to Botticelli 2004.

## Key Contributions
- Bundle bone (tooth-dependent) resorbs completely after extraction regardless of implant
- Buccal BIC established 2.6±0.4mm apical to SLA surface margin
- Lingual BIC: 0.2±0.5mm apical to SLA margin (much closer to crest)
- Buccal bone resorption > lingual — asymmetric dimensional change
- Osseointegration confirmed histologically at 3 months
- Immediate implant does NOT prevent post-extraction ridge resorption

## Methodology
- 5 beagle dogs; bilateral split design
- Mandibular 3rd/4th premolar extraction + immediate SLA implant
- Histological processing, histomorphometric measurement
- Bone-to-implant contact (BIC), marginal bone levels assessed

## Results
| Parameter | Value |
|-----------|-------|
| Buccal BIC apical margin | 2.6±0.4mm below SLA |
| Lingual BIC apical margin | 0.2±0.5mm below SLA |
| Bundle bone | Completely resorbed |
| Buccal > lingual resorption | Yes (p<0.05) |

## Related Papers
- [[immediate-implant/botticelli-2004-immediate-implant-hard-tissue]] — human in-vivo study, same conclusion
- [[bone-regeneration/araujo-2005-dimensional-ridge-alterations-tooth-extraction]] — extraction-only (no implant) controls
