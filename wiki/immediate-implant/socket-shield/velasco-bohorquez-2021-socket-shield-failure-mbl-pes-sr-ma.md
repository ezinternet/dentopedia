---
title: "Failure Rate, Marginal Bone Loss, and Pink Esthetic with Socket-Shield Technique for Immediate Dental Implant Placement in the Esthetic Zone. A Systematic Review and Meta-Analysis"
authors: Pilar Velasco Bohórquez, Roberta Rucco, Álvaro Zubizarreta-Macho, José María Montiel-Company, Susana de la Vega Buró, Esther Cáceres Madroño, Lara San Hipólito Marín, Sofía Hernández Montero
year: 2021
date: 2021-06-18
doi: 10.3390/biology10060549
journal: Biology 10(6):549
source: velasco-bohorquez-2021-socket-shield-failure-mbl-pes-sr-ma.md
confidence: sr+ma
category: [immediate-implant/socket-shield]
pdf_path: /Users/oracleneo/llm-wiki/papers/velasco-bohorquez-2021-socket-shield-failure-mbl-pes-sr-ma.pdf
pdf_filename: velasco-bohorquez-2021-socket-shield-failure-mbl-pes-sr-ma.pdf
source_collection: external
tags: [socket-shield-technique, SST, failure-rate, MBL, PES, esthetic-zone, sr-ma, Biology-MDPI, Spain]
---

## 한줄요약
SR+MA (Universidad Alfonso X El Sabio Spain, Biology 2021): SST esthetic zone 즉시 임플란트 — **failure rate, MBL, PES 3 outcome 정량**. SST 분야 baseline data 제공, conventional 대비 미용·골보존 우월 신호.

## Summary
SST esthetic zone 적용의 핵심 outcome 3종을 동시 정량한 SR+MA. 분야 baseline data로 활용 — 이후 Lin 2022·Gharpure 2022·Ji 2025 (NMA)가 보강.

## Methodology
- Systematic review + meta-analysis
- DB 검색 + PRISMA
- Esthetic zone 한정 inclusion
- Outcomes: failure rate, MBL, PES

## Key Findings (요약)
| Outcome | SST | Conventional |
|---|---|---|
| Failure rate | 낮음 (본문) | 비교군 |
| MBL | 적음 | 비교군 |
| PES | 우월 | 비교군 |

(구체 수치는 본문 forest plot)

## Clinical Position
- **결정 트리**: esthetic zone 임플란트 미용 우선 시 SST 고려 — failure 낮음·MBL 적음·PES 우월
- **한계**: 2021 SR이므로 inclusion 한계, 후속 SR (2022+) 통합 필요
- **임상 활용**: 환자 설명 시 정량 outcome 제공 (failure %, MBL mm, PES 점수)

## Strengths and Caveats
[근거강함] SR+MA — 분야 strong evidence
[합의수준] SST 미용 우월 신호는 후속 SR과 일관
[claude해석] Heterogeneity·publication bias 평가 필요 — 본문 sensitivity analysis 검토
[미검증] Long-term (≥5년) outcome은 본 SR에도 제한적

## Related Papers
- [[immediate-implant/socket-shield/lin-2022-socket-shield-technique-systematic-review-meta]] — 후속 SR+MA
- [[immediate-implant/socket-shield/gharpure-2022-socket-shield-conventional-implant-sr-ma]] — 후속 SR+MA
- [[immediate-implant/socket-shield/ji-2025-socket-shield-conventional-network-ma]] — NMA
- [[immediate-implant/socket-shield/ogawa-2022-socket-shield-technique-systematic-review]] — SR
- [[immediate-implant/socket-shield/oliva-2023-socket-shield-buccal-bone-preservation-sr]] — JPIS SR
- [[immediate-implant/socket-shield/blaschke-2020-socket-shield-technique-critical-review]] — critical review
