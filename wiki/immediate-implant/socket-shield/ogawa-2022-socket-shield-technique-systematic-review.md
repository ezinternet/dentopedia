---
title: "Effectiveness of the socket shield technique in dental implant: A systematic review"
authors: Toru Ogawa, Ratri M. Sitalaksmi, Makiko Miyashita, Kenji Maekawa, Masahiro Ryu, Aya Kimura-Ono, Takeshi Suganuma, Takeshi Kikutani, Masanori Fujisawa, Katsushi Tamaki, Takuo Kuboki
year: 2022
date: 2021-03-09
doi: 10.2186/jpr.JPR_D_20_00054
source: ogawa-2022-socket-shield-technique-systematic-review.md
category: [immediate-implant/socket-shield]
confidence: sr
pdf_path: /Users/oracleneo/llm-wiki/papers/ogawa-2022-socket-shield-technique-systematic-review.pdf
pdf_filename: ogawa-2022-socket-shield-technique-systematic-review.pdf
source_collection: external
tags: [socket-shield, SST, immediate-implant, root-submergence, ridge-preservation, esthetic]
---

## One-line Summary
Systematic review (20 studies, 288 patients, follow-up 3–60 months) of the socket shield technique (SST) with immediate implant placement: 9.5% complication rate and 90.5% complication-free survival, but the evidence base is predominantly case reports of low methodological quality, so long-term predictability cannot be confirmed.

## 한줄요약
sr(J Prosthodont Res 2022): 소켓실드 기법 20편(288명, 3-60개월); 합병증 9.5%; 합병증 없는 생존율 90.5%; 증례보고 위주로 근거수준 낮음 — 장기 예측성 불명확.

## Summary
Systematic review (Japan Prosthodontic Society) of 20 studies covering 288 patients treated with the socket shield technique (SST) combined with immediate implant placement, follow-up 3–60 months. Demonstrates feasibility with 9.5% complication rate, but evidence base is predominantly case reports with low methodological quality.

## Key Contributions
- 20 studies: 1 RCT, 2 cohort, 14 case reports, 3 retrospective case series
- 288 patients; follow-up range 3–60 months
- Complication/adverse event rate: **9.5%** (26/274 cases)
- Survival without complications: **90.5%**
- Quality assessment: 12/20 studies rated good quality
- Cases >12 months: mostly good esthetic appearance documented
- Long-term predictability: cannot be confirmed — insufficient high-quality evidence

## Complication Types
| Complication | Description |
|-------------|-------------|
| Failed osseointegration | Implant loss |
| Shield mobility + infection | Shield movement, secondary infection |
| Shield exposure | Shield becomes visible through mucosa |
| Shield migration | Displacement of retained root fragment |
| Apical root resorption | Resorption of retained root |

## Socket Shield Technique Principle
- Retain a thin buccal root fragment in the socket
- Preserves bundle bone (PDL blood supply maintained)
- Immediate implant placed palatal to shield
- Aims to prevent buccal bone collapse without grafting

## Related Papers
- [[immediate-implant/botticelli-2004-immediate-implant-hard-tissue]] — buccal bone resorption without SST
- [[immediate-implant/araujo-2005-ridge-alterations-immediate-implant-dog]] — bundle bone biology; SST rationale
- [[immediate-implant/esthetic-soft-tissue/chappuis-2017-dimensional-bone-soft-tissue-post-extraction]] — facial wall <1mm in 90%; SST aims to preserve this
