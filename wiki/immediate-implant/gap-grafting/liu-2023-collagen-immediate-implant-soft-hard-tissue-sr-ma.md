---
title: "Influence of using collagen on the soft and hard tissue outcomes of immediate dental implant placement: A systematic review and meta-analysis"
authors: "Liu J, Hua F, Zhang H, Hu J"
year: 2023
date: 2023-01-13
doi: "10.1016/j.jormas.2023.101385"
source: liu-2023-collagen-immediate-implant-soft-hard-tissue-sr-ma.md
category: [immediate-implant/gap-grafting]
evidence_level: sr+ma
pdf_path: /Users/oracleneo/llm-wiki/papers/liu-2023-collagen-immediate-implant-soft-hard-tissue-sr-ma.pdf
pdf_filename: liu-2023-collagen-immediate-implant-soft-hard-tissue-sr-ma.pdf
source_collection: external
tags: [immediate-implant, jumping-gap, collagen, bone-graft, esthetic-zone]
relations:
  - type: extends
    target: zaki-2021-bone-substitute-materials-immediate-implant-sr-ma
  - type: reinforces
    target: sanz-2017-bone-graft-gap-immediate-implant-rct
---

## Three-line Summary

SR+MA of 7 RCTs (205 patients, 279 implants) isolating the effect of adding collagen to bone-grafting material in the jumping gap of immediate implant placement, using Cochrane ROB 2.0.

Adding collagen significantly improved buccal bone thickness (MD −0.43 mm, 95% CI −0.72 to −0.41) and esthetic score (MD −1.23, 95% CI −1.90 to −0.55), with no significant difference in implant failure rate or soft-tissue thickness.

Grafting the jumping gap with a collagen-containing bone substitute is preferable for buccal-bone and esthetic preservation in immediate implants, though two of seven RCTs had high risk of bias.

## 세줄요약

즉시식립 Jumping Gap에 골이식재 단독 대비 골이식재 + 콜라겐 병용 효과를 분석한 RCT 한정 SR+MA(7편, 205명·279 임플란트, Cochrane ROB 2.0 평가).

콜라겐 추가군에서 협측 골두께(MD −0.43 mm)와 심미 점수(MD −1.23)가 유의하게 개선되었고, 임플란트 실패율과 연조직 두께는 차이가 없었다.

콜라겐 함유 이식재 사용이 협측 골 및 심미 보존에 유리하지만, 7편 중 2편은 비뚤림 위험이 높아 근거의 질에 주의가 필요하다.

## Summary

This systematic review and meta-analysis pooled 7 RCTs (205 patients, 279 implants) to isolate the effect of **collagen** when used together with bone-grafting material in the jumping gap of immediate implant placement. Collagen incorporation conferred a significant advantage on buccal bone thickness and pink-esthetic outcomes, while implant failure rate and soft-tissue thickness were unaffected. The authors conclude that grafting the gap with a collagen-containing bone substitute is preferable for hard-tissue and esthetic preservation in immediate implants.

## Key Contributions

- Isolates the collagen variable in immediate-implant gap grafting via an RCT-only meta-analysis.
- Demonstrates a measurable buccal-bone and esthetic benefit attributable to collagen.
- Shows no failure-rate or soft-tissue-thickness penalty from collagen addition.

## Methodology

- RCT-only SR+MA; PubMed, Embase, Cochrane CENTRAL + hand search; Cochrane ROB 2.0 (2 low / 3 moderate / 2 high risk).
- Two subgroup comparisons: collagen+bone graft vs bone graft alone, and vs no graft.
- Outcomes: implant failure, buccal bone resorption/thickness, soft-tissue thickness, esthetic score. Fixed- and random-effects models.

## Results

- Implant failure: no significant difference (95% CI 0.17–11.84).
- Soft-tissue thickness: no significant difference.
- Buccal bone thickness: MD −0.43 mm (95% CI −0.72 to −0.41), favoring collagen.
- Esthetic score: MD −1.23 (95% CI −1.90 to −0.55), favoring collagen.

## Related Papers

- [[immediate-implant/zaki-2021-bone-substitute-materials-immediate-implant-sr-ma]] — extends: collagen variable added to the bone-substitute question
- [[immediate-implant/sanz-2017-bone-graft-gap-immediate-implant-rct]] — reinforces: gap grafting preserves buccal contour
- [[immediate-implant/elsheikh-2023-prf-xenograft-alloplastic-immediate-implant]] — related gap-filling material comparison
- [[immediate-implant/seyssens-2022-immediate-implant-socket-grafting-sr-ma]] — socket grafting adjunct evidence
