...
| `find-contradiction-candidates.py` | signal | 본문에 명시적 충돌 표현(contradict/counterpoint/반박 등)이 있으나 그 쌍에 `relations:` 타입 엣지가 **(어떤 타입이든)** 없고 `superseded_by:` 포인터도 없는 논쟁 레이더 백필 후보. (2026-08-15: supersession도 억제 근거에 포함 — 대체 배너 문구가 "뒤집음"·"overturn" 그 자체라 정상 처리된 대체 쌍이 매일 후보로 돌아왔다. 배너는 여러 줄 콜아웃이고 링크는 헤더에만 있어, 후속 `>` 줄이 헤더 target을 상속하게 했다 — 상속은 **대체 배너로만** 한정한다, 모든 blockquote로 넓히면 `[!summary] 한국어 핵심요약`에서 앞 bullet 링크가 뒤 bullet으로 새어 과억제된다.) Tier1(키워드에 가장 가까운 wikilink로 대상 지목)·Tier2(대상 불명/너무 멂/동일 줄 비최근접/soft). 기계가 충돌을 확정하지 않고 신호만 — LLM이 두 페이지 읽고 판단해 엣지를 단다. **type_hint를 그대로 엣지로 옮기지 말 것** — 2026-07-17 전수 검토에서 contradicts 계열 지목 122건 중 실제 contradicts는 1건이었다 |
