...
A single entry-point runs all 25 audits and writes their logs to `logs/`:
...
## The 25 audits — 4 hard guards + 1 rationale (errors block) + 20 signals
...
| `operations-lint.py` | error | OPS files (agenda/slides/interactives) cross-link chain. **+ §7 라이트 고정** (2026-08-31 추가): `interactives/*.html`에 `prefers-color-scheme: dark` / `[data-theme="dark"]`가 있으면 error. 이 규칙은 네 차례 지적되고 `OPERATIONS.md` §7에 hard rule로 박혀 있었는데도 **기계 판독 고리가 없어** 2026-08-31 `2026-08-31_it-stability-clinical-tool.html`이 미디어쿼리 다크 + `[data-theme]` + 토글 버튼을 다 갖고 태어났다 — 당시 감사 22개 중 operations-lint는 frontmatter만, interactive-staleness는 `source_wiki` 날짜만 보고 있었다. **signal이 아니라 error인 이유**: signal-not-gate는 *판단이 개입하는* 감사(인제스트 압력·종합 백로그)를 겨눈 원칙이고, 이건 리터럴 CSS 토큰 두 개의 결정론적 존재 검사라 오탐이 원리상 없고 고치는 데 판단이 필요 없다 — `lint.py`(YAML 구조)와 같은 부류다. 도입 시점 실측 87개 중 위반 0이라 block이 오늘 아무것도 막지 않는다. **`EXEMPT_FILES`(자동생성 대시보드)에도 이 검사만은 적용한다** — 생성기가 다크 CSS를 뱉기 시작하면 진료실 화면에서는 똑같이 다크이고, 고칠 곳이 파일이 아니라 생성기일 뿐이다 |
