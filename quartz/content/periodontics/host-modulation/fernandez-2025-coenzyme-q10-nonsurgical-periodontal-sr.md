---
title: "Clinical efficacy of adjunctive use of coenzyme Q10 in non-surgical periodontal treatment: A systematic review"
authors: Fernandez MDS, Martins TM, Meza-Mauricio J, Ribeiro MC, Silva FH, Casarin M, Muniz FWMG
year: 2025
date: 2025-02-07
doi: 10.1111/eos.70002
source: fernandez-2025-coenzyme-q10-nonsurgical-periodontal-sr.md
category: [periodontics/host-modulation]
evidence_level: sr+ma
source_collection: pubmed-text
full_text: false
pmid: "39920883"
source_url: https://doi.org/10.1111/eos.70002
text_path: /Users/oracleneo/llm-wiki/papers/fernandez-2025-coenzyme-q10-nonsurgical-periodontal-sr.txt
text_filename: fernandez-2025-coenzyme-q10-nonsurgical-periodontal-sr.txt
tags: [coenzyme-q10, coq10, antioxidant, host-modulation, nutraceutical, periodontitis, adjunct, systematic-review]
relations:
  - type: contradicts
    target: rasoolzadeh-2022-coenzyme-q10-periodontitis-sr-ma
  - type: reinforces
    target: heo-2022-omega-3-fatty-acids-periodontitis-ma
---

## Three-line Summary

PROSPERO-registered systematic review (10 RCTs, search to May 2024) of CoQ10 as an adjunct to SRP for periodontitis, stratified by delivery route — local gel vs oral supplementation.

Locally delivered CoQ10 gel (topical/intra-pocket) had no significant effect on PD or CAL; oral supplementation at 120 mg/day gave small but significant improvements (PD −0.41 mm, 95% CI 0.02–0.80; CAL −0.52 mm, 95% CI 0.26–0.78) at ~12 weeks.

CoQ10 gel marketed for gum health lacks efficacy evidence; oral 120 mg/day shows a signal but is overall very-low-certainty evidence, insufficient for an evidence-based recommendation and contradicting the earlier pooled MA (Rasoolzadeh 2022) that mixed delivery routes.

## 세줄요약

PROSPERO 등록 체계적 고찰 (10편 RCT, 검색 2024년 5월까지): SRP 보조제로서의 코엔자임 Q10 (CoQ10)을 투여 경로별(국소 겔 vs 경구 보충제)로 층화 분석.

국소 CoQ10 겔(도포/치주낭내)은 치주낭깊이(PD)·임상부착수준(CAL)에 유의한 효과 없음; 경구 120 mg/일은 12주 시 소폭 유의 개선(PD −0.41 mm; CAL −0.52 mm).

'잇몸 제품'으로 판매되는 CoQ10 겔은 근거 없음; 경구 120 mg/일은 신호는 있으나 근거 확실성 very low로 권고 불가, 투여 경로를 혼합하여 겔 효과를 과장한 이전 MA (Rasoolzadeh 2022)와 직접 상충한다.

## Summary

This 2025 European Journal of Oral Sciences systematic review (10 RCTs, PROSPERO-registered, search to May 2024) is the most current and most carefully designed appraisal of **coenzyme Q10 (CoQ10)** as an adjunct to non-surgical periodontal treatment (SRP). Its decisive move is **route stratification** — separating the four trials of *local* CoQ10 (one topical, three intra-pocket) from the six trials of *oral* supplementation, rather than pooling them.

The result reframes the marketing claim: the **gel route — the one sold as a "gum" product — showed no significant effect** on PD or CAL. Only **oral supplementation at 120 mg/day** produced statistically significant but small improvements (PD −0.41 mm, CAL −0.52 mm) at ~12 weeks. Crucially, the overall **certainty of evidence is very low**, so the review stops short of an evidence-based recommendation.

This contradicts the earlier pooled meta-analysis **Rasoolzadeh 2022**, which mixed routes and gingivitis-leaning indices (Plaque/Bleeding/Gingival index) and concluded *in favour* of CoQ10 gel. The disagreement is methodological: pooling routes inflates the apparent gel benefit. For practice, CoQ10 should be presented to patients as low/very-low-certainty, route-dependent, and adjunctive only — not a substitute for scaling and maintenance.

## Key Contributions

- Route-stratified analysis isolates that the **gel route lacks effect** while only **oral 120 mg/day** moved PD/CAL.
- Formal GRADE certainty: **very low** — no firm recommendation.
- Anchors the CoQ10 node of the periodontal-adjunct supplement map and exposes the conflict with the pooled 2022 MA.

## Methodology

- 10 RCTs, adults with periodontitis, SRP ± CoQ10; five databases to May 2024.
- Subgroups: local (topical n=1; intra-pocket n=3) vs oral supplementation (n=6).
- Risk of bias + GRADE certainty assessed; meta-analytic mean differences for PD and CAL.

## Results

| Route | PD reduction | CAL gain | Significant? |
|---|---|---|---|
| Local CoQ10 gel (topical/intra-pocket) | — | — | No significant effect |
| Oral 120 mg/day | −0.41 mm (95% CI 0.02–0.80) | −0.52 mm (95% CI 0.26–0.78) | Yes (12 wk) |

Overall certainty of evidence: **very low**.

## Related Papers

- [[periodontics/host-modulation/rasoolzadeh-2022-coenzyme-q10-periodontitis-sr-ma]] — earlier CoQ10 SR+MA endorsing gel; the direct contradiction (route-pooling artifact).
- [[periodontics/host-modulation/heo-2022-omega-3-fatty-acids-periodontitis-ma]] — sibling host-modulation/nutraceutical adjunct (omega-3), also small-effect.
- [[periodontics/sanz-2020-treatment-stage-i-iii-periodontitis-efp]] — EFP guideline framing for SRP adjuncts.
- [[periodontics/non-surgical-instrumentation/smiley-2015-nonsurgical-periodontitis-srp-guideline]] — mechanical-therapy backbone.
- [[overviews/periodontics-treatment-ladder]] — where antioxidant/host-modulation adjuncts sit in the ladder.
