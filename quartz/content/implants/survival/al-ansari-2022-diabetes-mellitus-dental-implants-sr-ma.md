---
title: "Diabetes Mellitus and Dental Implants: A Systematic Review and Meta-Analysis"
authors: Al Ansari Y, Shahwan H, Chrcanovic BR
year: 2022
date: 2022-04-29
doi: 10.3390/ma15093227
journal: Materials 15(9):3227
source: al-ansari-2022-diabetes-mellitus-dental-implants-sr-ma.md
evidence_level: sr+ma
category: [implants/survival]
pdf_path: /Users/oracleneo/llm-wiki/papers/al-ansari-2022-diabetes-mellitus-dental-implants-sr-ma.pdf
pdf_filename: al-ansari-2022-diabetes-mellitus-dental-implants-sr-ma.pdf
source_collection: external
tags: [diabetes, implant-failure, MBL, OR, meta-analysis, T1DM, T2DM, maxilla, marginal-bone-loss]
relations:
  - type: refines
    target: andrade-2021-survival-rate-peri-implant-evaluation
---

## Three-line Summary

SR+MA (89 publications, 68,290 implants; 5,510 diabetic vs 62,780 non-diabetic) evaluating dental implant failure odds and marginal bone loss (MBL) in diabetic vs non-diabetic patients, stratified by DM type and jaw location.

Diabetic patients had 77.7% higher implant failure odds (OR 1.777, p<0.001); T1DM carried significantly greater risk than T2DM (OR 4.477); MBL was +0.776 mm greater in DM and widened by +0.032 mm/month; the failure difference was significant only in the maxilla, not the mandible.

Maxillary implants in diabetes require stricter HbA1c monitoring and longer surveillance; posterior mandible implants appear relatively safe even in DM; T1DM warrants a more conservative approach than T2DM.

## 세줄요약

당뇨 vs 비당뇨 환자의 임플란트 실패 오즈 및 변연골소실(MBL)을 당뇨 유형·악골 위치별로 분류한 SR+MA (89편, 총 68,290개 임플란트; 당뇨 5,510 vs 비당뇨 62,780).

당뇨 환자의 임플란트 실패 위험 77.7% 높음(OR 1.777, p<0.001); T1DM이 T2DM보다 유의하게 고위험(OR 4.477); MBL +0.776 mm·월 +0.032 mm 증가; 실패 차이는 상악에서만 유의(하악은 유의하지 않음).

상악 임플란트는 당뇨 환자에서 엄격한 HbA1c 조절·장기 모니터링 필요; 하악 구치부 임플란트는 당뇨에서도 상대적으로 안전; T1DM은 T2DM보다 보수적 접근이 필요하다.

## Summary
Largest meta-analysis on DM and dental implants (89 publications, 68,290 implants total). Diabetic patients have **77.7% higher implant failure risk** (OR 1.777, p<0.001). Type 1 DM carries a significantly greater risk than Type 2 (OR 4.477). MBL is 0.776 mm greater in DM patients and widens over time.

## Meta-Analysis Results

| Outcome | DM vs Non-DM | p-value |
|---|---|---|
| Implant failure (OR) | **1.777** | <0.001 |
| T1DM vs T2DM failure | OR **4.477** | 0.032 |
| MBL difference | **+0.776 mm** | 0.027 |
| MBL gap per additional month | +0.032 mm/mo | <0.001 |
| Maxilla failure difference | Significant | — |
| Mandible failure difference | **Not significant** | — |

## Key Interpretation
- **Mandible**: DM patients do NOT show significantly higher failure → posterior mandible implants relatively safe even in DM
- **Maxilla**: DM significantly increases failure → extra caution in upper jaw
- MBL diverges over time → long-term monitoring more critical in DM patients
- OR decreases slightly with longer follow-up → surviving implants in DM eventually become stable

## Clinical Takeaway
- Maxillary implants in DM: higher risk → stricter HbA1c control, longer monitoring
- MBL monitoring mandatory at every recall in DM patients (gap widens over time)
- T1DM: significantly more risky than T2DM → more conservative approach warranted

## Related Papers
- [[implants/survival/andrade-2021-survival-rate-peri-implant-evaluation]] — **refines**: Andrade's SR+MA finds no immediate-loading survival penalty *even in uncontrolled DM2*; this larger 89-study SR+MA qualifies that optimism by showing the DM failure signal is real but **localized to the maxilla and to long-term MBL** (which widens over time) — i.e. the two agree on short-term mandibular survival but conflict on maxillary/long-term risk. See [[overviews/diabetic-patient-immediate-implant-decision]].
- [[implants/survival/wagner-2022-diabetes-mellitus-dental-implants-sr]] — SR: controlled DM = comparable to healthy; HbA1c <8% threshold; antibiotic protocol
- [[implants/survival/moy-2005-dental-implant-failure-rates-risk]] — DM RR 2.75; long-term cohort 4,680 implants
