---
title: "Gas Plasma Treatment Improves Titanium Dental Implant Osseointegration—A Preclinical In Vivo Experimental Study"
authors: Nevins M, Chen CY, Parma-Benfenati S, Kim DM
year: 2023
date: 2023-10-01
doi: 10.3390/bioengineering10101181
source: nevins-2023-gas-plasma-titanium-implant-osseointegration.md
category: [implants/surface/plasma]
evidence_level: animal
pdf_path: /Users/oracleneo/llm-wiki/papers/nevins-2023-gas-plasma-titanium-implant-osseointegration.pdf
pdf_filename: nevins-2023-gas-plasma-titanium-implant-osseointegration.pdf
source_collection: external
tags: [plasma, gas-plasma, chairside, osseointegration, BIC, foxhound, time-course, SLA]
---

## Three-line Summary

Preclinical in vivo (6 foxhounds, 36 SLA implants, split-mouth randomized, 2/4/6 wk sacrifice): chairside gas-plasma applied immediately before placement was tested against untreated controls with histomorphometric and radiographic endpoints.

BIC was numerically higher for plasma at all timepoints and reached significance at 4 weeks (p=0.046); radiographic marginal bone level was significantly better for plasma at 6 weeks (p=0.016) while earlier timepoints were non-significant.

Benefits are timepoint-specific rather than uniformly significant, per-timepoint n is only 2 animals, and no ISQ, resonance frequency, or clinical human data are available.

## 세줄요약

줄1: 전임상 in vivo(foxhound 6마리, SLA 임플란트 36개, split-mouth 무작위, 2·4·6주 희생): 식립 직전 chairside gas-plasma vs 무처리 대조, 조직형태계측+방사선 평가.

줄2: BIC 전 시점 플라즈마군 높고 4주 유의(p=0.046); 방사선 변연골 수준 6주 유의(p=0.016), 이전 시점은 NS.

줄3: 유의성이 특정 시점에만 나타나고 시점당 n=2마리에 불과하며, ISQ·공진주파수·인체 임상 데이터 없음.

## Summary

Validates the chairside, immediately-before-placement plasma protocol in a large animal with a proper randomized split design and time-course. The benefit is real but timepoint-specific: BIC trended higher throughout and hit significance at 4 weeks, while the radiographic bone-level advantage emerged at 6 weeks. Per-timepoint samples are small.

## Key Contributions

- Randomized large-animal validation of chairside pre-placement gas plasma.
- Time-course histomorphometry + radiography.

## Methodology

- 6 male foxhounds; mandibular premolar/first-molar extraction, 42-day socket healing.
- 36 implants, mandibles randomized plasma vs control; sacrifice 2/4/6 wk (2 animals each). SLA surface.

## Results

- BIC higher for plasma at all timepoints; significant at 4 wk (p=0.046).
- Radiographic bone level: NS at 2 & 4 wk; significant at 6 wk (p=0.016, more control bone loss).

**Boundary (living-document):** small per-timepoint n (2 animals); significance is timepoint-specific, not uniform; no RTQ/clinical/loading data; animal only.

## Related Papers

- [[implants/surface/plasma/naujokat-2019-cold-argon-plasma-osseointegration-pigs]] — pig, 8 wk.
- [[implants/surface/plasma/kahm-2024-vacuum-plasma-surface-treatment-osseointegration]] — rabbit (non-significant).
- [[implants/surface/plasma/zheng-2020-non-thermal-atmospheric-plasma-titanium]] — rat + mechanism.
- [[implants/surface/plasma/pesce-2020-photo-plasma-activation-titanium-sr-ma]] — meta-analysis.
