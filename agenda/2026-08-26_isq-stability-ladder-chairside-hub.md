...
- [x] **입력**: 부위(상/하악 × 전치/구치), 골질 Type I–IV, 동반술식(없음/상악동거상/GBR), 발치와(신선/치유), ISQ 4방향, 삽입토크(Ncm)
- [x] **보정 임계값 자동 계산** — 기준 70에 상악동거상·GBR +3, Type IV +2 (ladder 축2 규칙)
- [x] **4방향 편차 검사** — 1방향만 평균과 ≥10 차이 시 재측정 경고
- [x] **부하 결정 3분기** — 즉시(ISQ≥임계 + IT≥35) / 조기·재측정(60–69) / 통상(<60)
- [x] **신선 발치와 예외** — ISQ 인위적 저하(~57 vs 치유골 ~72) → IT 우선 기준으로 전환
- [x] **ISQ↔IT 불일치 경고** — 상관 r=0.44·I²>90%라 둘이 갈릴 수 있고, 갈리는 것 자체가 정보
- [x] **30일 stability dip 궤적** — 즉시부하 선택 시 Balshi 2005 곡선(70.35→66.38→68.01→68.82)과 "60일 이전 프로토콜 변경 금지"
- [x] **축3 예측 패널** — 피질골 두께·직경·악궁 위치로 식립 전 ISQ 기대치
- [x] **5개 전문 도구 라우팅** — 각 결정 지점에서 해당 도구로
- [x] 모든 수치에 인용 표기, 라이트 배경 고정, frontmatter는 HTML 주석
- [x] `operations-lint` 통과 · `interactive-staleness` OK

# Notes / Decisions

- 2026-08-26: **브라우저 실측 검증 6케이스 통과** — ①기본(하악·전치·II·치유골, ISQ 72.0/IT 40) 임계 70 → 즉시부하 충족 ②증대+Type IV 임계 **75로 상향돼 같은 ISQ의 판정이 뒤집힘**(보정이 장식이 아님을 확인) ③4방향 편차 10.8 → **판정 보류**로 차단(딥 카드도 숨김) ④신선 발치와+IT 40 → 토크 1차 기준으로 전환 ⑤ISQ 74/IT 25 → r=0.44 불일치 경고 ⑥ISQ 52 → 통상+추가치유. 콘솔 에러 0 · `operations-lint` OK 168 / ERRORS 0 · 다크모드 블록 없음(OPERATIONS.md §7 준수).
- 2026-08-26: 이 도구는 **동시 실행된 다른 세션**이 저작·커밋했다(`689c98e`, 01:52:09). 본 세션은 같은 파일을 다시 쓰지 않고 검증만 수행한 뒤 이 agenda의 완료 처리를 맡았다 — 같은 트리를 두 세션이 만질 때의 표준 처리.
