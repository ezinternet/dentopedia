---
name: ingest-paper
description: 'Add dental research PDF(s) to the LLM Wiki. Full pipeline: copy
  PDF → write sources/*.md → write wiki/{category}/*.md → update index.md → qmd
  re-index. One paper runs serially; 2+ papers take a parallel fan-out (one
  subagent per paper, then serial finalize) so wall-clock ≈ the single slowest
  paper. Invoke when the user says "논문 추가", "ingest", "인제스트", or provides PDF
  path(s). Always run lint after completing.'
