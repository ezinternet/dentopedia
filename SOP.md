...
작성: 2026-05-21 · v1 / 개정: 2026-06-23 · v2 / 2026-07-15 · v3 / 2026-07-19 · v3.1 (relations: 방향 규칙 추가, 같은 날 축 정정: 위키등재순→발행연도순)
...
## 1-quater. `relations:` 방향 규칙 (2026-07-19 추가, 같은 날 축 정정)

> 필드 전체 스펙(vocabulary 5종·순환 참조 금지 등)의 SSOT는 **`INGEST.md`**다. 여기는 방향(누가 누구를 가리키나)만 운영 절차로 푼 것.

**축은 "논문 발행연도"다. "위키 등재 순서"가 아니다.**

`extends`/`reinforces`/`refines`는 전부 지적 선후관계 주장이다 — "이 논문이 저 논문을 딛고/독립적으로 확인/좁혔다"는 건, target이 citing page보다 **먼저 발행됐을 때만** 참이다. 2013년 RCT는 2017년 SR을 `extends`할 수 없다 — 4년 뒤에 나올 논문을 딛고 설 방법이 없다. 두 가지 다른 제약을 섞지 말 것:

1. **기계적 제약** (파일시스템 문제일 뿐): `target`은 이미 `wiki/`에 존재하는 stem이어야 한다 — 없는 페이지는 못 가리킨다. 필요조건이지 충분조건이 아니다.
2. **내용 제약** (주장을 참으로 만드는 실제 조건): `target`의 발행일이 citing page의 발행일보다 **같거나 이전**이어야 한다. `type:` 선택을 지배하는 건 이 축이다.

둘 다 만족할 때만(= target이 이미 위키에 있고 **+** 발행연도가 더 이르다) typed edge를 넣는다. 기계적 제약만 만족하면(위키엔 있지만 target이 **더 늦게** 발행됨) **엣지를 넣지 않는다** — 진짜 엣지가 있다면 그건 target 페이지 쪽에 (거꾸로) 달려야 하는데, 오늘 인제스트 범위 밖의 옛 페이지를 고치는 건 forward-only 위반(`superseded_by`와 같은 원칙)이라 지금은 못 넣는다. `## Related Papers` 프로즈로만 남기고, 필요하면 `relation-judgment` deviation으로 "나중에 target 페이지 쪽에서 역방향 엣지 검토 후보"라고 로그해둔다. 두 발행일이 너무 가까워(같은 달, 정확한 날짜 불명) 순서를 확신 못하면 방향 미확정으로 보고 역시 프로즈만 쓴다.

- **같은 배치로 병렬 인제스트되는 형제 논문끼리는 발행연도와 무관하게 서로 안 보인다.** Phase 1 서브에이전트 A, B가 동시에 파일을 쓰는 중이면 A는 B의 stem을(아직 디스크에 없으므로) typed edge로 못 가리킨다 — 프로즈만. 나중에 둘 다 커밋되고 발행연도 확인 후 새 인제스트가 엣지를 추가할 수 있다.

**실측 사례** (2026-07-19): 골막이완절개 8편 일괄 인제스트에서 이 규칙이 처음엔 거꾸로("위키 등재 순서" 축) 정리됐고, 그 상태로 typed edge 3개가 시간역행으로 들어갔다 — `ogata-2013 extends→ plonka-2017`(2013이 2017보다 먼저인데 나중 논문을 extends), `bahaa-2022 extends→ attia-2023`, `pohl-2023 reinforces→ wang-2025`. 셋 다 `scripts/relations-audit.py`(target 존재 여부만 검사, 발행일 선후는 검사 안 함)와 lint를 통과했고, 사용자가 Ogata 엣지를 직접 지적하고서야 발견됐다. 세 엣지 모두 제거하고 프로즈 링크로 대체(원래 프로즈가 이미 있어서 관계 자체는 안 없어짐, typed edge만 뺌). 애매한 네 번째(`hur-2025 extends→ raabe-2025-impact`, 둘 다 2025년 2월·정확한 날짜 불명)도 방향 미확정으로 프로즈로 내렸다. **`relations-audit.py`는 아직 발행일 선후를 검사하지 않는다 — citing page `date:` vs target `date:` 비교해 target이 더 늦으면 잡는 체크가 없다(향후 추가 후보).**
