...
- Tier 2 (대상 불명/soft, review): **584**
- (억제됨) 이미 typed 엣지가 있어 제외: **245** · 동일 줄 비최근접으로 Tier 2 강등: **11**
...
- `coyac-2019-preclinical-model-links-osseo-densification-misfit`  —[refut · 반증]→  **`osseodensification-clinical-applications`**
  ...
  - ▸ 대상(`osseodensification-clinical-applications`) 세줄: Fontes Pereira 2023 SR을 spine으로 37편을 4개 임상 시나리오(상악동저 보강·좁은 ridge·저밀도골 D3–D4·즉시식립)에 허브-스포크로 통합. 일관된 이득은 삽입토크(Insertion Torque, IT) 상승 — 벤치·사체·동물·임상 전 계층에서 재현; in vitro 골-임플란트 접촉률(BIC) ~3배; 그러나 임플란트 안정성 지수(Implant Stability Quotient, ISQ)는 2025년 두 독립 인체 SR+MA(Mohammadi 2025 7편 NS·S
...
- `osseodensification-clinical-applications` [overviews] (HIGH-no-target, '상충' · 상충)
  - **근거 문장**: - **임상 ↔ 전임상 상충**: 임상 pooling은 densification에 유리하나, **전임상 모델(양·돼지·쥐)은 BIC·골통합 이득을 보이지 않는다.**
  - ▸ 출발(`osseodensification-clinical-applications`) 세줄: Fontes Pereira 2023 SR을 spine으로 37편을 4개 임상 시나리오(상악동저 보강·좁은 ridge·저밀도골 D3–D4·즉시식립)에 허브-스포크로 통합. 일관된 이득은 삽입토크(Insertion Torque, IT) 상승 — 벤치·사체·동물·임상 전 계층에서 재현; in vitro 골-임플란트 접촉률(BIC) ~3배; 그러나 임플란트 안정성 지수(Implant Stability Quotient, ISQ)는 2025년 두 독립 인체 SR+MA(Mohammadi 2025 7편 NS·S
...
- `clear-aligner-indications-limitations` [overviews] (SOFT→charoenrat-2025-clear-aligner-anterior-open-bite-molar-intrusion-sr-ma, 'whereas' · 반면(대조))
  ...
  - ▸ 출발(`clear-aligner-indications-limitations`) 세줄: 투명교정(Clear Aligner Therapy, CAT) 위키 58편을 효율(착용 프로토콜·개방교합 기전·제품라인별 예측성 포함)·이동특이 한계(발치 Roller Coaster Effect·스피 곡선 성형 실패 포함)·Class II 전략·Class III camouflage/성장기 증례·생체역학/설계(attachment 재료과학·실측 force/moment 포함)·확장·치근흡수/치조골(발치 프로토콜별 위험 포함)·치주(구강미생물총 기전 포함)·저작/턱관절/이갈이·가속보조·환자 기대/동의 11축
...
- `clear-aligner-indications-limitations` [overviews] (HIGH-no-target, 'overturn' · 결론 뒤집음)
  - **근거 문장**: The highest synthesis level available on this axis reaches the same verdict by a different route, and adds the time dimension the others lack. An umbrella review ([[orthodontics/clear-aligner/di-spirito-2023-clear-aligner-periodontal-umbrella-review]], a systematic review of 4 SRs-with-meta-analysis, 2,042 patients: 978 aligner vs 1,064 fixed) pools at the SR layer rather than the trial layer and 
  - ▸ 출발(`clear-aligner-indications-limitations`) 세줄: 투명교정(Clear Aligner Therapy, CAT) 위키 58편을 효율(착용 프로토콜·개방교합 기전·제품라인별 예측성 포함)·이동특이 한계(발치 Roller Coaster Effect·스피 곡선 성형 실패 포함)·Class II 전략·Class III camouflage/성장기 증례·생체역학/설계(attachment 재료과학·실측 force/moment 포함)·확장·치근흡수/치조골(발치 프로토콜별 위험 포함)·치주(구강미생물총 기전 포함)·저작/턱관절/이갈이·가속보조·환자 기대/동의 11축

- `clear-aligner-indications-limitations` [overviews] (HIGH-no-target, 'contradict' · 반박·충돌)
  - **근거 문장**: As an adjunct, consumer probiotics reduced aligner biofilm (CFU + EPS) short-term ([[orthodontics/clear-aligner/thakur-2026-probiotics-clear-aligner-biofilm-rct]], crossover RCT n=20), but this is preliminary (label-only verification, no mechanical-cleaning comparator). The mechanism behind CAT's plaque/gingival advantage is now partly characterized at the microbiome level: the first SR unifying m
  - ▸ 출발(`clear-aligner-indications-limitations`) 세줄: 투명교정(Clear Aligner Therapy, CAT) 위키 58편을 효율(착용 프로토콜·개방교합 기전·제품라인별 예측성 포함)·이동특이 한계(발치 Roller Coaster Effect·스피 곡선 성형 실패 포함)·Class II 전략·Class III camouflage/성장기 증례·생체역학/설계(attachment 재료과학·실측 force/moment 포함)·확장·치근흡수/치조골(발치 프로토콜별 위험 포함)·치주(구강미생물총 기전 포함)·저작/턱관절/이갈이·가속보조·환자 기대/동의 11축
