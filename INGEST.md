...
Optional block on the **citing (newer) page**, pointing out to the pages it relates to. **"Newer" means newer *by publication date* — a paper can only extend/reinforce/refine work that already existed when it was written.** See the direction rule below, it is not optional reading.
...
**Direction rule (SSOT — corrected 2026-07-19 after an initial version of this rule got the axis backwards and produced three chronologically-impossible edges; see worked example below).** The axis is **publication chronology**, never wiki-membership order. `extends`/`reinforces`/`refines` all assert an intellectual-priority claim — *this paper built on / independently confirmed / narrowed that paper* — which is only true if the target was published **before** the page you're writing. A 2013 RCT cannot `extends` a 2017 SR; its authors could not have built on a paper that didn't exist yet, no matter which one happens to be typed into the wiki first. Two separate constraints apply, and they must not be conflated:

1. **Mechanical constraint** (file-system/graph-construction only): `target` must already exist as a wiki stem — you cannot link to a page that isn't written yet. This is *necessary but not sufficient*.
2. **Content constraint** (what actually makes the claim true): `target`'s publication date must be **on or before** the citing page's publication date. This is the one that governs `type:` selection.

When both hold — target already exists in `wiki/` **and** predates the page you're writing — add the typed edge. When only the mechanical constraint holds (target exists in `wiki/` but was published *later* than your paper), **do not add a typed edge on your page**: the true edge, if one exists, belongs on the *target's* page (pointing back at yours), and you cannot add it there — editing an old page outside your ingest scope violates forward-only/grandfather, same principle as `superseded_by`. Use a prose `## Related Papers` mention instead, and optionally log a deviation (`relation-judgment`) flagging it as a **candidate reverse-edge for a future ingest/audit pass that revisits the target's page**. If the two publication dates are too close to order with confidence (same month, exact day unknown for one or both), treat direction as unverifiable and default to prose-only rather than asserting either way.

- **Sibling pages in the same parallel-ingest batch are mutually invisible until Phase 2 lands them**, regardless of their publication dates — if paper A and paper B are being fanned out to two subagents in the same batch, neither can add a typed edge to the other (its file doesn't exist on disk yet). Prose-only for those; a *later* ingest touching either topic may add the missing edge once both exist and chronology is checked.
- `contradicts` and `applies-to` are less strictly priority-bound (a later paper can contradict an earlier one in either direction of "who said it first" without either side being wrong about what conflicts), but default to the same before/after check unless there's a clear reason the relationship is symmetric.

**Worked example (2026-07-19).** Ingesting 8 papers on periosteal releasing incision, three edges were added backwards before this rule was corrected: `ogata-2013-... extends→ plonka-2017-...` (2013 predates 2017 — Ogata's RCT cannot extend a paper written 4 years later), `bahaa-2022-... extends→ attia-2023-...` (2022 predates 2023), and `pohl-2023-... reinforces→ wang-2025-...` (2023 predates 2025). All three were caught only because a human reviewer questioned the Ogata edge specifically — the mistake was invisible to `scripts/relations-audit.py` (it validates that targets exist, not that they predate the citing page) and to lint. All three typed edges were removed and replaced with prose-only `## Related Papers` mentions (the prose already existed in every case, so no relationship was lost — only the machine-readable typed edge). A fourth edge (`hur-2025-... extends→ raabe-2025-impact-...`, both February 2025, exact days unknown for one) was also downgraded to prose-only as unverifiable rather than guessed. **`scripts/relations-audit.py` does not currently check publication-date direction — this is a known gap; a chronology check would need to compare each edge's citing-page `date:` against its target's `date:` and flag any target dated after the citing page.**
